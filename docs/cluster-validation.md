...
#### Random-basket p is necessary, not sufficient — three grades of non-tradeability

A low random-basket p establishes only that the cohort co-moves more than chance. It does NOT establish a *tradeable, distinct* cohort: a cohort can pass the null and still be untradeable as a standalone factor. The full verdict needs the random-basket p read together with the threshold scan and the intra-advantage vs the natural benchmark. The June 2026 audit produced clean worked examples of three grades of non-tradeability, ordered by where the failure shows up:

1. **Fails the random-basket null (p ≥ 0.05) — pure dispersion, no factor.** The cohesion is indistinguishable from a random N-pick of comparable names; the label has no return factor at all. Examples: [[Theatrical exhibition]] (p 0.099) and [[DTC Telehealth]] (p 0.107) — both shatter into singletons with negative intra-advantage vs their sector / growth ETF.
2. **Passes only marginally, via shared beta (0.01 ≤ p < 0.05, usually with a negative intra-advantage vs the sector ETF) — a sector-beta factor, nothing cohort-specific.** The names beat random baskets only because they share sector or risk-level beta, not a distinct cluster factor. Example: [[Medtech]] (random-basket 0.0096, vol-matched 0.0143) — it clears random only on shared healthcare beta and correlates more with IHI/XLV than with itself.
3. **Passes at the floor (p < 0.001) yet still not a *distinct* cohort — a real factor, but not a separable one.** This is the grade the p-value alone hides: the cohort genuinely is one factor, but that factor is already owned by a benchmark or parent cohort, so the intra-advantage vs that benchmark is ~0 or negative. Examples: [[Copper equity beta]] (all nulls at the 0.0001 floor, but +0.033 vs the copper price CPER and −0.044 vs the COPX miners ETF — it IS the copper trade) and [[Neoclouds]] (floor on every null, but +0.000 vs the [[Sectors/Crypto-to-AI|Crypto-to-AI]] cohort it merges with). A commodity or single-driver cohort will *always* land here — cohesion is guaranteed when one exogenous price drives every member, which is exactly why the null carries no information for it.

The operating rule: the random-basket null is necessary, never sufficient. Grade 1 is settled by the null itself; grades 2 and 3 are settled by the intra-advantage vs the right benchmark (sector ETF for grade 2, parent cohort / commodity ETF for grade 3) plus the threshold scan. A validated, *tradeable, distinct* cohort must clear all three bars: floor-passing null AND positive intra-advantage vs every relevant benchmark AND a non-zero stable threshold width. [[AI interconnect]] is the instructive near-miss — it passes the nulls but is two tight pairs averaged, so it fails on threshold width and on the optical sleeve's negative advantage vs broad optics.
...
### Sector performance chart (required for Sectors/ cohort notes)

Every cohort-owner note under `Sectors/` must also carry a `## Sector performance` section with a normalized performance chart — the cohort members plotted against the sector ETF the cohort is tested against — kept distinct from the cluster-structure charts. It makes the structure result legible: distinct cohorts visibly diverge from the ETF, ETF-replicable ones track it. Generate via `/api/chart/lw?tickers=<members>,<sectorETF>&normalize=true&primary=<primary>&start=2023-01-01` (Flask test client or chart server), save as `investing/attachments/{slug}-performance.png`, embed under `## Sector performance` with an italicized caption tying the trace to the verdict, and register it in `chart-registry.md` so the refresh plugin keeps it current — unlike the `{prefix}-cluster-*` structure charts, which stay UNregistered as frozen point-in-time validation snapshots (fixed `window_end`/`definition_date`). Legacy notes use the equivalent `{sector}-sector-chart.png` name (e.g. [[WFE]], [[Defense IT Services]]); both satisfy `check_note_compliance.py`. The `cluster-validation` check flags a missing performance chart on Sectors/ cohort notes (currently a migration warning; promote to error once the existing corpus is backfilled). Concepts/ cluster notes (the commodity-beta / equity-beta family) are out of scope.
...
11. Embed the dendrogram + summary table in the actor or concept note. Include p-values in the status callout when available. For cohort-owner notes, include the join-distance table, PC1 index-weight table, and rolling historical-tightness chart, with sentences distinguishing topology from PC1-replication weights and current tightness from historical evolution. For Sectors/ cohort-owner notes, also add a `## Sector performance` section with a normalized members-vs-sector-ETF chart (`{slug}-performance.png`) and register it in `chart-registry.md` — see "Sector performance chart" above. On a *refresh* of an existing cohort-owner note, regenerate BOTH chart families, not just the one from the pipeline you re-ran: (a) the `{prefix}-cluster-*` diagnostics from `cluster_analysis.py`, and (b) the note's registry-driven sector price chart (`{sector}-sector-chart.png`, an `/api/chart/lw` render defined in `chart-registry.md` — regenerate via the Flask test client / chart server). The freshness rule covers every embedded non-event chart; a stale top price chart left behind because only the diagnostic pipeline was re-run is a recurring miss (e.g., WFE's `wfe-sector-chart.png` was ~5 months stale after a Jun 2026 refresh). Re-running on a current window can also surface an un-back-adjusted split in a member's series (a single-day discontinuity at a clean split ratio with a sustained level shift, collapsing intra-corr) — diagnose and correct it before trusting the refreshed numbers (worked cases: BKNG 25:1, KLA 10:1). The price-pattern flag is a candidate, NOT a verdict: a real −44% earnings crash (FISV, Oct 2025) or a 10x rally matches a clean split ratio without being a split. Run `scripts/check_split_discontinuities.py --verify-yf` to cross-check each flag against yfinance's recorded splits — it suppresses no-split flags (real moves) and keeps only genuine un-adjusted splits. Fix a confirmed split with `add_ticker.py TICKER` (re-fetch back-adjusts forward + most reverse splits); when the vendor mis-dates a fresh reverse split, the re-fetch leaves a residual jump and you back-adjust the pre-break closes by the split factor manually (worked Jun 2026: FUBO 1:12 — yf dated it 3 months off the DB jump; OPAD 1:10 — yf dated it 4 days off, so `auto_adjust` left pre-jump closes on the old scale).

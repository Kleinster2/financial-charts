#!/usr/bin/env python3
"""
create_ecpr_index.py

Create Edge Control-Plane Risk (ECPR) basket index.
Tracks edge and runtime-adjacent names most exposed when model vendors
pull execution, state, and orchestration upstream.

Methodology (Apr 2026):
- Weights derived from Apr 10, 2026 Managed Agents selloff
- Bigger drop = higher weight (market-revealed control-plane exposure)
- Constituents: NET, FSLY, AKAM

Usage:
    python scripts/create_ecpr_index.py [--store]
"""
...
ECPR_WEIGHTS = {
    'NET': 0.26,   # Cloudflare (-13.5%)
    'FSLY': 0.42,  # Fastly (-21.7%)
    'AKAM': 0.32,  # Akamai (-16.7%)
}
...
def get_component_prices(start_date: str = '2019-05-17'):
    conn = sqlite3.connect(DB_PATH)
    tickers = list(ECPR_WEIGHTS.keys())
...
def calculate_ecpr_index(start_date: str = '2019-05-17'):
    print(f"\n{'='*70}")
    print("Edge Control-Plane Risk (ECPR) Index")
    print(f"{'='*70}\n")

    print("Constituents:")
    for ticker, weight in ECPR_WEIGHTS.items():
        print(f"  {ticker:12s} {weight*100:5.1f}%")
    print(f"\n  Total: {sum(ECPR_WEIGHTS.values())*100:.0f}%")
    ...
    for _, row in result_df.tail(10).iterrows():
        print(f"  {str(row['Date'])[:10]}: {row['ECPR']:.2f}")

    return result_df


def store_ticker(df: pd.DataFrame):
    df_narrow = df[['Date', 'ECPR']].rename(columns={'ECPR': 'Close'}).copy()
    df_narrow['Ticker'] = 'ECPR'
    ...
    try:
        ...
        conn.commit()
        print(f"  ECPR: upserted {len(rows):,} rows into prices_long")
    finally:
        conn.close()
    ...
    ''', ('ECPR', 'Edge Control-Plane Risk Index', df['Date'].min(), df['Date'].max(), len(df)))
    conn.commit()
    conn.close()

    print("  [SUCCESS] ECPR stored")


def main():
    store = '--store' in sys.argv or '-s' in sys.argv
    result_df = calculate_ecpr_index()
    if result_df is not None and store:
        store_ticker(result_df)
        print("\n[DONE] You can now chart 'ECPR' in your charting app")

# Investing Vault

Personal research vault for investment ideas and market analysis.

---

## Quick stats

| Folder | Count | Description |
|--------|-------|-------------|
| [[#Actors]] | 498 | Companies, people, entities |
| [[#Concepts]] | 117 | Ideas, trends, frameworks |
| [[#Theses]] | 17 | Investment theses |
| [[#Sectors]] | 13 | Sector overviews |
| [[#Regions]] | 4 | Geographic hubs |
| [[#Events]] | 12 | Market events |
| [[#Questions]] | 5 | Research questions |
| [[#Daily]] | 4 | Daily notes |

---

## Sectors

Sector-level analysis and key players:

| Sector | Focus |
|--------|-------|
| [[Semiconductors]] | Value chain, NVIDIA, TSMC, memory |
| [[AI Infrastructure]] | Compute, data centers, power |
| [[Sectors/Robotics\|Robotics]] | Humanoids, industrial, surgical |
| [[Banks]] | Big Four, investment banks |
| [[Insurance]] | P&C, health insurers |
| [[Defense]] | Primes, defense AI |
| [[Cybersecurity]] | Platforms, consolidation |
| [[Energy and Utilities]] | Power generation, nuclear |
| [[Healthcare]] | Pharma, biotech, medical devices |
| [[Consumer]] | Retail, e-commerce, staples |
| [[Transport]] | Railroads, airlines, logistics, autonomous |
| [[SaaS]] | Cloud software, enterprise, SMB |
| [[Biopharma]] | Pharma, biotech, GLP-1, gene therapy |

---

## Regions

Geographic hubs for international markets:

| Region | Focus |
|--------|-------|
| [[Regions/Asia\|Asia]] | China, Taiwan, Korea, Japan, India, SEA |
| [[Regions/LATAM\|LATAM]] | Brazil, Mexico, Venezuela, Cuba |
| [[Regions/GCC\|GCC]] | Saudi Arabia, UAE, Qatar |
| [[Regions/Europe\|Europe]] | Netherlands, Germany, UK, France |

---

## Theses

Active investment theses:

### Long positions

| Thesis | Core idea |
|--------|-----------|
| [[Long NVIDIA]] | AI platform dominance |
| [[Long TSMC]] | Foundry moat |
| [[Long memory]] | HBM cycle |
| [[Long nuclear]] | AI power demand |
| [[Long defense AI]] | Software-defined warfare |
| [[Long datacenter infrastructure]] | Picks & shovels |
| [[Long Broadcom]] | Networking + ASICs |
| [[Long Anthropic]] | AI lab positioning |
| [[Long Apple]] | Ecosystem + AI |
| [[Long Cerebras]] | Wafer-scale niche |
| [[Long inference stack]] | Inference > training |
| [[Long WFE]] | Equipment leverage |
| [[Long OSAT and test equipment]] | Packaging bottleneck |

### Short/relative value

| Thesis | Core idea |
|--------|-----------|
| [[Short standalone ASICs]] | NVIDIA consolidates |
| [[Short TSMC long Korea]] | Samsung/SK Hynix upside |

### Speculative/event-driven

| Thesis | Core idea |
|--------|-----------|
| [[Cuba opening]] | Regime change optionality |
| [[Venezuela oil rebuild]] | Post-Maduro oil production |

---

## Questions

Open research questions:

- [[Can Samsung ever catch TSMC]]
- [[How durable is TSMC's moat]]
- [[Samsung memory vs SK Hynix]]
- [[What happens if Taiwan is disrupted]]
- [[Will Intel Foundry Services succeed]]

---

## Key concepts

High-signal concepts to track:

| Theme | Concepts |
|-------|----------|
| **AI demand** | [[AI hyperscalers]], [[Agentic AI]], [[Power constraints]] |
| **Supply chain** | [[Advanced packaging]], [[Memory shortage 2025-2026]], [[Export controls]] |
| **Geopolitics** | [[Taiwan concentration risk]], [[Sovereign AI race]], [[N-2 rule]] |
| **Energy** | [[Nuclear renaissance]], [[Clean energy for AI]] |
| **New tech** | [[Sectors/Robotics\|Robotics]], [[Inference disaggregation]] |

---

## Recent events

| Event | Date | Impact |
|-------|------|--------|
| [[CES 2026]] | Jan 2026 | NVIDIA Rubin, AMD Helios, robotics |
| [[JPM Healthcare Conference 2026]] | Jan 12-15 | Biopharma M&A, GLP-1, AI deals |
| [[Project Stargate]] | Jan 2026 | $500B AI infrastructure |
| [[Maduro capture 2026]] | Jan 2026 | Venezuela oil catalyst |
| [[DeepSeek day]] | 2025 | China AI capabilities |

---

## Daily notes

- [[2026-01-08]]
- [[2026-01-06]]
- [[2025-12-30]]
- [[2025-12-29]]

---

## Navigation tips

- **Search**: `Ctrl/Cmd + O` for quick open
- **Graph view**: See connections between notes
- **Backlinks**: Check what links to current note
- **Tags**: Filter by `#actor`, `#thesis`, `#concept`, etc.

---

## Meta

- [[Meta/CLAUDE.md|Vault guidelines]]
- [[Meta/Taxonomy discussion|Taxonomy discussion]]

---

*Last updated: 2026-01-09*

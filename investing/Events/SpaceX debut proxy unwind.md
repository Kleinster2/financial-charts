...
- Cluster question (resolved): the cross-section splits cleanly. The four thematic pure-plays (ASTS, LUNR, RKLB, RDW) are members of the validated [[Space pure-plays]] cohort (1Y intra-correlation ~0.61, PC1 ~69%, a single factor that has strengthened since the Nov 2025 regime shift) — they genuinely co-move as a factor outside event windows. The ownership proxy SATS is not a cohort member: its average correlation to the seven space pure-plays is 0.205 over the trailing year — indistinguishable from its 0.208 correlation to [[SPY]] (i.e., market beta) — and it is already a documented exclusion in the cohort's [[Space pure-plays#Missing-name screen — does the cohort have the right boundary?|missing-name screen]] (0.231, "different beta profile"). Tellingly, the SATS-ASTS pair is the weakest of all (0.109): the two names that screened as June 12's cleanest sigma events are the least structurally related. So the unwind was a validated four-name factor plus one structurally-unrelated outlier, both hit by the same idiosyncratic catalyst — catalyst-driven, not a shared cluster. SATS is the [[Destiny Tech100]]-style "tactical SpaceX-event co-mover" the cohort note describes, not a member of the space factor.

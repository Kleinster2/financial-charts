#event #apple #ar #vr #2024

**Apple Vision Pro launch** — February 2024 US launch. $3,499 spatial computer. Mixed reception, slow sales.

---

## Event details

| Attribute | Value |
|-----------|-------|
| Announced | June 2023 (WWDC) |
| US launch | February 2, 2024 |
| Company | [[Apple]] |
| Price | $3,499 |
| Category | Spatial computing / AR/VR |

---

## Specifications

| Spec | Value |
|------|-------|
| Display | Micro-OLED, 23M pixels |
| Chip | M2 + R1 |
| Eye tracking | Yes |
| Hand tracking | Yes |
| Battery | 2 hours external |

---

## Market reception

| Factor | Assessment |
|--------|------------|
| Reviews | Impressive tech, limited use cases |
| Sales | Below expectations (~500K year 1) |
| Developer support | Limited |
| Return rate | Higher than typical Apple |

---

## Use cases

| Use | Viability |
|-----|-----------|
| Entertainment | Strong (movies, spatial video) |
| Productivity | Limited |
| Gaming | Minimal content |
| Enterprise | Exploring |

---

## Investment implications

| Theme | Expression |
|-------|------------|
| Apple diversification | New category bet |
| AR/VR timeline | Still early |
| [[Snap]] competition | AR glasses market |
| Component suppliers | Sony displays, TSMC chips |

---

## Future outlook

| Factor | Expectation |
|--------|-------------|
| Price cuts | Lower-cost version expected |
| Vision Pro 2 | Improved, lighter |
| visionOS | Maturing |
| Developer ecosystem | Growing slowly |

---

## Related

### Actors
- [[Apple]] — manufacturer
- [[Meta]] — Quest competitor
- [[Snap]] — AR competitor

### Concepts
- [[Consumer AI]] — adjacent theme

---

*Created 2026-01-16*

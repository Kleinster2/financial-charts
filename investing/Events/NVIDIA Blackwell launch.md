#event #nvidia #semiconductors #ai #2024

**NVIDIA Blackwell launch** — Next-gen AI GPU architecture announced March 2024. B100/B200 chips. 4x inference improvement.

---

## Event details

| Attribute | Value |
|-----------|-------|
| Date | March 2024 (GTC) |
| Company | [[NVIDIA]] |
| Type | Product launch |
| Significance | Major AI hardware upgrade cycle |

---

## Blackwell specs

| Chip | Specs |
|------|-------|
| B100 | 208B transistors, 4nm |
| B200 | Dual-die design |
| GB200 | Grace Blackwell superchip |
| Memory | HBM3e support |

---

## Performance claims

| Metric | Improvement |
|--------|-------------|
| Inference | 4x vs Hopper |
| Training | 2.5x vs Hopper |
| Energy efficiency | 25x vs Hopper |

---

## Market impact

| Effect | Description |
|--------|-------------|
| NVIDIA stock | Rally on announcement |
| Hyperscaler orders | Massive pre-orders |
| Competition | AMD, Intel further behind |
| Supply chain | [[TSMC]] CoWoS constraints |

---

## Investment implications

| Theme | Expression |
|-------|------------|
| NVIDIA dominance | Extended AI leadership |
| HBM demand | [[SK Hynix]], Micron benefit |
| Packaging | [[TSMC]] advanced packaging |
| Power/cooling | Data center infrastructure |

---

## Related

### Actors
- [[NVIDIA]] — chip maker
- [[TSMC]] — manufacturer
- [[SK Hynix]] — HBM supplier

### Concepts
- [[AI datacenter architecture]] — infrastructure
- [[Advanced packaging]] — CoWoS

---

*Created 2026-01-16*

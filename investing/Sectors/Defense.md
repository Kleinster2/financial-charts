---
aliases: [Defense sector, Defense contractors, Military industrial, Aerospace and defense]
---
#sector #defense #government

# Defense

Global defense and aerospace sector. Structural rearmament underway across US, Europe, and Asia-Pacific.

---

## Global rearmament

Defense spending entering structural upcycle driven by:
- Great power competition (US-China, Russia-NATO)
- Ukraine war depleting stockpiles
- Taiwan contingency planning
- Middle East instability

### Spending trajectory

| Metric | 2025 | 2027E | 2040 (S&P est) |
|--------|------|-------|----------------|
| Global defense spending | **>$2.5T** | **>$3T** | **$6T** |
| Growth | — | Double-digit | +140% vs 2024 |

Sector +55% past year (Jan 2026). Global X expects double-digit spending growth for "foreseeable future."

> "Every advanced weapon system depends on the metal." — S&P Global (on copper/materials constraints)

---

## Regional breakdown

### United States

| Metric | Value |
|--------|-------|
| FY25 budget | ~$895B |
| FY27 proposed | **$1.5T** (+68%) |
| Focus | AI, space, modernization |

See [[Trump defense budget]] for Jan 2026 executive order on capital returns.

### Europe

| Country | 2024 spend | NATO target |
|---------|------------|-------------|
| Germany | ~$80B | 2%+ GDP |
| UK | ~$75B | 2.5% target |
| France | ~$60B | Growing |
| Poland | ~$35B | 4%+ GDP (leader) |

See [[European defense spending]]. EU targeting 3M artillery shells/year by end of decade.

### Asia-Pacific

| Country | Trend |
|---------|-------|
| Japan | Record budgets, constitutional shift |
| South Korea | Domestic industry growth |
| Australia | AUKUS submarine program |
| Taiwan | Asymmetric defense buildup |
| India | Modernization, domestic production |

China defense spending officially ~$230B but likely understated.

---

## Prime contractors

| Company | Market cap | Specialization |
|---------|------------|----------------|
| Lockheed Martin | ~$140B | F-35, missiles, space |
| RTX (Raytheon) | ~$160B | Missiles, engines, avionics |
| [[Northrop Grumman]] | ~$80B | B-21 bomber, space, cyber |
| Boeing Defense | (part of BA) | Aircraft, satellites |
| General Dynamics | ~$85B | Submarines, Gulfstream, IT |
| L3Harris | ~$45B | Communications, ISR |

---

## Drone pure-plays

| Company | Market cap | Focus |
|---------|------------|-------|
| [[AeroVironment]] | ~$9B | Small UAS, loitering munitions (Switchblade) |
| [[Kratos]] | ~$8B | Attritable drones (Valkyrie), hypersonic targets |

Drone business "red hot" (William Blair, Jan 2026). Backlogs surging on [[Golden Dome]], Ukraine lessons, and global rearmament. Both trading at 90-106x forward earnings.

See [[Drone warfare]] for full landscape: cost asymmetry, Ukraine lessons, demand drivers, counter-drone problem.

---

## New defense players

| Company | Focus | Status |
|---------|-------|--------|
| [[Anduril]] | Autonomous systems, AI | Private, ~$14B valuation |
| [[Palantir]] | AI/data analytics | Public, ~$180B |
| [[Shield AI]] | Autonomous aircraft | Private |
| SpaceX | Launch, Starlink | Private |

**Theme:** Software-defined warfare disrupting traditional primes.

---

## Key programs

| Program | Contractor | Status |
|---------|------------|--------|
| F-35 | Lockheed | Production |
| B-21 Raider | Northrop | Production ramp |
| NGAD (6th gen fighter) | TBD | Development |
| Sentinel ICBM | Northrop | Development |
| Columbia-class sub | General Dynamics | Construction |
| Hypersonics | Multiple | Development |
| [[Golden Dome]] | Multiple | Development ($1T+ program) |

---

## Munitions & supply chain

See [[Defense supply chain]] — artillery production ramp (US 93K→1.2M shells/yr, EU targeting 3M/yr), missile production constraints, critical materials (copper, rare earths, titanium, tungsten), propellant bottlenecks.

---

## Investment themes

### Traditional defense

| Theme | Beneficiaries |
|-------|---------------|
| Budget growth | All primes |
| Backlog visibility | Lockheed, RTX, Northrop |
| International sales | F-35 exporters |

### Defense AI

| Theme | Beneficiaries |
|-------|---------------|
| Autonomous systems | [[Anduril]], [[Shield AI]] |
| AI analytics | [[Palantir]] |
| Cyber/electronic warfare | Northrop, L3Harris |
| Space | SpaceX, Northrop |

See [[Long defense AI]].

---

## Sector characteristics

| Factor | Implication |
|--------|-------------|
| **Long cycles** | 5-10+ year programs |
| **Cost-plus contracts** | Margin stability |
| **Classified programs** | Limited visibility |
| **Bipartisan support** | Budget resilience |
| **Workforce constraints** | Skilled labor shortage |

---

## Sector KPIs

| Metric | Definition | Good / Target | Why it matters |
|--------|------------|---------------|----------------|
| **Book-to-Bill** | Orders / Revenue | **>1.0x** | Backlog growth signal (>1 means growing) |
| **Backlog Coverage** | Backlog / Annual Revenue | **>1.5-2.0x** | Revenue visibility (safety net) |
| **Operating Margin** | Op Income / Revenue | **10-12%** | Profitability (capped by gov't contracts) |
| **FMS Mix** | Foreign Military Sales % | **Growing** | Higher margins than domestic US sales |
| **FCF Conversion** | Free Cash Flow / Net Income | **>100%** | Quality of earnings & buyback fuel |

---

## Risks

| Risk | Impact |
|------|--------|
| Budget politics | Sequestration risk |
| Program delays/cancellations | Revenue volatility |
| Fixed-price contracts | Margin pressure |
| Supply chain | Component shortages |
| Competition from new entrants | Primes disrupted |

---

## Valuation

| Segment | P/E (Jan 2026) | Historical |
|---------|----------------|------------|
| Large US primes | **24x** | 16x (Jan 2025) |
| Drone pure-plays (AVAV, KTOS) | **90-106x** | — |
| AI defense (Palantir) | 100x+ | — |

Sector re-rated sharply in 2025 on rearmament thesis. Primes now trading well above historical 15-20x range. Expected EPS growth **+15%** in 2026 (vs 6% historical avg) partially justifies expansion. Drone stocks priced beyond even AI multiples.

---

## Defense industries by region

- [[US Defense Industry]] — world's largest, technology leader
- [[European Defense Industry]] — consolidating, rearmament tailwind
- [[Korean Defense Industry]] — export success story (K9, K2)
- [[Israeli Defense Industry]] — missile defense, drones, cyber
- [[Japanese Defense Industry]] — constitutional shift, 2% GDP target
- [[Turkish Defense Industry]] — drone pioneer (Bayraktar)
- [[Indian Defense Industry]] — import substitution, Make in India

---

## Related

- [[Long global rearmament]] — investment thesis
- [[Long defense AI]] — AI-first defense thesis
- [[Trump defense budget]] — $1.5T FY27, capital return restrictions
- [[European defense spending]] — NATO rearmament
- [[Golden Dome]] — $1T+ missile defense program
- [[Copper]] — critical material, munitions demand
- [[Anduril]], [[Palantir]] — AI-first defense
- [[Defense supply chain]] — munitions ramp, critical materials
- [[Drone warfare]] — attritable concept, Ukraine validation, demand drivers
- [[Northrop Grumman]] — B-21, space

---

*Updated 2026-01-23*

...
### Apr 2026 validation: DeepSeek V4 on domestic clusters

[[DeepSeek]] V4 Preview is the first major public model-side validation of this roadmap. [[CNN]] reported on Apr. 24, 2026 that [[Huawei]] supports V4 with Supernode technology combining large [[Ascend]] 950 clusters, while [[Counterpoint Research|Counterpoint]]'s Wei Sun said V4 runs on domestic chips from Huawei and [[Cambricon Technologies]] rather than NVIDIA hardware.

This does not prove China has closed the frontier-training gap. It does prove that the domestic cluster stack is now good enough to host a flagship open model with 1M-token-context ambitions. The practical implication is ecosystem feedback: once model developers target Ascend clusters, application developers, cloud buyers, and state customers have a stronger reason to accept Huawei's hardware/software stack despite CUDA's maturity advantage.

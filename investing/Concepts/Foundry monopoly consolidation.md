...
### 1. Unit economics barrier

Advanced chips are too expensive to manufacture in-house. The cost to build a leading-edge fab ($20B+) and achieve competitive yields requires scale that only a dedicated foundry can achieve.

| Customer | Why they can't leave |
|----------|---------------------|
...
If TSMC hits packaging targets, customers have no alternative for either foundry or packaging.
...
New architectures (FinFET → GAA/nanosheet) require massive R&D and capital. Only the leader has the cash flow to invest through transitions.

| Transition | Leader advantage |
|------------|------------------|
| Planar → FinFET | TSMC first (2012) |
...
## How the dynamics reinforce each other

```
Higher yields → Lower costs → Better pricing → More customers
     ↑                                              ↓
More R&D ← Higher margins ← Volume growth ← Customer lock-in
     ↑                                              ↓
Packaging moat ← Resource lock-up ← Capital to invest
```

Each advantage feeds the others. The flywheel accelerates.
...
| Win customers | Customers won't risk unproven yields |
...
*Framework adapted from @jenniatech (Jan 2026)*

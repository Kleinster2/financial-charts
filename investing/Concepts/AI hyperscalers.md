...
> Intra-cluster correlation only 0.29 across MSFT/GOOGL/AMZN/META/ORCL — all 5 names are singletons at the 0.4 hierarchical threshold. Conceptual thesis intact (capex drives semi stack); cohort is NOT a tradable basket. The actual validated supplier-chain cluster is [[AI capex chain basket]]. Null-test history tracks the pool cleanups, not the cohort: p = 0.028 (May, polluted pool) → 0.135 (Jun 10, stock-typed pool that still carried ~150 mistyped funds/ETFs) → 0.031 (Jul 1 2026, fully verified 1,062-name stock pool). On the honest null the five DO co-move more than a random 5-pick — but only via shared mega-cap/AI beta: the intra-advantage vs QQQ-style benchmarks stays negative and all 5 remain singletons, so the falsification as a tradable basket stands on structure, not on the permutation test (grade-2 non-tradeability: shared beta, no distinct factor). OOS the ambiguity resolved at 40 post-definition obs (formal, 2026-07-02): OOS-STRENGTHENING — intra 0.262 → 0.385 post-definition (ratio 1.44), beating clean-pool random baskets at p 0.0031. Read together with the clean-pool in-sample pass, the verdict is settled as grade-2: the five are a real, strengthening, shared-AI-beta co-movement — and still not a tradable five-name factor (singletons at the cut, negative advantage vs the mega-cap benchmarks). See "Cluster validation — failed cluster test" section below.
...
Cash-flow inversion: The [[James Anderson]] / [[Morgan Samet]] FT framing makes the buyer-side problem explicit, and the May 8, 2026 FT / [[Visible Alpha]] cash-flow pass puts numbers on it. [[Amazon]], [[Alphabet]], [[Microsoft]], and [[Meta]] are now expected to spend roughly $725B on AI infrastructure in 2026 while combined Q3 FCF compresses toward about $4B. The hyperscalers are still the demand engine, but AI turns them from asset-light software/platform compounds into capital-intensive infrastructure buyers. That does not invalidate the capex thesis; it explains why the validated return cluster sits in [[AI capex chain basket]] rather than in the hyperscaler equities themselves.

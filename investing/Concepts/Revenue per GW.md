...
- [[OpenAI]]: $20B exit ARR at end-2025 on a 1.9 GW footprint — both company-disclosed in the Jan 2026 "A business that scales with the value of intelligence" post, CFO [[Sarah Friar]]'s revenue-tracks-compute-1:1 argument (compute grew 0.2 GW 2023 → 0.6 GW 2024 → ~1.9 GW 2025) — gives $10.5B ARR/GW blended; the same ratio [[Brad Gerstner]] quotes as the "Sarah Friar number" in his 5-6-year payback formula.
- Provenance and planning use (Jun 2026 update): the gigawatts-to-cash framing predates the blog — All-In's hosts date Friar's "1 GW ≈ $10B/yr of revenue" articulation to roughly late 2024 — and Friar confirmed at [[Liquidity 2026]] (All-In, Jun 2 2026) that OpenAI plans outer-year revenue from it directly: near years are bottom-up P×Q, but "when you get into the outer years... this amount of compute should equate somewhat this amount of revenue." The metric is the CFO's planning tool, not just an analyst's lens. She also gave the allocation lever inside the blended number: API revenue per token runs "an order of magnitude" above consumer, and the consumer subsidy is deliberate — so blended $/GW understates what a fully-API-allocated GW would earn. Tang allocates the footprint ~60/40 inference/training, putting inference-allocated monetization at ~$22B/GW. Training capacity generates no revenue directly; whether that is overhead or the product depends on the slope of inference growth.
...
Snowflake's two bases both verify (Jun 12, checked against local `income_statement_quarterly` + the FY26 prints): the (35%) is the GAAP operating margin on a trailing-twelve-month window through Oct 31, 2025 — operating loss −$1.503B ÷ revenue $4.387B = −34.3% ≈ (35%) — the TTM of the four quarters publicly reported as of his Mar 1 post, not the last full fiscal year (FY2025, ended Jan 2025, was −40.2%). And the $3B OI/GW implies $3B ÷ $34B = 8.8%, matching Snowflake's FY26 non-GAAP operating-margin guide of 9% (Q3 FY26 printed 11%, +450bp YoY). So the cell is accurate on both bases; the only fault is displaying them unlabeled in one row — which makes the column look self-contradictory ($34B revenue, +$3B operating income, negative margin). The bridge between the two bases is almost entirely stock-based compensation: Snowflake's equity comp runs roughly mid-30s percent of revenue (≈ $1.5B/yr against ~$4.4B TTM revenue), and non-GAAP excludes it (plus payroll tax on equity and intangible amortization) — accounting for essentially all of the ~43-point gap between −34.3% GAAP and +8.8% non-GAAP. The chart's $3B dot is therefore profit before paying employees in stock; a GAAP-consistent dot (−$11.7B/GW) would sit far off the left edge of an axis that starts at $0, which is presumably why the substitution was made.

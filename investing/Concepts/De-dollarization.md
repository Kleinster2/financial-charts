#concept #macro #geopolitics #currency

**De-dollarization** — Reducing USD dominance in global trade/reserves. BRICS efforts, sanctions response, slow progress.

---

## What is de-dollarization

| Aspect | Description |
|--------|-------------|
| Definition | Shift away from USD in trade, reserves, finance |
| Drivers | US sanctions, geopolitics, BRICS ambitions |
| Reality | Very slow, USD still dominant |

---

## USD dominance (current)

| Metric | USD share |
|--------|-----------|
| FX reserves | ~58% |
| FX transactions | ~88% |
| Trade invoicing | ~40-50% |
| SWIFT payments | ~40% |

---

## De-dollarization efforts

| Initiative | Status |
|------------|--------|
| BRICS currency | Discussed, no progress |
| Yuan internationalization | Growing slowly |
| Bilateral local currency | Limited use |
| Gold accumulation | Central bank buying |
| Digital currencies | CBDC experiments |

---

## Why USD remains dominant

| Factor | Explanation |
|--------|-------------|
| Network effects | Everyone uses it |
| Deep markets | Treasury liquidity |
| Rule of law | Property rights |
| No alternative | Euro, yuan have issues |
| Military backing | Petrodollar |

---

## China's yuan push

| Mechanism | Progress |
|-----------|----------|
| Trade settlements | Growing with partners |
| CIPS | Alternative to SWIFT |
| [[Currency swap lines]] | 40+ agreements, ¥4.5T total |
| Belt and Road | RMB loans |
| Oil trades | Some with Saudi, Russia |
| Reserves | ~2.5% globally |

Limited by capital controls. See [[Currency swap lines]] for detailed network analysis.

---

## Investment implications

| Scenario | Impact |
|----------|--------|
| Gradual decline | Slow USD weakness |
| Status quo | USD maintains premium |
| Crisis catalyst | Sharp moves possible |

---

## What to watch

| Signal | Meaning |
|--------|---------|
| Reserve allocation shifts | Central bank decisions |
| Trade settlement changes | Invoicing currency |
| BRICS summits | Policy announcements |
| Sanctions expansion | Accelerates alternatives |

---

## Related

### Concepts
- [[Currency swap lines]] — PBOC network vs Fed network
- [[BRICS]] — alternative bloc
- [[Petrodollar]] — oil-USD link
- [[Digital Yuan]] — CBDC

### Actors
- [[China]] — yuan promoter
- [[Saudi Arabia]] — petrodollar key
- [[Kenneth Rogoff]] — *Our Dollar, Your Problem* (2025)

---

*Created 2026-01-16*

---
aliases: [cryptocurrency, blockchain, digital assets]
---
#concept #fintech #crypto

**Crypto** — Blockchain-based digital assets. Relevant to vault as: tokenization infrastructure, fintech disruption, speculative asset class, and Bitcoin as "digital gold."

---

## Why crypto matters for this vault

| Relevance | Connection |
|-----------|------------|
| Tokenization | Infrastructure for [[Tokenized private shares]] |
| Fintech | [[Robinhood]], [[Coinbase]] revenue driver |
| Bitcoin | [[Block]] strategy, store of value thesis |
| Stablecoins | USDC ([[Circle]]), payments infrastructure |
| AI intersection | Compute tokens, AI model marketplaces |

---

## Key assets

| Asset | Description |
|-------|-------------|
| Bitcoin (BTC) | Digital gold, store of value |
| Ethereum (ETH) | Smart contract platform |
| Stablecoins | Dollar-pegged (USDC, USDT) |
| Others | Thousands of altcoins |

---

## Infrastructure companies

| Company | Role | Ticker |
|---------|------|--------|
| [[Coinbase]] | Exchange, custody | COIN |
| [[Block]] | Cash App Bitcoin | SQ |
| [[Robinhood]] | Trading | HOOD |
| [[Circle]] | USDC stablecoin | Private |
| [[PayPal]] | PYUSD stablecoin | PYPL |

---

## Regulatory landscape (US)

| Era | Stance |
|-----|--------|
| Gensler SEC (2021-2024) | Hostile, enforcement actions |
| Atkins SEC (2025+) | Pro-innovation, clearer rules |

**Tailwind:** New administration more crypto-friendly.

---

## 2025: The Turning Point

A watershed year for US crypto policy under [[Trump II]]:

| Timeline | Event | Impact |
|----------|-------|--------|
| **Jan 2025** | Paul Atkins appointed SEC Chair | End of "regulation by enforcement" (Gensler out). |
| **Q2 2025** | **SAB 121 Repealed** | Banks ([[JPMorgan Chase]], etc.) allowed to custody crypto assets. |
| **July 2025** | **[[GENIUS Act]] Passed** | Federal stablecoin framework; legitimizes USDC. |
| **Q4 2025** | **[[Strategic Bitcoin Reserve]]** | Lummis Bill introduced; US Gov to hold BTC. |

**Theme:** Shift from "Crackdown" to "Strategic Asset."

---

## Tokenization thesis

**Crypto infrastructure for real-world assets:**
- Private company shares (OpenAI, SpaceX tokens)
- Real estate
- Art, collectibles
- Securities

See [[Tokenized private shares]] for deep dive.

---

## Bitcoin thesis

**Digital gold / store of value:**
- Fixed supply (21M cap)
- Decentralized
- Inflation hedge (debated)
- Institutional adoption growing

**Corporate treasury:** Some companies hold Bitcoin (MicroStrategy, Tesla, Block).

---

## Investment approaches

| Approach | Vehicles |
|----------|----------|
| Direct | Buy Bitcoin/ETH on exchange |
| ETF | Spot Bitcoin ETFs (approved 2024) |
| Equities | COIN, SQ, MARA, RIOT |
| Private | VC in crypto startups |

---

## Risks

| Risk | Description |
|------|-------------|
| Volatility | 50%+ drawdowns common |
| Regulatory | Rules still unclear |
| Security | Exchange hacks, wallet loss |
| Fraud | Scams, rug pulls |
| Obsolescence | Technology risk |

---

## AI intersection

**Emerging themes:**
- Compute marketplaces (decentralized GPU)
- AI model marketplaces
- Data marketplaces
- AI agent payments

Early stage, mostly speculative.

---

## Related

- [[Meme coins]] — speculative tokens (DOGE, AI agent coins, celebrity tokens)
- [[Tokenized private shares]] — key application
- [[Coinbase]] — largest US exchange
- [[Block]] — Bitcoin-focused fintech
- [[Robinhood]] — crypto trading
- [[Circle]] — USDC stablecoin
- [[PayPal]] — PYUSD stablecoin


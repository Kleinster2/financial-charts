...
> The three exhibitor names do not trade as one factor: intra-corr 0.328 (below the 0.50 floor), PC1 only 55.5% with a large PC2 (27.6%), and hierarchical clustering never returns them as a clean cluster at any threshold (zero stable width). The decisive test is the random-basket null — the cohort's cohesion is statistically indistinguishable from a random 3-pick of comparable US-listed names (random-basket p 0.099, vol-matched p 0.108, both failing to reject), even though the independence null is rejected (p 0.0001). [[AMC Entertainment]] is a levered meme stock (71% annualized vol; correlates just 0.17 with [[IMAX]]), [[IMAX]] is an asset-light premium-format licensor, and [[Cinemark]] is the only clean operating exhibitor. Cohesion has declined steadily — 0.459 in 2021 to 0.328 in 2026 (holdout WEAKENED, ratio 0.81). See below.
...
| Random-basket p (10k) | 0.099 | FAILS to reject — cohesion equals a random 3-pick |

#concept #commodity #battery #ev

# Lithium

**Lithium** — Battery metal rebounding from 2023-24 crash. +58% YoY as of Jan 2026. Deficit expected 2026 as EV demand grows.

---

## Synthesis

The lithium rally has prices embedded in it. Those prices implicitly answer the question "how much lithium does the world need to mine to meet future battery demand?" The rally prices three things accurately — supply tightening ([[China]] permit cancellations, Zimbabwe export suspension, [[Albemarle|Kemerton]]), supply response ([[Brazil]], [[Direct lithium extraction|DLE]], [[Argentina]]), and demand acceleration ([[IEA]]'s +30% 2024 vs ~10% CAGR baseline). It also attempts to price the fourth thing: the share of future battery demand that will not be lithium at all.

Two forces are actively subtracting from what-would-have-been lithium demand:

1. **[[Sodium-ion batteries|Sodium-ion]] substitutes at the cell level** — different chemistry. Every [[BYD]] Sea Gull EV or grid-storage site running [[CATL]] Naxtra is a unit of demand that uses zero lithium. Energy density still constrains sodium-ion to the lower-range EV segment plus most of grid storage, but at CATL's 20-30% LFP-replacement target in small vehicles and majority grid-storage share by 2028, that is 5-10% of global lithium demand that evaporates.
2. **[[Battery recycling|Recycling]] substitutes at the mine level** — same chemistry, different source. Every tonne of recycled lithium does not need to be mined. At IEA's and Wood Mackenzie's modeled ~20% secondary supply by 2030, that is another ~500,000 tpa LCE where primary mining capacity is not needed.

Combined: roughly 25-30% of what the 2022 bullish forecasts modeled as primary-mining demand growth is being covered by non-lithium chemistry plus recycled supply. Both ramp on the same 2026-2030 timeline the deficit forecasts use — sodium-ion goes commercial in 2026 (GB 38031-2025 standard), recycling reaches 20% by 2030.

The bull case requires sodium-ion ramp slower than CATL's roadmap AND recycling below 20% — both have to underperform simultaneously. The bear case is the opposite.

**Forecast spread — the rally prices one point in it, not all of it:**

- [[Fastmarkets]] (2026 deficit), Wood Mackenzie (2028 deficit), IEA (2030s deficit) disagree on timing by 2-5 years.
- Both Wood Mackenzie and IEA explicitly model sodium-ion and recycling in their deficit forecasts, so substitution is partially priced via those forecasts.
- The later-arriving forecasts have higher substitution assumptions; the earlier-arriving forecast (Fastmarkets) weights supply tightening heavier.

**Defensible reframe:** The rally prices the Fastmarkets-aligned forecast — early deficit, less substitution impact. The Wood Mackenzie and IEA-aligned forecasts, which incorporate more substitution, imply a later and smaller deficit that would not support [[Albemarle|ALB]] at $215. The market is pricing one point in a spread, and that point happens to weight substitution less heavily than the alternative forecasts. That is different from "not pricing substitution at all." The claim is: the rally is priced closer to the low-substitution end of the forecast spread.

---

## Reading path

Start here if you're approaching the vault's lithium coverage for the first time. The sections below walk the topic in the order that builds context without circular lookups.

1. **Commodity fundamentals** — read the Current price, 2026 outlook, and Supply/demand balance sections below. That's the price signal and the supply context that every producer's thesis sits on.
2. **Demand side** — continue through EV demand, Energy storage, and Data center demand sections in this note. This is what's pulling price.
3. **Producers by region** — then walk the actor notes in this order:
   - *Chilean brine (low cost, political):* [[SQM]] → [[Albemarle]] (both operate Salar de Atacama)
   - *Chinese integrated (refining chokepoint):* [[Ganfeng Lithium]] → [[Tianqi Lithium]]
   - *Argentine brine (market-open):* [[Lithium Argentina]] (LAAC, Caucharí-Olaroz)
   - *US domestic (policy-backed):* [[Lithium Americas]] (Thacker Pass Nevada) → [[Standard Lithium]] (DLE Smackover)
   - *Brazilian hard-rock (cost undercut):* [[Sigma Lithium]] → [[Atlas Lithium]] → [[AMG]]
   - *Bolivian state (stalled):* [[YLB]]
4. **Political regimes** — the country-level frame: [[Lithium Triangle]] (Chile / Argentina / Bolivia). Then the specific mechanisms: [[Codelco]]-[[SQM]] Nova Andino Litio JV, [[José Antonio Kast]]'s incoming "concessionable" framework, Milei's RIGI in Argentina (covered in [[Lithium Argentina]]).
5. **Processing and refining** — China ~60% refining dominance is the structural constraint. See Supply chain section below, plus [[AMG]] for Europe's non-China LiOH bet at Bitterfeld.
6. **Technology** — [[Direct lithium extraction]] is the supply-side wild card. Read that concept note for DLE mechanics, Smackover Formation economics, and the 36 active projects.
7. **Today's event** — the "Apr 16, 2026 — lithium producer sigma-dispersion event" section below. The Apr 16 rally ties the threads together: supply discipline from four uncoordinated regimes pricing through simultaneously.
8. **Synthesis** — `/report lithium-3` at `investing/Reports/2026-04-16-lithium-3.md` is the narrative that weaves commodity + producers + political regimes + technology into a single read. Four-paragraph synthesis; disposable; re-run anytime.

Supporting concepts mentioned throughout: [[Critical minerals]] (strategic framing), [[Rare earth leverage]] (parallel chokepoint playbook), [[Lithium-ion shipping fires]] (regulatory/insurance angle).

---

## Current price (Apr 16, 2026)

| Metric | Value |
|--------|-------|
| Price ([[China]]) | **~167,500 CNY/T** |
| USD equivalent | ~$23,000/t |
| Monthly gain | **+6%** (CNY) |
| **YoY change** | **+134%** |
| Late-March level | ~165,000 CNY/T (nearly 1-month high) |
| YTD gain | **+40%+** |

Rebounding from brutal 2023-24 crash (down ~80% from 2022 peak) — Apr 2026 level now sits well above the Jan 2026 ~119,500 CNY/T print and is the highest price since mid-2023.

**Drivers of the Apr 2026 acceleration:**
- Crude oil/product price surge since March — favorable NEV adoption optics
- China EV charging doubling to **180 GW by 2027** — storage + charging pull
- **Zimbabwe suspended exports of lithium concentrates** to stimulate domestic refining — supply-side shock from a marginal producer
- China planning **2026 overcapacity measures** on lithium mining — Beijing's anti-involution campaign (see Supply response section)
- Marginal-cost miners shutdown through 2025 not restarting

---

## 2026 outlook

**Price forecasts:**

| Source | [[Target]] |
|--------|--------|
| Ganfeng Lithium (chairman) | 150,000-200,000 CNY/t (+30-40% demand growth) |
| [[Goldman Sachs]] | $13,250/t avg, rising to $17,077 by 2028 |
| Analyst range | $12,000-17,000/t |
| Bull case | $28,000/t if demand surges |

---

## Supply/demand balance

**Shifting to deficit:**

| Year | Balance |
|------|---------|
| 2023 | Surplus 175,000t |
| 2024 | Surplus 154,000t |
| 2025 | Small surplus |
| **2026** | **Deficit ~1,500t** (Fastmarkets) |

**Supply response:**
- [[China]] canceling mining permits (Yichun: 27 permits)
- [[CATL]] Jiangxi mine suspended Aug 2025 (licence expiry) — 58,500 tonnes/yr, ~4% of global supply (Morgan Stanley)
- [[Zangge Mining]] ordered to halt production Jul 2025
- High-cost mines shutting — nearly half of global production did not cover cash costs at mid-2025 prices (Bernstein)
- Government preventing deflationary pressures — analysts framed mine closures as part of Beijing's "anti-involution" campaign against excessive price competition (Kenny Ng, Everbright Securities International)

**Inventory (mid-2025):** China lithium carbonate stockpile rose from 115,000 tonnes (Jan 2025) to 150,000 tonnes (May 2025) — Bernstein. Already-planned capacity additions expected to outstrip demand growth in 2025 despite mine closures.

![[ft-lithium-carbonate-price-2020-2025.png]]
*China lithium carbonate price per tonne (Rmb '000s), 2020-2025. Source: FT, Aug 2025*

## Direct lithium extraction — supply revolution?

[[Direct lithium extraction]] (DLE) could reshape the supply side. The technology separates lithium from underground brines in hours (vs 18 months for evaporation ponds), with 70-90% recovery rates vs 40-60%, using less land and water. The US Smackover Formation (Arkansas and neighboring states) holds up to 19M tonnes of lithium resource (USGS, Oct 2024).

Key DLE players: [[Standard Lithium]]/[[Equinor]] ($1.5B project, Arkansas), [[Exxon|ExxonMobil]]/Saltwerx (target: lithium for 1M+ EVs by 2030), [[Rio Tinto]] ($2.5B Rincon expansion, Argentina). At least 36 DLE projects worldwide, 13 in [[China]] (Columbia University). All-in costs currently ~$10,000/t — most need $18-20K/t prices to proceed, but high-concentration Smackover brines may undercut that.

US producers accuse [[China]] of deliberately crashing prices (~$9,000/t, down 80% from 2022) to frustrate new entrants. Beijing has also restricted export of lithium processing technologies.

## Demand trajectory

Global demand tripled 2020-2024 to ~1.2M tonnes (Wood Mackenzie). Forecast: 5.8M tonnes by 2050.

| Year | Demand ('000t LCE) | Source |
|------|-------------------|--------|
| 2020 | ~400 | Statista |
| 2024 | ~1,200 | Wood Mackenzie |
| 2025 | ~1,250 | Statista |
| 2030 | ~2,500 | Statista |
| 2050 | ~5,800 | Wood Mackenzie |

---

## EV demand drivers

**Global EV market:**

| Metric | Value |
|--------|-------|
| 2025 EV sales | 20M+ units (~25% of new cars) |
| 2025 growth | +27% |
| 2026 forecast | 25M+ units |
| 2030 forecast | 50M+ units |

**Battery costs:**
- 2026 pack price: ~$105/kWh (down 3%)
- LFP (lithium iron phosphate) gaining share
- [[China]] manufacturing glut pressuring prices

---

## Energy storage — fastest growing

**[[Grid storage]] emerging:**

| Metric | Value |
|--------|-------|
| 2025 storage growth | +44% (vs +25% total battery) |
| Storage share of demand | ~25% of global battery demand |
| [[China]] charging capacity | Doubling to 180 GW by 2027 |

Energy storage becoming as important as EVs for lithium demand.

## Data center demand and geopolitical vulnerability

Data centers rely almost entirely on lithium-ion battery systems for uninterruptible power supply (UPS), bridging outages until backup generators start. In 2024, the US imported $15.3B of lithium batteries from [[China]] — roughly a quarter of China's total lithium battery exports (FT, May 2025).

This creates a supply-chain chokepoint: [[China]] controls 60%+ of global lithium refining, and Beijing has already demonstrated willingness to restrict critical battery technologies and processing for [[Lithium|lithium]] and [[Gallium|gallium]]. The 2023 [[Germanium|germanium]]/gallium export controls provide precedent for using critical minerals as geopolitical leverage.

Alternatives emerging for data center storage:
- Organic flow batteries — non-toxic, non-flammable organic molecules in saltwater; no lithium, cobalt, or nickel. Prometheus Hyperscale (US data center developer) signed a deal for deployment at scale (May 2025)
- [[Sodium-ion batteries]] — avoid lithium entirely; [[CATL]] scaling by end of 2026

For [[Tianqi Lithium]] and [[Ganfeng Lithium]], any shift in US data center demand away from Chinese-refined lithium batteries represents a risk to one of their key export markets.

![[statista-lithium-demand-forecast-2020-2030.png]]
*Lithium demand forecast ('000 tonnes LCE), 2020-2030. Battery-grade (EV) drives growth from ~350 to ~2,500. Source: Statista via FT, May 2025*

---

## Investment thesis

**Bull case:**
- Deficit starting 2026
- EV adoption accelerating (25M+ units 2026)
- Energy storage +44% growth
- Supply discipline ([[China]] permit cancellations)
- Rebounding from oversold levels

**Bear case:**
- [[China]] controls processing/refining
- LFP uses less lithium than NMC
- [[Battery recycling]] increasing
- Oversupply risk if mines restart
- Price still well below 2022 highs

---

## Investment vehicles

**ETFs:**

| Ticker | Name | Focus |
|--------|------|-------|
| LIT | Global X Lithium & Battery Tech | Broad exposure |
| BATT | Amplify Lithium & Battery Tech | Battery value chain |

**Miners:**

| Company | Ticker | Notes |
|---------|--------|-------|
| [[Albemarle]] | ALB | \#1 Western producer, +74% in 2025 |
| [[SQM]] | SQM | Chilean brine, Atacama |
| Livent | LTHM | Pure-play lithium |
| [[Standard Lithium]] | SLI (TSX) | US DLE, Smackover (Arkansas) |
| Piedmont Lithium | PLL | US development |
| Pilbara Minerals | PLS (ASX) | Australian spodumene |
| [[Ganfeng Lithium]] | 1772 (HK) | Chinese integrated |

---

## Supply chain

**Geographic concentration:**

| Stage | Dominant |
|-------|----------|
| Mining | [[Australia]], [[Chile]], [[China]] |
| Refining | **[[China]] (~60%+)** |
| Battery production | **[[China]] (~75%)** |

[[China]] dominates midstream/downstream — geopolitical risk.

---

## Apr 16, 2026 — lithium producer sigma-dispersion event

On Apr 16, 2026, the two largest Western lithium producers both posted sigma-event moves on the same session:

| Ticker | Actor | Prev close | Apr 16 close | Move | Sigma (60d vol, beta-adj) |
|--------|-------|------------|--------------|------|---------------------------|
| ALB | [[Albemarle]] | $185.38 | **$215.62** | **+16.3%** | **+4.9σ** |
| SQM | [[SQM]] | $87.62 | **$95.31** | **+8.8%** | **+2.6σ** |

**Durable catalysts stacked into the print:**

1. **Lithium carbonate at 167,500 CNY/T (+134% YoY)** — highest since mid-2023; Apr 2026 extension of the rally that began late-2024.
2. **Analyst ratchet** — UBS PT $230 on ALB (Apr 9); BofA and Berenberg had raised SQM earnings estimates in late March 2026.
3. **Supply-side shock** — Zimbabwe export suspension on lithium concentrates.
4. **Policy support** — China's 2026 overcapacity measures (anti-involution); EV charging 180 GW build by 2027.
5. **Setup pre-earnings** — ALB early May 2026 print is the proof-point; stock was expecting positive EPS revisions.

**Read on the dispersion:** The +16% ALB vs +8.8% SQM split reflects leverage-to-price differences. ALB is higher-cost with more lithium-mix concentration (EV / grid storage pure-play); SQM has diversified plant nutrition / iodine revenue that dampens the single-commodity leverage. A 20%+ rally in spot lithium prices repriced ALB disproportionately. Statistically unusual moves in correlated sector names on a common catalyst should be grouped and read as a sector event — consistent with quick_movers.py discipline.

See [[Albemarle]] and [[SQM]] actor notes for stock-specific updates; underlying commodity driver is captured above in the Current price section.

*Sources: Reuters lithium price coverage (Apr 16), [QuiverQuant: Albemarle jumps as lithium prices extend 2026 rebound](https://www.quiverquant.com/news/Albemarle+jumps+as+lithium+prices+extend+2026+rebound+and+bullish+analyst+actions+build), trading economics lithium data, [Simply Wall St on SQM](https://simplywall.st/stocks/us/materials/nyse-sqm/sociedad-quimica-y-minera-de-chile/news/did-upgraded-lithium-earnings-forecasts-just-shift-sqms-sqm).*

---

*Created 2026-01-05 — last expanded 2026-04-16*

---

## Related

- [[Albemarle]] — \#1 Western producer
- [[SQM]] — Chilean producer (\#2 global)
- [[Rio Tinto]] — diversifying into lithium (Arcadium acquisition)
- [[BYD]] — major EV/battery producer
- [[Tesla]] — EV demand driver
- [[CATL]] — \#1 battery maker
- [[NIO]] — battery swap EV
- [[Rivian]] — US EV demand
- [[Rare earth leverage]] — [[China]] supply chain control
- [[Clean energy for AI]] — storage applications
- [[Direct lithium extraction]] — emerging supply technology (DLE)
- [[Standard Lithium]] — US DLE pioneer (Smackover, Arkansas)
- [[Ganfeng Lithium]] — Chinese integrated producer
- [[Tianqi Lithium]] — Chinese producer/refiner
- [[Zangge Mining]] — Chinese miner (production halted 2025)
- [[Liontown Resources]] — Australian developer
- [[Sigma Lithium]] — Brazilian producer (Grota do Cirilo, 270→520 ktpa)
- [[Atlas Lithium]] — Brazilian developer (Neves, 145% IRR, Japan-US partnership)
- [[AMG]] — Brazilian concentrate + German LiOH (Mibra-Bitterfeld integrated)
- [[Lithium Americas]] — US-domestic (Thacker Pass Nevada, GM JV, DOE $2.23B loan)
- [[Lithium Argentina]] — Argentine brine (Caucharí-Olaroz, Ganfeng JV, sub-$6,000/t)
- [[YLB]] — Bolivian state enterprise (21% global reserves, stalled production)
- [[Lithium Triangle]] — geographic-political frame (Chile/Argentina/Bolivia)
- [[Lithium-ion shipping fires]] — regulatory / insurance risk on battery transport
- [[Sodium-ion batteries]] — demand substitution (grid storage + sub-$15,000 EVs)
- [[Battery recycling]] — secondary supply (~20% of demand by 2030)
- [[Solid-state batteries]] — potential lithium-demand intensification (Li-metal anodes, Li-sulfide electrolytes)
- [[Bolivia]] · [[Rodrigo Paz]] — largest reserves, political transition Nov 2025

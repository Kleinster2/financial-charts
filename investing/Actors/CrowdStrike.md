---
aliases: [CRWD]
---
#actor #cybersecurity #usa #public

**CrowdStrike** — Endpoint security leader. Falcon platform. July 2024 outage caused global IT crisis.

---

## Why CrowdStrike matters

| Metric | Value |
|--------|-------|
| Ticker | CRWD (NASDAQ) |
| Market cap | ~$90B |
| Position | #1 endpoint security |
| Key product | Falcon platform |

---

## July 2024 outage

**Global IT crisis:**
- Faulty content update crashed 8.5M Windows devices
- Airlines, banks, hospitals affected
- $5.4B+ estimated damages
- Stock dropped ~30%, recovered partially

**Aftermath:**
- CEO George Kurtz testified to Congress
- Enhanced testing procedures
- Customer trust damaged but retention high

---

## Business model

| Segment | Description |
|---------|-------------|
| Falcon platform | Cloud-native endpoint protection |
| Threat intelligence | CrowdStrike Intelligence |
| IR services | Incident response |
| Identity protection | Expanding TAM |

**Land and expand:** Average customer uses 5+ modules.

---

## Competitive position

| Competitor | Focus |
|------------|-------|
| **CrowdStrike** | Endpoint leader |
| [[Palo Alto Networks]] | Platform consolidation |
| [[SentinelOne]] | #2 endpoint |
| Microsoft Defender | Bundled with Windows |

---

## China cybersecurity ban (Jan 2026)

**Banned by Beijing** — China instructed companies to stop using CrowdStrike products and replace with domestic alternatives by H1 2026.

**CrowdStrike response:** "We do not sell into China and do not have offices, hire people or host infrastructure there, and thus could only be negligibly affected."

**Rationale:** Beijing fears Western cybersecurity software could collect and transmit confidential information abroad.

See [[China cybersecurity ban]] for full context.

---

## Investment case

**Bull:**
- Endpoint market leader
- Platform expansion (identity, cloud)
- High retention despite outage
- Cybersecurity secular growth
- No China revenue exposure

**Bear:**
- July 2024 trust damage
- Microsoft bundling threat
- Palo Alto platformization
- Premium valuation

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | CRWD |
| Market cap | ~$90B |
| Founded | 2011 |
| CEO | George Kurtz |

*Updated 2026-01-15*

---

## Related

- [[Palo Alto Networks]] — competitor (platform consolidation)
- [[SentinelOne]] — competitor (#2 endpoint)
- [[Cybersecurity consolidation]] — thesis context
- [[Zscaler]] — peer (zero trust)
- [[China cybersecurity ban]] — banned in China (Jan 2026)
- [[China]] — policy driver

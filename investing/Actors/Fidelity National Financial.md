---
aliases: [FNF, [[Fidelity]] National Title, Chicago Title]
---
#actor #finance #insurance #realestate #usa #public

**[[Fidelity]] National Financial** — Largest US title insurer with 32% market share. Brands include [[Fidelity]] National Title, Chicago Title, Commonwealth Land Title. Record margins through technology. NYSE: FNF.

---

## Why it matters

| Metric | Value |
|--------|-------|
| Market share | **32%** (#1 in US) |
| Ticker | FNF |
| Fortune 500 rank | #313 (highest in title industry) |
| State rankings | #1 or #2 in 39 states |

---

## Brands

| Brand | Type |
|-------|------|
| [[Fidelity]] National Title | Title insurance |
| Chicago Title | Title insurance |
| Commonwealth Land Title | Title insurance |
| Alamo Title | Title insurance |
| National Title of New York | Title insurance |

**Collectively issue more title policies than any other company in the US.**

---

## Q3 2025 performance

| Metric | Value | YoY |
|--------|-------|-----|
| Total revenue | $2.0B | +41% |
| Title segment revenue | $1.8B | +42% |
| Net earnings | $358M | +35% |
| EPS | $1.33 | vs $0.97 |
| Pretax margin | **17.8%** | Record |
| Commercial revenue | — | **+34%** (record) |

**Record title insurance margins despite low transaction environment.**

---

## Competitive moat

| Barrier | Detail |
|--------|--------|
| **State licensing** | Required in each state |
| **Relationships** | Deep ties to agents, lenders |
| **Data repositories** | Massive title/property data |
| **Scale** | Distribution density unmatched |
| **Technology** | SoftPro, inHere, AI investments |

**Four major players:** FNF (32%), [[First American Financial]], Old [[Republic]], Stewart.

---

## Technology transformation

**Decade of investment:**
- SoftPro (title production software)
- inHere (digital closing platform)
- AI integration

**Result:** Record margins (17.8%) even in weak transaction environment.

---

## Housing market exposure

| Transaction type | Sensitivity |
|------------------|-------------|
| **Residential purchase** | Volume-driven |
| **Refinance** | Rate-driven |
| **Commercial** | +34% Q3 2025 |

**Current environment:** Low residential volume offset by strong commercial.

---

## Investment implications

**Bull case:**
- #1 market share, wide moat
- Technology driving margin expansion
- Commercial strength offsetting residential weakness
- Housing recovery = volume upside

**Bear case:**
- Housing transaction volume depressed
- Rate sensitivity (refi volume)
- Cyclical business
- Digital disruption risk (long-term)

---

## Related

- [[Title Insurance]] — sector
- [[First American Financial]] — #2 title insurer
- [[Housing]] — sector context
- [[Mortgage rates]] — refi volume driver
- [[Rocket Companies]] — customer (title services)
- [[UWM]] — customer (title services)

*Created 2026-01-16*

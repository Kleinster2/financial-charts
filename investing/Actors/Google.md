...
| Region | Revenue | Share | YoY |
|--------|---------|-------|-----|
| United States | 48.8 | 48% | +13% |
| EMEA | 29.9 | 29% | +17% |
| APAC | 17.8 | 17% | +23% |
| Other Americas | 6.1 | 6% | +19% |
...
DeepMind's robotics team, led by Carolina Parada, is one of the labs behind the 2024 robot-learning inflection (FT visual story, Nov 20 2024): "There's an excitement in the air" — the field advancing faster than its own researchers expected. Joint work with [[Stanford]] ([[Chelsea Finn]]) trained diffusion policies on the most dexterous tasks the teams could devise, all successfully — see [[Diffusion models (robotics)]].

Two details mark the foundation-model shift: a robot dog taught to open doors with its paw retained the skill inside the foundation model permanently — trained once, like a real pet; and Parada's generalisation test for household robotics is whether the coat-hanger task extends to different clothing and settings ("a robot that really could do your entire laundry"). The harder unsolved problem is proactive human interaction — reading non-verbal intent signals, "read the room" being extremely hard for a robot. Her timeline shift: what was once "not feasible in this lifetime" is now, for most in the field, "before I retire."

The robotics line continued into [[Gemini]] Robotics and the Jan 2026 [[Boston Dynamics]] Atlas partnership. See [[Physical AI]] for the field structure.
...
### GridVista launch (Feb 17, 2026)

Partnered with CTC Global to launch GridVista — an AI-powered grid monitoring system that embeds fiber-optic sensors directly inside power line conductor cores (CTC's ACCC Conductor).

| Feature | Detail |
|---------|--------|
| Technology | Fiber-optic sensors embedded in conductor core |
| Data processing | Google Earth Engine, BigQuery, Tapestry (Alphabet X moonshot) |
| Capacity gain | Up to 120% of previous capacity on existing lines |
| Advantage over legacy | Eliminates blind spots — continuous sensing vs discrete sensor points |
| Status | Pilot stage; expanding utility partners in 2026 |

GridVista addresses the grid bottleneck Google itself is creating: AI data center demand outstripping interconnection capacity. The system turns existing transmission lines into smart infrastructure without building new ones — a near-term solution while nuclear (Kairos) and co-location strategies develop.

Tapestry (Alphabet X) is building what it claims is the first unified model of the US grid. Partnered with PJM Interconnection (largest US grid operator) in 2025 for faster grid connection timelines.
...
- [[CTC Global]] — GridVista power line sensing partner
- [[Tapestry (Alphabet X)]] — grid modeling moonshot

---
aliases: []
---
#actor #energy #nuclear #usa #public

**Oklo** — Sam Altman-backed SMR company. [[Aurora]] microreactor. OpenAI connection.

---

## Why Oklo matters

Sam Altman's nuclear bet:

| Metric | Value |
|--------|-------|
| Ticker | OKLO |
| Market cap | ~$2B |
| Chairman | Sam Altman |
| Status | Pre-commercial |
| Reactor | [[Aurora]] |

---

## Sam Altman connection

**AI + nuclear:**
- Altman is Chairman
- Invested pre-OpenAI
- AI power demand thesis
- OpenAI data center angle?

Altman sees nuclear as AI enabler.

---

## [[Aurora]] microreactor

| Spec | Value |
|------|-------|
| Power | 15-50 MW |
| Fuel | Recycled nuclear fuel |
| Type | Fast reactor |
| Timeline | Late 2020s |

Smaller than NuScale's design.

---

## Fuel recycling angle

**Unique approach:**
- Uses spent nuclear fuel
- Solves waste problem
- DOE fuel access
- Differentiated technology

---

## NRC application

**Regulatory status:**
- Initial rejection (2022)
- Resubmitting application
- Longer path than NuScale
- Regulatory uncertainty

---

## Business model

**Power-as-a-service:**
- Build, own, operate reactors
- Sell power via PPA
- Recurring revenue model
- Customer: data centers, remote sites

---

## Customers

| [[Target]] | Use Case |
|--------|----------|
| Data centers | AI power demand |
| Remote sites | Mining, military |
| Industrial | Process heat |

---

## Investment case

**Bull:**
- Sam Altman backing
- Fuel recycling angle
- AI power narrative
- Small reactors = flexible

**Bear:**
- Pre-revenue
- NRC rejection history
- Long timeline
- [[NuScale]] ahead on approval
- High valuation for stage

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | OKLO |
| Market cap | ~$2B |
| Chairman | Sam Altman |
| Status | Pre-commercial |

*Updated 2026-01-01*

---

## Related

- [[OpenAI]] — connection (Sam Altman chairs both, AI power demand)
- [[Sam Altman]] — chairman (invested pre-OpenAI)
- [[NuScale]] — competitor (SMR peer, further along on NRC)
- [[Constellation Energy]] — peer (nuclear power)
- [[Nuclear renaissance]] — thesis context (AI power demand)


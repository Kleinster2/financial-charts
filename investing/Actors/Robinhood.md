---
aliases: [HOOD]
---
#actor #fintech #brokerage #usa #public

Robinhood — Retail brokerage that democratized trading. Now pushing into crypto and tokenized private shares. First to offer OpenAI/[[SpaceX]] tokens to retail.

---

## Sector correlation

| Sector | ETF | Correlation |
|--------|-----|-------------|
| Technology | XLK | 0.78 |
| Software | IGV | 0.73 |
| [[Semiconductors]] | SMH | 0.72 |
| *S&P 500* | *SPY* | *0.75* |

HOOD trades as a core Technology name (XLK r = 0.78).

---

## Why Robinhood matters

Robinhood is the gateway for retail into new asset classes:

| Innovation | Impact |
|------------|--------|
| Commission-free trading | Forced industry to follow |
| Mobile-first UX | Made trading accessible |
| Fractional shares | Lowered barriers |
| Crypto trading | Mainstream adoption |
| Tokenized private shares | OpenAI/[[SpaceX]] exposure |

---

## Tokenized private shares (June 2025)

Most ambitious crypto push yet:
- Launched tokenized OpenAI, [[SpaceX]] shares in [[Europe]]
- Retail can now get "exposure" to private companies
- Not actual equity — derivative instruments
- OpenAI publicly did not endorse the move

Why it matters:
- First major brokerage to offer private company tokens
- Tests regulatory boundaries
- Could open floodgates if successful

See [[Tokenized private shares]] for full analysis.

---

## Business segments

| Segment | Description |
|---------|-------------|
| Equities | Commission-free stock trading |
| Options | Retail options trading |
| Crypto | Bitcoin, Ethereum, altcoins |
| Cash management | Debit card, interest on cash |
| [[Gold]] subscription | Premium features ($5/month) |

---

## Revenue model

Payment for order flow (PFOF):
- Routes orders to market makers
- Market makers pay for flow
- Controversial — conflicts of interest
- Under regulatory scrutiny

Other revenue:
- Net interest income
- [[Gold]] subscriptions
- Crypto spreads

---

## Crypto strategy

| Move | Status |
|------|--------|
| Basic crypto trading | Live |
| Tokenized private shares | [[Europe]] (June 2025) |
| Wallet features | Expanding |
| Staking | Some markets |

Positioning: Crypto as infrastructure layer for new asset classes.

---

## Controversies

| Issue | Context |
|-------|---------|
| GameStop halt (2021) | Restricted trading during meme stock squeeze |
| PFOF scrutiny | Conflicts with "free" trading |
| Gamification | Criticized for encouraging risky behavior |
| Crypto volatility | Users lost money on speculative coins |

---

## Investment case

Bull:
- First-mover on tokenized private shares
- Young user base (future wealth)
- Crypto infrastructure play
- Regulatory tailwinds (SEC chair Atkins pro-innovation)
- International expansion

Bear:
- PFOF under regulatory threat
- Meme stock hangover
- Competition from Schwab, [[Fidelity]]
- Crypto volatility affects revenue
- Tokenization regulatory risk

---

## Meme stock legacy (5-year retrospective, Jan 2026)

90% of the 2.5M accounts added in January 2021 are still on the platform (Robinhood disclosure, Jan 2026).

Those users have matured: many now use retirement accounts, high-yield savings, and less speculative products. "They were 20-something-year-olds. Now, maybe they're in their 30s" — Steve Quirk, Chief Brokerage Officer.

Robinhood is in an arms race with [[Interactive Brokers]] and [[Charles Schwab]] to serve an increasingly sophisticated retail base.

---

## Q4 & FY 2025 results (reported Feb 10, 2026)

Record year. Stock fell ~7% after hours on crypto revenue miss.

| Metric | Q4 2025 | YoY | FY 2025 | YoY |
|--------|---------|-----|---------|-----|
| Revenue | $1.28B | +27% | $4.5B | +52% |
| Net income | $605M | — | $1.9B | — |
| Diluted EPS | $0.66 | Beat $0.61 | $2.05 | — |
| Adj EBITDA | $761M | +24% | $2.5B | +76% |
| ARPU | $191 | +16% | — | — |

Q4 revenue breakdown: Options $314M (+41%), crypto $221M (-38%), equities $94M (+54%), event contracts $147M (+300%), net interest $411M (+39%).

Operating metrics: 27.0M funded customers (+7%), platform assets $324B (+68%), Gold subs 4.2M (+58%), net deposits $68.1B (35% organic growth), retirement AUC $26.5B (+102%), margin book $16.8B (+113%).

[[Prediction markets]] exceeded 8.5B contracts traded — on track for billion-dollar annual business. Short selling launched mid-November with billions in notional volume. UK [[stocks and shares ISA]] launched. [[Bitstamp]] acquisition expanding global crypto footprint.

New CFO Shiv Verma replaced Jason Warnick (Feb 6, 2026).

2026 outlook: adj opex + SBC $2.6-2.725B (+18% midpoint).

Analyst reaction: [[Barclays]] $124 PT (from $159), [[Goldman Sachs]] $130 (from $152). Consensus bullish — crypto weakness seen as transitory.

---

## Financials

| Metric | Value |
|--------|-------|
| Ticker | HOOD (NASDAQ) |
| Market cap | ~$50B |
| Revenue (FY 2025) | $4.5B |
| Users | 27.0M funded accounts (Q4 2025) |
| AUC | $324B |
| Gold subscribers | 4.2M |
| Founded | 2013 |
| IPO | 2021 |

---

## Competitive landscape

| Competitor | Strength |
|------------|----------|
| Schwab/TD | Scale, advice, bank |
| [[Fidelity]] | Retirement, research |
| Interactive Brokers | Sophistication, global |
| Webull | Similar retail focus |
| [[Republic]] | [[Private markets]] focus |

Robinhood = retail-first, mobile-first, crypto-forward.

---

## Related

- [[GameStop]] — meme stock crisis (halted buying Jan 28, 2021)
- [[Retail investor flows]] — structural shift in market participation
- [[Citadel Securities]] — largest PFOF partner
- [[Interactive Brokers]] — competitor (sophisticated segment)
- [[Tokenized private shares]] — key innovation (OpenAI/[[SpaceX]] tokens)
- [[OpenAI]] — tokenized on Robinhood [[Europe]]
- [[Republic]] — peer (private markets platform)
- [[Crypto]] — infrastructure layer
- [[Bitstamp]] — acquisition (global crypto exchange)
- [[Prediction markets]] — event contracts platform
- [[Barclays]] — analyst coverage ($124 PT)
- [[Goldman Sachs]] — analyst coverage ($130 PT)


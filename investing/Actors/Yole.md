---
aliases: [Yole Développement, Yole Group, Yole Intelligence]
---
#actor #research #non-investable

**Yole** — French semiconductor research firm. Specializes in advanced packaging, MEMS, power electronics.

---

## Why Yole matters for this vault

Deep expertise in semiconductor manufacturing trends:

| Coverage | Why it matters |
|----------|----------------|
| **Advanced packaging** | CoWoS, 2.5D/3D, chiplets |
| **MEMS/sensors** | Mobile, automotive |
| **Power electronics** | EVs, infrastructure |
| **Compound semis** | SiC, GaN |

---

## Key coverage areas

| Area | Relevance to vault |
|------|-------------------|
| **CoWoS/packaging** | [[TSMC]], [[SK Hynix]] capacity constraints |
| **[[HBM]]** | Memory stacking technology |
| **Chiplets** | [[AMD]], [[Intel]] architectures |
| **SiC/GaN** | EV power electronics |

---

## How to use in vault

Yole data useful for:
- Advanced packaging capacity/demand
- Understanding [[Advanced packaging]] bottlenecks
- Chiplet adoption trends
- Power semiconductor market sizing

**Strength**: Technical depth on manufacturing processes.

---

## Peer research firms

| Firm | Overlap |
|------|---------|
| **Yole** | Packaging, MEMS (deepest) |
| [[TrendForce]] | Memory pricing |
| [[Omdia]] | [[Semiconductors]] (broader) |
| VLSI Research | WFE equipment |
| SemiAnalysis | Technical deep dives |

---

## Quick stats

| Metric | Value |
|--------|-------|
| Type | Market research firm |
| HQ | Lyon, [[France]] |
| Focus | Semiconductor technology |
| Coverage | Packaging, MEMS, power, compound |
| Investable | No |

*Updated 2026-01-15*

---

## Related

- [[Advanced packaging]] — Yole tracks CoWoS, 2.5D/3D
- [[TSMC]] — packaging capacity data
- [[SK Hynix]] — [[HBM]] packaging analysis
- [[Foundry Wars]] — manufacturing trend context

---
aliases: []
---
#actor #data #finance #uk #acquired

**Preqin** — [[Private markets]] data and analytics provider. Acquired by BlackRock (2024). Industry-standard for alternatives data.

---

## Why Preqin matters

| Metric | Value |
|--------|-------|
| Founded | 2003 |
| HQ | London |
| Focus | [[Private markets]] data |
| Acquirer | [[BlackRock]] (2024) |

---

## Acquisition by BlackRock

| Detail | Value |
|--------|-------|
| Year | 2024 |
| Deal value | Part of ~$28B spree |
| Rationale | Data infrastructure for private markets |
| Status | Completed |

Complements BlackRock's Aladdin platform for alternatives.

---

## Product offering

| Product | Description |
|---------|-------------|
| Database | PE, VC, hedge fund, real estate, infrastructure |
| Analytics | Performance benchmarking |
| Investor profiles | LP intelligence |
| Fund manager data | GP tracking |

Industry-standard data source for alternatives.

---

## Market position

**Competitive landscape:**
- PitchBook ([[Morningstar]]) — competitor
- Burgiss — competitor
- Preqin = leading independent

Acquisition removes key competitor from market.

---

## Strategic fit

**For BlackRock:**
- Data layer for private markets push
- Complements Aladdin platform
- Intelligence on LP/GP universe
- Supports product development

Data = moat in alternatives.

---

## Quick stats

| Metric | Value |
|--------|-------|
| Founded | 2003 |
| HQ | London |
| Focus | [[Private markets]] data |
| Status | Acquired by BlackRock |

*Created 2026-01-15*

---

## Related

- [[BlackRock]] — acquirer
- [[Larry Fink]] — BlackRock CEO
- [[Aladdin]] — BlackRock's risk platform
- [[PitchBook]] — competitor ([[Morningstar]]-owned)

#actor #space #usa #private

**Axiom Space** — Building ISS replacement. Also developing space data centers.

---

## Why Axiom matters

| Business | Status |
|----------|--------|
| **Axiom Station** | Commercial space station (ISS successor) |
| **ISS modules** | Attaching to ISS, then detaching |
| **Space data centers** | In development |
| **Spacesuits** | NASA contract for Artemis |

---

## Axiom Station

**Commercial ISS replacement:**

| Phase | Description |
|-------|-------------|
| 1 | Attach modules to ISS |
| 2 | Detach as free-flying station |
| 3 | Expand commercial operations |

NASA planning to retire ISS by ~2030. Axiom positioned as successor.

---

## Space data centers

**Dual business model:**
- Space station for research/tourism
- Orbital compute infrastructure

Combines human spaceflight expertise with data center ambitions.

---

## Funding

| Round | Amount |
|-------|--------|
| Series C (2024) | $350M |
| Total raised | $500M+ |
| Valuation | ~$3B+ |

Investors include Google Ventures, Airbus Ventures.

---

## NASA relationships

| Contract | Value |
|----------|-------|
| ISS module attachment | Access rights |
| Artemis spacesuits | ~$230M |
| Private astronaut missions | Multiple flown |

Strong government relationships enable commercial expansion.

---

## Investment case

**Not investable** — private.

**Watch for:**
- IPO potential (space station nearing completion)
- DC revenue stream
- NASA ISS transition

---

## Quick stats

| Metric | Value |
|--------|-------|
| Status | Private |
| Founded | 2016 |
| HQ | Houston, Texas |
| CEO | Michael Suffredini (ex-NASA ISS program manager) |
| Valuation | ~$3B+ |

*Updated 2026-01-15*

---

## Related

- [[Space data centers]] — developing orbital compute
- [[SpaceX]] — launch provider
- [[NASA]] — key customer

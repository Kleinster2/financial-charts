---
aliases: [Neko]
---
#actor #company #sweden #healthcare #ai #private

**Neko Health** — Swedish AI-powered preventive healthcare company. Co-founded 2018 by [[Daniel Ek]] ([[Spotify]]) and Hjalmar Nilsonne. Full-body scans using 70+ sensors, 50M data points, AI analysis. **$1.8B valuation** (Jan 2025), $325M total raised. £299/scan in [[UK]]. 100K waitlist, 80% rebooking rate.

---

## Why Neko Health matters

| Metric | Value |
|--------|-------|
| Valuation | **$1.8B** (Jan 2025) |
| Total raised | **$325M** |
| Latest round | $260M Series B (Jan 2025) |
| Founded | 2018 |
| HQ | Stockholm, Sweden |
| Scans performed | 10,000+ |
| Waitlist | 100,000 |

---

## The product

### Full-body scan

| Metric | Detail |
|--------|--------|
| Price | £299 ([[UK]]) |
| Duration | ~15 minutes |
| Data points | **50 million** per scan |
| [[Sensors]] | 70+ (thermal cameras, 3D cameras, ECG, lasers, finger sensors) |

### What it detects

| Condition | Method |
|-----------|--------|
| Heart disease | ECG, vascular analysis |
| Skin cancer | Thermal + 3D imaging |
| Type 2 diabetes | Metabolic markers |
| Other conditions | AI pattern recognition |

### Business model

- [[Consumer]]-pay preventive health
- Annual subscription model (80% rebooking rate)
- Expanding clinic footprint

---

## Funding history

| Date     | Round        | Amount    | Valuation | Lead                                              |
| -------- | ------------ | --------- | --------- | ------------------------------------------------- |
| 2018     | Seed         | €30M      | —         | [[Daniel Ek]] (self-funded via [[Prima Materia]]) |
| 2023     | Series A     | $65M      | —         | Lakestar, Atomico, [[General Catalyst]]           |
| Jan 2025 | **Series B** | **$260M** | **$1.8B** | [[Lightspeed Venture Partners]]                   |

**Total: $325M**

### Investors

| Investor | Rounds |
|----------|--------|
| [[Prima Materia]] | Seed, A, B |
| Lightspeed Venture Partners | B (lead) |
| General Catalyst | A, B |
| Atomico | A, B |
| Lakestar | A, B |
| O.G. Venture Partners | B |

---

## Founders

| Person | Role | Background |
|--------|------|------------|
| **[[Daniel Ek]]** | Co-founder, Chairman | [[Spotify]] founder |
| **Hjalmar Nilsonne** | Co-founder, CEO | [[Healthcare]]/tech background |

---

## Expansion

| Market | Status |
|--------|--------|
| Sweden | Operating (Stockholm) |
| [[UK]] | Operating (London, largest market) |
| US | **Pending FDA approval** |
| [[Europe]] | Expanding |

---

## Competitive landscape

| Competitor | Approach |
|------------|----------|
| Traditional checkups | Slow, expensive, reactive |
| Executive health programs | $5K+, hospital-based |
| Prenuvo | MRI-based full-body scans (~$2,500) |
| **Neko** | Fast, affordable, AI-first |

---

## Related

- [[Daniel Ek]] — co-founder
- [[Prima Materia]] — primary investor (Ek's vehicle)
- [[Spotify]] — Ek's other company

---

*Created 2026-01-25*

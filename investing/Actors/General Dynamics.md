---
aliases: [GD]
---
#actor #defense #aerospace #usa #public

**General Dynamics** — [[Defense]] prime. Submarines, tanks, Gulfstream jets. Less AI-focused than peers.

---

## Sector correlation

| Sector | ETF | Correlation |
|--------|-----|-------------|
| [[Defense]] | ITA | 0.76 |
| Industrials | [[XLI]] | 0.71 |
| Financials | [[XLF]] | 0.61 |
| *[[S&P 500]]* | *[[SPY]]* | *0.60* |

GD trades as a core Defense name (ITA r = 0.76).

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | GD ([[NYSE]]) |
| Exchange | [[NYSE]] |
| Market cap | ~$80B |
| 2025 revenue | $52.5B |
| Core exposure | Defense backlog, submarines, Gulfstream business jets |

---

## Why General Dynamics matters

| Metric | Value |
|--------|-------|
| Market cap | ~$80B |
| Ticker | GD ([[NYSE]]) |
| Focus | Naval, ground vehicles, business jets |
| Backlog | $90B+ |

---

## Charts

![[general-dynamics-price-chart.png]]
*GD trades as a defense-backlog and execution instrument, with Marine Systems now the key swing factor.*

![[general-dynamics-fundamentals-chart.png]]
*Revenue has stepped higher with defense demand and Gulfstream deliveries; net income follows execution and mix.*

![[general-dynamics-sankey.png]]
*The Sankey view is useful for separating program revenue scale from the lower-margin shipbuilding execution burden.*

---

## Financials

### Annual

| Year | Revenue | Net income |
|------|---------|------------|
| 2021 | $38.5B | $3.3B |
| 2022 | $39.4B | $3.4B |
| 2023 | $42.3B | $3.3B |
| 2024 | $47.7B | $3.8B |
| 2025 | $52.5B | $4.2B |

*Source: local fundamentals data in `market_data.db`.*

---

## Leadership

| Role | Person | Notes |
|------|--------|-------|
| CEO | Phebe Novakovic | Long-tenured operator; execution and capital discipline define the GD thesis |
| Controller | Chief accounting officer | Signed the Apr 2026 earnings 8-K |

---

## Business segments

| Business line | Products |
|---------|----------|
| Marine Systems | Submarines (Virginia, Columbia class) |
| Combat Systems | Tanks (Abrams), armored vehicles |
| [[Aerospace]] | Gulfstream business jets |
| Technologies | [[GDIT]] ([[IT services]]), Mission Systems |

See [[GDIT]] for detailed coverage of the [[IT services]] business (~$10B+ revenue, 30K employees).

---

## Key programs

| Program | Description |
|---------|-------------|
| Columbia-class | Nuclear ballistic missile submarines |
| Virginia-class | Attack submarines |
| Abrams | Main battle tank upgrades |

---

## Trump defense policy (Jan 2026)

### Budget ($1.5T) — Jan 7

Stock rallied with peers on $1.5T budget announcement. Submarine programs are "must fund."

### Executive order — Jan 8

Stock fell -4.2%, giving back gains.

| Restriction | Impact |
|-------------|--------|
| Dividend ban | Until production investment |
| Buyback ban | Programs paused |
| Exec pay cap | $5M max |

Shipbuilding cited: Trump's order mentioned "slow naval shipbuilding timelines." Virginia-class and Columbia-class behind schedule.

See [[Trump defense budget]] for full context.

---

## Gulfstream certification dispute (Feb 2026)

[[Donald Trump]] threatened to decertify all Canadian-built aircraft (i.e. [[Bombardier]]) and impose 50% tariffs unless [[Canada]] certified Gulfstream business jets. Transport Canada certified the G500 and G600 on Feb 15, 2026. The G700 and G800 remain pending due to a fuel-system icing concern — the [[FAA]] and EASA granted temporary exemptions, but Canada held to stricter standards.

The dispute exposed the competitive tension: Bombardier's Global Express competes directly with Gulfstream in the ultra-long-range segment. See [[Canada Gulfstream certification]] for full timeline.

---


## Q1 2026 earnings and defense backlog signal (Apr 2026)

General Dynamics' Apr 29 move was an order-book repricing: the stock moved from $313.68 to $338.73, +8.0%, after Q1 results showed both operating leverage and very strong defense demand.

| Metric | Q1 2026 | YoY / note |
|--------|---------|------------|
| Revenue | $13.481B | +10.3% |
| Operating earnings | $1.420B | +12.0% |
| Diluted EPS | $4.10 | +12.0% |
| Operating cash flow | $2.2B | 192% of net earnings |
| Orders | $26.6B | 2.0x book-to-bill |
| Backlog | $130.8B | — |
| Total estimated contract value | $188.4B | backlog + potential contract value |

The segment mix was the important detail. Marine Systems revenue rose 21.0% to $4.343B and operating earnings rose 26.4% to $316M, while Aerospace revenue rose 8.4% and the defense segments posted a 2.2x book-to-bill. That makes the quarter less about one-time margin and more about submarine/shipbuilding demand converting into funded backlog.

| Business line | Q1 revenue | YoY | Operating earnings | YoY |
|---------|------------|-----|--------------------|-----|
| Aerospace | $3.279B | +8.4% | $493M | +14.1% |
| Marine Systems | $4.343B | +21.0% | $316M | +26.4% |
| Combat Systems | $2.283B | +4.9% | $310M | +6.5% |
| Technologies | $3.576B | +4.2% | $339M | +3.4% |

*Source: General Dynamics Q1 2026 earnings release, filed as [[SEC]] 8-K earnings exhibit (Apr 29, 2026). Price move verified in local DB: Apr 28 close $313.68 → Apr 29 close $338.73.*

---

## Investment case

Bull:
- Columbia-class is "no fail" program (\#1 DoD priority)
- Submarine demand ([[China]] threat)
- Gulfstream business jet cycle
- $90B+ backlog visibility

Bear:
- Executive order restrictions (Jan 2026)
- Shipbuilding execution risk
- Less AI/autonomy exposure than peers
- Business jet cyclicality

---

## Related

### Securities
- [[General Dynamics securities note]] — equity-market note


Subsidiaries:
- [[GDIT]] — [[IT services]] division (~$10B+)

Policy:
- [[Trump defense budget]] — -4.2% on EO, shipbuilding cited

Competitors:
- [[Bombardier]] — business jet competitor (Global Express, Challenger)

Peers:
- [[RTX]], [[Northrop Grumman]] — defense primes
- [[Lockheed Martin]], [[L3Harris]] — defense primes

Theses:
- [[Long defense AI]] — thesis (less direct)

Events:
- [[Canada Gulfstream certification]] — G500/G600 certified, G700/G800 pending

Context:
- [[European defense spending]] — contrast

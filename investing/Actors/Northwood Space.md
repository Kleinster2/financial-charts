---
aliases: [Northwood]
---
#actor #space #satellite #infrastructure #private

**Northwood [[Space]]** — Satellite ground station infrastructure. Building the "data highway between Earth and space."

---

## Why Northwood [[Space]] matters

| Metric | Value |
|--------|-------|
| Founded | 2022 |
| HQ | Torrance, California |
| Total raised | **$36M+** |
| CEO | [[Bridgit Mendler]] |
| CTO | Griffin Cleverly |

---

## Funding

| Round | Date | Amount | Lead |
|-------|------|--------|------|
| Seed | Feb 2024 | $6.[[3M]] | [[a16z]], [[Founders Fund]] |
| **Series A** | **Apr 2025** | **$30M** | Alpine [[Space]] Ventures, [[a16z]] |
| **Total** | | **$36M+** | |

**Series A investors:**
- Alpine [[Space]] Ventures (lead)
- [[Andreessen Horowitz]] ([[a16z]])
- Also Capital
- [[Founders Fund]]
- [[StepStone Group]]
- BoxGroup, Humba Ventures, Balerion [[Space]] Ventures

---

## Team

### Founders

| Name | Role | Background |
|------|------|------------|
| [[Bridgit Mendler]] | CEO | MIT, Harvard Law, [[Disney]] |
| Griffin Cleverly | CTO | [[Lockheed Martin]] |
| Shaurya Luthra | Co-founder | [[Lockheed Martin]] |

### Early employees from:
- SpaceX
- [[Palantir]]
- [[Northrop Grumman]]

---

## Technology

### Portal terminal

| Spec | Value |
|------|-------|
| Throughput | Up to **100 Gbps** backhaul |
| EIRP | **20x higher** than previous gen |
| Status | Passed operational tests (Jun 2025) |
| Claim | Most powerful commercial comms phased array ever built |

**First test:** Collected data from US [[Defense]] Meteorological Satellite Program spacecraft.

### Key innovations

| Metric | Improvement |
|--------|-------------|
| Cost vs. existing phased array | **10x reduction** |
| Time to field | **5x reduction** |
| Approach | Vertically integrated, software-defined |

---

## Manufacturing

| Metric | Value |
|--------|-------|
| Facility | 35,000 sq ft, Torrance, CA |
| Capacity | **16 Portal arrays/month** |
| Approach | Designed for mass production |

---

## Deployment

| Milestone | Timeline |
|-----------|----------|
| First sites | Q4 2025 |
| Global deployment | 2026 |
| Six continents | End of 2026 |

**Goal:** Surge capacity, redundancy, and global reach for modern satellite missions.

---

## Business model

**Ground Station as a Service:**
- Shared infrastructure for satellite operators
- Software-defined, flexible capacity
- Alternative to building dedicated ground stations

**Customer demo:** [[Planet Labs]] — validated 10x cost reduction.

---

## Market context

**Problem:** Satellite ground infrastructure is:
- Expensive to build
- Inflexible
- Bottleneck for satellite data

**Opportunity:** Mega-constellations (Starlink, OneWeb, etc.) need scalable ground segment.

---

## Competitive landscape

| Company | Approach |
|---------|----------|
| **Northwood [[Space]]** | Shared, high-throughput phased array |
| AWS Ground Station | Cloud-integrated |
| [[Microsoft]] Azure Orbital | Cloud-integrated |
| KSAT | Traditional ground station network |
| Leaf [[Space]] | Ground station as a service |

---

## Investment case

**Bull:**
- Elite backers ([[a16z]], [[Founders Fund]])
- 10x cost reduction demonstrated
- Most powerful commercial phased array
- Vertically integrated manufacturing
- Mega-constellation tailwind

**Bear:**
- Private — no direct investment
- Early stage, pre-revenue
- AWS/Azure competition
- Execution risk on global deployment
- Satellite market cyclicality

---

## Quick stats

| Metric | Value |
|--------|-------|
| Status | Private |
| Total raised | $36M+ |
| Key product | Portal terminal |
| Throughput | 100 Gbps |

---

## Related

- [[Bridgit Mendler]] — CEO/founder
- [[SpaceX]] — potential customer/competitor
- [[Starlink]] — mega-constellation context
- [[Amazon]] — AWS Ground Station competitor

---

## Sources

- [SpaceNews on Series A](https://spacenews.com/northwood-raises-30-million-to-establish-ground-station-network/)
- [Via Satellite coverage](https://www.satellitetoday.com/finance/2025/04/22/northwood-space-raises-30m-series-a-to-disrupt-satellite-ground-stations/)
- [Payload [[Space]] on Portal](https://payloadspace.com/northwoods-satellite-ground-terminal-is-ready-to-talk/)

*Created 2026-01-09*

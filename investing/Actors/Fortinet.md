---
aliases: [FTNT]
---
#actor #tech #cybersecurity #usa #public

**Fortinet** — [[Cybersecurity]] company. Firewall and network security leader.

---

## Sector correlation

| Sector | ETF | Correlation |
|--------|-----|-------------|
| Software | IGV | 0.73 |
| Technology | XLK | 0.69 |
| [[Consumer]] | XLY | 0.66 |
| *S&P 500* | *SPY* | *0.70* |

FTNT trades as a core Software name (IGV r = 0.73).

---

## Why Fortinet matters

| Metric | Value |
|--------|-------|
| Market cap | ~$75B |
| Ticker | FTNT |
| Revenue | ~$6B/year |
| CEO | Ken Xie (founder) |

---

## Products

| Product | Description |
|---------|-------------|
| **FortiGate** | Next-gen firewall (core) |
| **FortiManager** | Centralized management |
| **FortiAnalyzer** | Security analytics |
| **FortiSIEM** | Security operations |
| **FortiSASE** | Secure access service edge |

Security Fabric = integrated platform.

---

## Competitive position

| Company | Focus |
|---------|-------|
| [[Palo Alto Networks]] | Enterprise, cloud security |
| **Fortinet** | Mid-market, integrated platform |
| [[CrowdStrike]] | Endpoint, cloud-native |
| [[Check Point]] | Legacy firewall |

Fortinet strong in mid-market, price-competitive.

---

## Business model

**Hardware + subscription:**
- FortiGate appliances
- FortiGuard security subscriptions
- Services and support
- Billings growth key metric

High gross margins, growing recurring revenue.

---

## Investment case

**Bull:**
- Integrated security platform
- Mid-market dominance
- SASE opportunity
- Founder-led

**Bear:**
- Enterprise penetration limited
- Cloud-native competition
- Hardware refresh cycles
- Palo Alto competition

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | FTNT |
| Market cap | ~$75B |
| Core product | FortiGate firewall |
| CEO | Ken Xie |

*Created 2026-01-08*

---

## Related

- [[Palo Alto Networks]] — competitor
- [[CrowdStrike]] — competitor
- [[Cisco]] — competitor (network security)


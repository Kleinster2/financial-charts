---
aliases: [BA.L, BAE]
---
#actor #defense #uk #public #ai

**BAE Systems** — UK defense prime. Electronic warfare, naval ships, combat vehicles. AI for defense systems.

---

## Why BAE matters

| Metric | Value |
|--------|-------|
| Market cap | ~£45B (~$55B) |
| Ticker | BA.L (LSE) |
| Focus | Electronic warfare, naval, land |
| Backlog | £70B+ |

---

## Business segments

| Segment | Products |
|---------|----------|
| Electronic Systems | EW, sensors, precision munitions |
| Platforms & Services | Combat vehicles, naval ships |
| Air | Typhoon, F-35 components, Tempest |
| Maritime | Naval ships, submarines |

---

## AI/autonomy applications

| Area | Application |
|------|-------------|
| Electronic warfare | AI signal processing |
| Autonomous vehicles | Ground robotics |
| Cyber | AI threat detection |
| Manufacturing | AI optimization |

---

## Investment case

**Bull:**
- European defense spending surge
- NATO rearmament
- Electronic warfare expertise
- Typhoon/F-35 production

**Bear:**
- UK pound exposure
- UK government dependency
- US market competition
- Long program cycles

---

## Related

- [[Long defense AI]] — thesis
- [[Raytheon]], [[Northrop Grumman]] — US peers
- [[Anduril]] — AI defense disruptor

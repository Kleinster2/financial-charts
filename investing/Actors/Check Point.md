---
aliases: [CHKP, Check Point Software]
---
#actor #cybersecurity #israel #public

**Check Point** — Israeli cybersecurity pioneer. Invented the firewall. Network security focus.

---

## Why Check Point matters

| Metric | Value |
|--------|-------|
| Ticker | CHKP (NASDAQ) |
| Market cap | ~$20B |
| HQ | Tel Aviv, Israel |
| Founded | 1993 |
| Key product | Firewall (invented it) |

---

## Products

| Category | Products |
|----------|----------|
| **Network Security** | Quantum firewall (flagship) |
| **Cloud Security** | CloudGuard |
| **Endpoint** | Harmony |
| **Security Management** | Infinity platform |

**Historical significance:** Check Point invented the commercial firewall in 1994 — foundational cybersecurity technology.

---

## China cybersecurity ban (Jan 2026)

**Banned by Beijing** — China instructed companies to stop using Check Point products and replace with domestic alternatives by H1 2026.

**Other Israeli firms banned:** CyberArk, Orca Security, Cato Networks, Imperva ([[Thales]])

**Rationale:** Beijing fears Western cybersecurity software could collect and transmit confidential information abroad.

**Timing:** Ban announced days after Chinese envoy Zhai Jun visited Israel (Jan 2026).

See [[China cybersecurity ban]] for full context.

---

## Three-startup acquisition spree (Feb 2026)

On Feb 12, 2026, Check Point acquired three early-stage Israeli startups for a combined ~$150M:

| Company | Focus | Seed raised | Est. deal value |
|---------|-------|-------------|-----------------|
| Cyclops Security | Agentic exposure management / CAASM | $12.5M | ~$85M |
| [[Cyata]] | Agentic identity governance | $8.5M | — |
| Rotate | MSP security platform (acqui-hire) | ~$8M | — |

CEO Nadav Zafrir framed it as securing customers' AI transformation. [[Cyata]] adds AI agent discovery, governance, and control — addressing the [[Agentic AI security]] gap where traditional IAM and NHI tools fall short. Cyclops provides attack surface management across cloud/OT/SaaS. Rotate accelerates Check Point's MSP channel.

---

## Competitive position

| Company | Focus | Origin |
|---------|-------|--------|
| **Check Point** | Network security, firewall pioneer | Israel |
| [[Palo Alto Networks]] | Platform consolidation | USA |
| [[Fortinet]] | Mid-market firewall | USA |
| [[CrowdStrike]] | Endpoint | USA |

---

## Investment case

**Bull:**
- Firewall market leader
- Recurring revenue model
- Israeli cyber expertise
- Stable cash generation

**Bear:**
- Growth slower than peers
- Platform competition (Palo Alto)
- China market now closed
- Premium valuation for growth rate

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | CHKP |
| Market cap | ~$20B |
| Founded | 1993 |
| Founder | Gil Shwed |

*Created 2026-01-15*

---

## Related

- [[Palo Alto Networks]] — competitor (platform)
- [[Fortinet]] — competitor (firewall)
- [[CrowdStrike]] — competitor (endpoint)
- [[China cybersecurity ban]] — banned in China (Jan 2026)
- [[China]] — policy driver
- [[Israel]] — home country
- [[Cyata]] — acquired Feb 2026 (agentic identity)
- [[Agentic AI security]] — category entered via Cyata
- [[CyberTech Global Tel Aviv 2026]] — keynote participant

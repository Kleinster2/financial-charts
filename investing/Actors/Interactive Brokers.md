---
aliases: [IBKR, IB]
---
#actor #fintech #brokerage #usa #public

**Interactive Brokers** — Global electronic brokerage. Favored by sophisticated traders. Lowest margin rates, global market access.

---

## Why IBKR matters

IBKR is the broker for serious traders:

| Strength | Description |
|----------|-------------|
| Global access | 150+ markets, 27 currencies |
| Lowest costs | Best margin rates industry |
| Technology | Professional-grade platform |
| Client type | Sophisticated, high-volume |

---

## Differentiation

**Not competing with Robinhood:**
- Complex platform (learning curve)
- Professional tools
- Global markets access
- Options, futures, forex, bonds
- Hedge fund / RIA services

**Target:** Traders who outgrow Robinhood.

---

## Business model

| Revenue | Description |
|---------|-------------|
| Commissions | Low but high volume |
| Net interest | Margin lending, cash |
| Market making | Timber Hill legacy |
| Fees | Data, platform |

---

## Founder-led

**Thomas Peterffy:**
- Founder, chairman, major shareholder
- Pioneer of electronic trading
- Hungarian immigrant success story
- Still actively involved

---

## GameStop systemic risk warning (Jan 2026)

Peterffy gave the most detailed account yet of how close the financial system came to collapse in Jan 2021. IBKR customers had bought [[GameStop]] on margin at low prices; those shares were loaned to short sellers. If margin customers sold a fraction to pay back loans, brokers would have been forced to recall shares — but SI exceeded total float.

"Wall Street would have collapsed... we were really, really shaking in our pants." The system was saved because retail investors held rather than sold — "I didn't know that these people didn't know what the rules were."

**The vulnerabilities remain unpatched** as of Jan 2026. IBKR has proposed:

| Proposal | Detail |
|----------|--------|
| Daily SI publication | Real-time short interest transparency |
| Rising margin on SI | Maintenance margin increases above 50% SI |
| Position limits at 100% SI | No new shorts when SI hits 100% |

None adopted by regulators.

Peterffy also notes: "The customer base is becoming much more sophisticated. People certainly learned a lot about the market."

*Source: Barron's, Jan 23 2026*

---

## Investment case

**Bull:**
- Global market access moat
- Lowest cost provider
- Sophisticated client base (sticky)
- Growing international
- Benefits from volatility

**Bear:**
- Complex platform limits TAM
- Founder risk (Peterffy aging)
- Competition from free brokers
- Regulatory risk (global operations)

---

## Financials

| Metric | Value |
|--------|-------|
| Ticker | IBKR (NASDAQ) |
| Market cap | ~$70B |
| Revenue (TTM) | ~$5B |
| Client accounts | 3M+ |
| Client equity | $500B+ |
| Founded | 1978 |

---

## vs Peers

| Aspect | IBKR | [[Charles Schwab]] | [[Robinhood]] |
|--------|------|-------------------|---------------|
| Target | Sophisticated | Mass affluent | Retail/young |
| Markets | 150+ global | US focused | US focused |
| Margin rates | Lowest | Higher | Higher |
| Platform | Complex | Moderate | Simple |
| Crypto | Limited | Limited | Core |

---

## Related

- [[GameStop]] — systemic risk event (halted buying Jan 28, 2021; Peterffy "unpatched" warning)
- [[Retail investor flows]] — structural shift; IBKR proposed reforms
- [[Charles Schwab]] — competitor (different segment)
- [[Robinhood]] — competitor (different segment)
- [[Citadel Securities]] — market structure peer


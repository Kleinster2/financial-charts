---
aliases: [PRU, Prudential, Prudential Financial Inc]
---
#actor #finance #insurance #life #assetmanagement #usa #public

**Prudential Financial** — Major US life insurer and asset manager. $1.6T AUM via PGIM. NYSE: PRU. Unlike MetLife, Prudential is *leaning into* the PE-insurance model via Prismic Life (Bermuda reinsurance sidecar with Warburg Pincus). Straddles traditional insurance and alternative asset management.

---

## Why Prudential matters

| Metric | Value |
|--------|-------|
| Ticker | PRU (NYSE) |
| Stock price | ~$117 (Dec 2025) |
| AUM (PGIM) | $1.6T |
| Segments | PGIM, Retirement Strategies, Group Insurance, Individual Life, International |
| Products | Annuities (fixed, indexed, variable), life, retirement, asset management |

---

## Business segments

### PGIM (asset management)
- $1.6T AUM across public fixed income, equities, real estate, private credit, alternatives
- One of the world's largest institutional asset managers
- Dec 2025: JV with Lincoln Property for outpatient medical investments ($213B PGIM AUM in the strategy)

### Retirement Strategies
- Institutional pension products for public/private/non-profit sponsors
- Individual variable and fixed annuities
- Participates in [[Pension risk transfer]] market

### Group Insurance
- Life, disability, supplemental health for employers

### Individual Life
- Term, universal, variable life products

### International
- Japan (major market), Brazil, other emerging markets

---

## Prismic Life — Prudential's Bermuda sidecar play

**Prudential is building its own PE-insurance model:**

Prismic Life is a Bermuda-based life and annuity reinsurance platform co-sponsored with Warburg Pincus:
- Nov 2025: Third capital raise of $1.3B
- Expected to allocate ~$15B to assets managed by PGIM and Warburg Pincus
- Effectively a [[Reinsurance sidecars|reinsurance sidecar]] — third-party capital + PGIM asset management

This positions Prudential as both a traditional insurer *and* a PE-style platform — using PGIM's asset management to generate spread on reinsured liabilities, similar to how [[Apollo]] uses [[Athene]].

---

## Annuity product focus

Key products reflect shift toward principal protection + market participation:
- **FlexGuard 2.0** — registered index-linked annuity (buffer annuity)
- **SurePath Income** — fixed indexed annuity
- **WealthGuard** — multi-year guaranteed annuity

Low annuity adoption (29% per Prudential's 2025 Global Retirement Pulse Survey) signals growth runway.

---

## Competitive positioning

Prudential is unique among traditional insurers: rather than simply shedding liabilities to PE platforms, it's building a competing model via Prismic/PGIM. This hybrid approach — traditional distribution + alternative asset management — could represent the next evolution of [[PE-insurance convergence]].

---

## Related

- [[MetLife]] — peer (traditional life insurer, de-risking approach)
- [[PE-insurance convergence]] — Prudential building hybrid model via Prismic
- [[Reinsurance sidecars]] — Prismic Life structure
- [[Athene]] — competitor (PE-owned platform)
- [[Pension risk transfer]] — Retirement Strategies participates
- [[Private credit]] — PGIM deployment asset class


---
aliases: [SNDK]
---
#actor #memory #storage #usa #public

**SanDisk** — Flash storage company. **Best S&P 500 performer 2025** (+559%). AI memory/storage beneficiary.

---

## Why SanDisk matters

| Metric | Value |
|--------|-------|
| Ticker | SNDK |
| 2025 return | **+559%** (best in S&P 500) |
| 2026 YTD | **+60%+** |
| Focus | NAND flash storage |
| Driver | AI memory demand |

---

## 2025-2026 performance

**Breakout stock:**

| Period | Return | Note |
|--------|--------|------|
| 2025 | **+559%** | Best S&P 500 performer |
| Jan 2026 | **+60%+** | Continues climbing |
| Catalyst | [[Jensen Huang]] highlighting memory/storage need |

**[[CES 2026]]:** Stock soared 37% in one week after Huang emphasized memory and storage demand.

---

## History

| Date | Event |
|------|-------|
| 1988 | Founded |
| 2016 | Acquired by Western Digital for $19B |
| 2023 | Spun off from [[Western Digital]] |
| 2024 | Re-listed as independent |

**WD spin-off:** Western Digital split into SanDisk (flash) and WD (HDDs) to unlock value.

---

## Products

| Category | Products |
|----------|----------|
| [[Consumer]] | USB drives, SD cards |
| Enterprise | SSDs for data centers |
| Embedded | Smartphone/device storage |

---

## AI storage demand

**Why AI needs storage:**
- Training data: Petabytes of datasets
- Model checkpoints: Save/resume training
- Inference: Fast model loading
- Logs/telemetry: Massive output

**[[Jensen Huang]] ([[CES 2026]]):** Highlighted memory and storage as critical alongside compute.

---

## Memory shortage tailwind

Part of [[Memory shortage 2025-2026]]:
- NAND supply tight
- AI consuming capacity
- Pricing power
- Validates SanDisk rally

---

## Investment case

**Bull:**
- AI storage demand secular
- Memory shortage tailwind
- Best-performing stock momentum
- Pure-play flash exposure

**Bear:**
- +559% already priced in?
- NAND cyclical historically
- Competition ([[Samsung]], SK Hynix, Micron)
- Valuation stretched

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | SNDK |
| 2025 return | +559% |
| Focus | NAND flash storage |
| History | WD spin-off (2023) |

*Updated 2026-01-15*

---

## Related

- [[Western Digital]] — former parent
- [[Memory shortage 2025-2026]] — demand driver
- [[Micron]] — competitor
- [[SK Hynix]] — competitor
- [[Kioxia]] — competitor (NAND)
- [[NVIDIA]] — Jensen highlighted storage need

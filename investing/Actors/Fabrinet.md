---
aliases: [FN]
---
#actor #interconnect #manufacturing #thailand

**Fabrinet** — Contract manufacturer for optical components. Thailand-based. Critical supply chain node.

---

## Why Fabrinet matters

Fabrinet manufactures the optical components that connect AI data centers:

| Metric | Value |
|--------|-------|
| Business | Precision optical manufacturing |
| Location | Thailand (mainly) |
| Customers | Coherent, Lumentum, others |
| AI exposure | Transceiver manufacturing |

---

## Business model

**Contract manufacturing:**
- Customers design products
- Fabrinet manufactures
- Precision assembly
- High-mix, low-to-medium volume

**Vertically integrated:**
- Optical assembly
- Test and packaging
- PCB assembly
- Complete modules

---

## Key customers

| Customer | Products |
|----------|----------|
| [[Coherent]] | Optical modules |
| [[Lumentum]] | Lasers, components |
| NVIDIA | Optical transceivers |
| Various | [[Telecom]], datacom |

**Customer concentration:** Top customers = significant revenue.

---

## Thailand advantage

**Why Thailand?**
- Skilled labor pool
- Competitive costs
- Established optical cluster
- Political stability

**Capacity expansion:**
- Adding facilities
- AI demand driving growth
- Investment in automation

---

## AI transceiver opportunity

**800G/1.6T ramp:**
- Transceivers need precision assembly
- Volume growing rapidly
- Fabrinet = preferred manufacturer
- Capacity constrained

**Supply chain role:**
```
Component supplier → Fabrinet → Transceiver customer → Hyperscaler
(Lumentum lasers)    (assembly)   (Coherent)          ([[Microsoft]])
```

---

## Competitive position

| Competitor | Location | Fabrinet vs |
|------------|----------|-------------|
| Jabil | Global | More optical focus |
| Flex | Global | Optical specialization |
| Sanmina | Global | Precision assembly |
| [[BYD]] Electronic | [[China]] | Non-[[China]] location |

Thailand location = supply chain diversification benefit.

---

## Investment case

**Bull:**
- AI optical demand
- Manufacturing partner to leaders
- Thailand capacity value
- Non-[[China]] supply chain
- Precision manufacturing moat

**Bear:**
- Customer concentration
- Contract manufacturer margins
- Labor cost inflation
- Geopolitical risks (Thailand)
- Capacity execution

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | FN |
| Market cap | ~$10B+ |
| HQ | George Town, Cayman Islands |
| Operations | Mainly Thailand |
| Focus | Optical manufacturing |

*Updated 2026-01-01*

---

## Related

- [[Coherent]] — customer (optical module manufacturing)
- [[Lumentum]] — customer (laser manufacturing)
- [[NVIDIA]] — customer (optical transceivers)
- [[Arista Networks]] — adjacent (networking OEM)
- [[AI hyperscalers]] — end customers (transceiver demand)
- [[Thailand Tech]] — location context

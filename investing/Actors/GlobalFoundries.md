---
aliases: [GFS, GloFo]
---
#actor #foundry #usa

**GlobalFoundries** — Third-largest foundry globally. Spun off from AMD (2009). Focus on mature/specialty nodes (12nm+) — automotive, RF, IoT. Mubadala-controlled, US fabs.

---

## Why GlobalFoundries matters

GF occupies a strategic niche between leading-edge and commodity:

| Role | Details |
|------|---------|
| Position | #3 foundry (after TSMC, Samsung) |
| Strategy | Mature nodes, specialty processes |
| Ownership | Mubadala (Abu Dhabi) ~85% |
| US presence | Malta NY fab — geopolitical value |

---

## Strategic decision: exit leading edge (2018)

**Pivotal choice:**

| Decision | Rationale |
|----------|-----------|
| Exit <7nm | Couldn't compete with TSMC capex |
| Focus | 12nm and above |
| Specialty | RF, embedded flash, FD-SOI |
| Result | Profitable, less cyclical |

Avoided the Samsung trap of bleeding cash on leading-edge race.

---

## Product portfolio

**Specialty processes:**

| Technology | Applications |
|------------|--------------|
| RF-SOI | [[5G]], WiFi, radar |
| FD-SOI | IoT, automotive, low-power |
| Embedded flash | MCUs, automotive |
| SiGe | RF front-end |
| BCD | Power management |

---

## Manufacturing footprint

| Location | Nodes | Notes |
|----------|-------|-------|
| Malta, New York | 12nm-14nm | Flagship, CHIPS Act funding |
| Dresden, [[Germany]] | 22nm-28nm | Former AMD fab |
| Singapore | 40nm-65nm | Acquired from Chartered |

**US fab = geopolitical asset** — DoD supplier, CHIPS Act beneficiary.

---

## Key customers

| Customer | Products |
|----------|----------|
| AMD (legacy) | Older chip designs |
| [[Qualcomm]] | RF, connectivity |
| [[NXP]] | [[Automotive]] MCUs |
| [[Infineon]] | Specialty |
| [[Broadcom]] | RF |
| [[STMicro]] | Specialty |

Auto and RF are core verticals.

---

## [[Automotive]] focus

**Sweet spot for auto semis:**

| Advantage | Details |
|-----------|---------|
| Mature nodes | Auto doesn't need 3nm |
| Specialty processes | Embedded flash, BCD |
| Quality | [[Automotive]]-grade certification |
| Long lifecycles | Auto chips run 10+ years |

Competes with tower Semiconductor, UMC, SMIC on mature auto nodes.

---

## CHIPS Act funding

**US government support:**

| Award | Details |
|-------|---------|
| Amount | ~$1.5B |
| Purpose | Malta fab expansion |
| Focus | Domestic capacity |
| National security | DoD supplier qualification |

GF is a key beneficiary of US reshoring push.

---

## Competitive position

| vs | GF advantage | GF disadvantage |
|----|--------------|-----------------|
| [[TSMC]] | Specialty processes, US presence | Scale, leading edge |
| [[Samsung]] | Profitability, focus | Advanced nodes |
| UMC | Technology breadth | Cost |
| SMIC | US/[[Europe]] accessible | [[China]] cost |
| [[Intel Foundry Services]] | Specialty expertise | IFS scale ambitions |

---

## Mubadala ownership

**Abu Dhabi sovereign wealth:**

| Metric | Value |
|--------|-------|
| Stake | ~85% |
| Acquired | AMD spinoff (2009) |
| IPO | Oct 2021 |
| Control | Effective control despite public listing |

Mubadala provides patient capital — not forcing leading-edge race.

---

## Financial position

| Metric | Value |
|--------|-------|
| Ticker | GFS (NASDAQ) |
| Market cap | ~$25B |
| Revenue (TTM) | ~$7B |
| Gross margin | ~25% |
| Capex | ~$1.5B/year |
| P/E | ~20x |
| Float | ~15% (Mubadala controls rest) |

---

## Investment implications

**Bull case:**
- Specialty node leader
- US fab = geopolitical premium
- CHIPS Act beneficiary
- [[Automotive]] secular growth
- Less cyclical than leading-edge

**Bear case:**
- Low float, Mubadala control
- Margin pressure from UMC, SMIC
- No leading-edge upside
- Auto semi cycle exposure

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | GFS |
| Market cap | ~$25B |
| Revenue | ~$7B |
| Position | #3 foundry |
| Nodes | 12nm+ (mature) |
| US fab | Malta, NY |
| Owner | Mubadala (~85%) |

*Updated 2026-01-04*

---

## Related

- [[TSMC]] — competitor (#1 foundry)
- [[Samsung]] — competitor (#2 foundry)
- [[Intel Foundry Services]] — competitor (US foundry)
- [[UMC]] — competitor (mature nodes)
- [[SMIC]] — competitor ([[China]])
- [[AMD]] — former parent (spun off 2009)
- [[Mubadala]] — majority owner (Abu Dhabi)
- [[CHIPS Act]] — beneficiary (US fab funding)
- [[NXP]] — customer (automotive)
- [[Infineon]] — customer (specialty)
- [[Automotive semiconductors]] — key market
- [[Taiwan concentration risk]] — GF as hedge

---
aliases: [SONO]
---
#actor #audio #consumer-electronics

**Sonos** — Premium wireless speaker company. [[Samsung]] now competing directly with new Wi-Fi speakers ([[CES 2026]]).

---

## Why Sonos matters

Defined the premium wireless audio category:

| Metric | Value |
|--------|-------|
| Ticker | SONO (NASDAQ) |
| Founded | 2002 |
| Focus | Wireless home audio |
| Position | Category leader |

---

## Product lineup

| Category | Products |
|----------|----------|
| Speakers | One, Era 100, Era 300, Five |
| Soundbars | Beam, Arc, Ray |
| Subwoofers | Sub, Sub Mini |
| Portable | Move, Roam |
| Headphones | Ace (2024) |

**Multi-room audio:** Core differentiator — synchronized playback across rooms.

---

## Competitive landscape

| Competitor | Approach |
|------------|----------|
| **Samsung** | New Wi-Fi speakers ([[CES 2026]]) |
| Apple HomePod | Siri ecosystem |
| [[Amazon]] Echo | Alexa ecosystem |
| [[Google]] Nest | [[Google]] Assistant |
| Bose | Premium audio |

**Samsung threat (Jan 2026):** CES reveal of "bold design" Wi-Fi speakers sets up direct Sonos competition. Samsung has distribution and brand advantage.

---

## Business challenges

**2024 app redesign crisis:**
- Major app update broke functionality
- Customer backlash
- CEO Patrick Spence departed
- Stock declined significantly

**Headphones launch:**
- Sonos Ace (2024) — first headphones
- Expanding beyond home audio
- Mixed reception

---

## Investment case

**Bull:**
- Category leadership
- Premium brand positioning
- Loyal customer base
- Multi-room ecosystem lock-in

**Bear:**
- Samsung/big tech competition
- App crisis damaged brand
- Hardware margins pressured
- Limited ecosystem vs Apple/[[Amazon]]/[[Google]]

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | SONO (NASDAQ) |
| Market cap | ~$1-2B |
| HQ | Santa Barbara, CA |
| Founded | 2002 |

*Created 2026-01-05*

---

## Related

- [[Samsung]] — competitor ([[CES 2026]] speakers)
- [[Apple]] — competitor (HomePod)

---
aliases: [1337]
---
#actor #gaming #singapore #consumer-electronics

**Razer** — [[Gaming]] peripherals and lifestyle brand. Known for RGB everything. Project Ava holographic AI avatar at CES 2026.

---

## Why Razer matters

[[Gaming]] hardware leader experimenting with AI:

| Metric | Value |
|--------|-------|
| HQ | Singapore (founded in San Diego) |
| Ticker | 1337 (Hong Kong) |
| Focus | [[Gaming]] peripherals, laptops |
| Brand | Premium gaming lifestyle |

---

## Product categories

| Category | Examples |
|----------|----------|
| **Mice** | DeathAdder, Viper, Basilisk |
| **Keyboards** | BlackWidow, Huntsman |
| **Headsets** | Kraken, BlackShark |
| **Laptops** | Blade series |
| **Controllers** | Wolverine, Kishi |
| **Streaming** | Seiren microphones, Kiyo webcams |

---

## CES 2026 — Project Ava

**Holographic AI avatar:**

| Feature | Description |
|---------|-------------|
| Form factor | Animated character in glass tube |
| Technology | Holographic display |
| Function | AI chatbot with physical presence |
| Status | Concept / demo |

Similar approach to Lepro's "Ami" — giving AI avatars physical form factor.

---

## Business model

**[[Gaming]] lifestyle brand:**
- Premium pricing (Razer tax)
- RGB lighting ecosystem (Chroma)
- Community-driven (cult following)
- Expanding beyond peripherals

---

## Investment case

**Bull:**
- Strong gaming brand loyalty
- Esports/streaming tailwinds
- AI experimentation (Project Ava)
- Expanding product categories

**Bear:**
- Premium pricing pressure
- Competition from Logitech, SteelSeries, Corsair
- [[Gaming]] hardware cyclical
- Hong Kong listing limits liquidity

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | 1337 (Hong Kong) |
| HQ | Singapore |
| Founded | 2005 |
| Founders | Min-Liang Tan, Robert Krakoff |
| Focus | [[Gaming]] peripherals, lifestyle |
| Investable | Yes (Hong Kong) |

*Updated 2026-01-15*

---

## Related

- [[CES 2026]] — Project Ava holographic AI
- [[Corsair]] — competitor (gaming peripherals)
- [[Logitech]] — competitor (peripherals)

---
aliases: [SHEL, Royal Dutch Shell]
---
#actor #energy #oil #lng #uk #public

**Shell** — Third-largest Western oil major. World's largest [[LNG]] trader. Transitioning portfolio toward gas and low-carbon.

---

## Sector correlation

| Sector | ETF | Correlation |
|--------|-----|-------------|
| [[Energy and Utilities|Energy]] | XLE | 0.81 |
| Oil & Gas E&P | XOP | 0.79 |
| Industrials | XLI | 0.53 |
| *S&P 500* | *SPY* | *0.55* |

SHEL trades as a core Energy name (XLE r = 0.81).

---

## Why Shell matters

| Metric | Value |
|--------|-------|
| Ticker | SHEL (NYSE/LSE) |
| Market cap | ~$200B |
| Position | \#3 Western major |
| [[LNG]] | \#1 global trader |

---

## Business mix

| Segment | Focus |
|---------|-------|
| Upstream | [[Oil]] & gas production |
| Integrated Gas | [[LNG]], GTL |
| Downstream | Refining, chemicals |
| Renewables | Wind, solar, hydrogen |

---

## [[LNG]] dominance

**World's largest [[LNG]] portfolio:**
- ~70 mtpa equity capacity
- Global trading/shipping fleet
- Qatar expansion partner
- US Gulf Coast exports

[[LNG]] relevance for AI: Gas-fired power for data centers.

---

## Energy transition

| Initiative | Status |
|------------|--------|
| Offshore wind | Building portfolio |
| EV charging | Shell Recharge network |
| Hydrogen | Blue/green projects |
| Carbon capture | Multiple JVs |

Scaled back some renewable targets (2023) to focus on shareholder returns.

---

## Investment case

**Bull:**
- [[LNG]] super-cycle beneficiary
- Strong cash returns (buybacks)
- Integrated model resilience
- Gas as transition fuel

**Bear:**
- European regulatory pressure
- Transition strategy unclear
- Refining decline
- Climate litigation risk

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | SHEL |
| Market cap | ~$200B |
| Dividend yield | ~4% |
| HQ | London |

*Created 2026-01-06*

---

## Related

- [[Exxon]] — peer (US supermajor)
- [[Chevron]] — peer (US \#2)
- [[TotalEnergies]] — peer (European major)
- [[BP]] — peer ([[UK]] major)
- [[Power constraints]] — context (gas for AI power)

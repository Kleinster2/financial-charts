---
aliases: [UBER]
---
#actor #tech #mobility #usa #public

Uber — Global mobility and delivery platform. Autonomous vehicle partnerships. AI for routing, pricing, matching.

---

## Q4 2025 earnings (Feb 4, 2026)

Beat on revenue, soft guidance. Stock -6%.

| Metric | Actual | Expected |
|--------|--------|----------|
| Q4 Revenue | $14.4B | $14.3B |
| Gross Bookings | $54.1B | $53.1B |
| EBITDA | $2.5B | — |
| MAUs | 202M | +18% YoY |
| Trips | 3.8B | +22% YoY |

Full year 2025: $193B gross bookings, $9.8B free cash flow (+42% YoY).

Q1 2026 guidance: EBITDA $2.37-2.47B (soft, stock sold off 6% post-call).

AV progress: Expects AV trips in up to 15 cities globally by end of 2026. AV trips per vehicle 30% higher on Uber platform vs standalone competitors.

*Source: CNBC, company filings, Feb 4 2026*

---

## Why Uber matters

| Metric | Value |
|--------|-------|
| Ticker | UBER (NYSE) |
| Market cap | ~$150B |
| Position | \#1 rideshare globally |
| Segments | Mobility, Delivery, Freight |

---

## Business segments

| Segment | Revenue | Focus |
|---------|---------|-------|
| Mobility | ~55% | Rideshare, rental |
| Delivery | ~35% | Uber Eats, grocery |
| Freight | ~10% | Trucking marketplace |

---

## AI applications

AI throughout the platform:

| Use Case | Technology |
|----------|------------|
| Dynamic pricing | Surge algorithms |
| ETA prediction | ML routing |
| Driver-rider matching | Optimization |
| Fraud detection | Pattern recognition |
| Delivery optimization | Multi-stop routing |

AI is core to unit economics — better matching = lower costs.

---

## Autonomous vehicle strategy

Partnerships over ownership:

| Partner | Focus |
|---------|-------|
| [[Waymo]] | Robotaxi deployment (US) |
| [[WeRide]] | Robotaxi (Middle East — 1,200 vehicles by ~2027) |
| Aurora | [[Autonomous trucking]] |
| Motional | Robotaxi ([[Hyundai]]/Aptiv) |
| Nuro | Autonomous delivery |

Uber exited AV development (sold ATG to Aurora) but partners with leaders.

---

## Robotaxi charging infrastructure (Feb 18, 2026)

$100M+ investment in EV charging for autonomous fleets:

| Detail | Value |
|--------|-------|
| Investment | >$100M |
| Chargers | ~1,000 new chargers globally |
| Initial US cities | Bay Area, LA, Dallas |
| Charging type | DC fast charging at autonomous depots + pit stops |
| AV city target | 10+ cities by end of 2026 |

Charging partnerships:

| Partner | Markets |
|---------|---------|
| [[EVgo]] | New York, LA, San Francisco, Boston |
| Electra | Paris, Madrid |
| Hubber | London |
| [[Ionity]] | London |

Strategy: Hybrid platform — human drivers and robotaxi partners on same network. Both need charging. Building infrastructure makes Uber essential to AV operators, not just a demand aggregator.

Uber currently offers robotaxis in 4 US cities plus Dubai, Abu Dhabi, and Riyadh. Has invested hundreds of millions in AV companies: [[Lucid]], Nuro, Wayve, Moove, Avomo. Stock +2.9% on announcement but -14% YTD — market still skeptical on AV strategy.

Source: [Bloomberg](https://www.bloomberg.com/news/articles/2026-02-18/uber-to-spend-100-million-on-autonomous-vehicle-charging-station-buildout)

---

## Waymo partnership

Robotaxi on Uber platform:
- Waymo vehicles bookable via Uber app
- Phoenix deployment (2023)
- Expanding to more cities
- Uber provides demand, Waymo provides AVs

[[Network effects]]: largest rider base makes Uber preferred AV partner.

---

## Competitive position

| Competitor | Market |
|------------|--------|
| [[Lyft]] | US rideshare \#2 |
| [[DoorDash]] | US delivery \#1 |
| Didi | [[China]] (exited) |
| Grab | [[Southeast Asia]] (invested) |

Uber is the only global scale player.

---

## Investment case

Bull:
- Path to sustained profitability
- AV optionality without capex
- [[Network effects]] moat
- Advertising revenue growth
- Freight marketplace potential

Bear:
- Regulatory risk (gig worker)
- AV disruption long-term
- Competition from robotaxi
- Delivery margin pressure

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | UBER |
| Market cap | ~$150B |
| Gross bookings | ~$35B/quarter |
| HQ | San Francisco |

*Created 2026-01-06*

---

## Related

- [[Waymo]] — partner (robotaxi)
- [[Aurora]] — partner (autonomous trucking)
- [[Tesla]] — potential competitor (robotaxi)
- [[Grab]] — investment ([[Southeast Asia]])
- [[WeRide]] — partner (Middle East robotaxi, 1,200 vehicles)
- [[Lime]] — strategic investment, demand channel for shared scooters/e-bikes
- [[EVgo]] — charging partner (NY, LA, SF, Boston)
- [[Lucid]] — AV investment

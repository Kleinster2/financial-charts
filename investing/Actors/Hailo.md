---
aliases: []
---
#actor #ai #semiconductor #israel #private

**Hailo** — Israeli edge AI chips. Hailo-8/10 accelerators. On-device AI.

---

## Why Hailo matters

Edge AI chip leader:

| Metric | Value |
|--------|-------|
| Focus | Edge AI accelerators |
| HQ | Tel Aviv, Israel |
| Raised | $340M+ |
| Product | Hailo-8, Hailo-10 |

---

## Edge vs cloud AI

**Why edge matters:**
- On-device processing
- No cloud latency
- Privacy (data local)
- Lower power
- Autonomous systems

---

## Product line

| Chip | Performance | Use Case |
|------|-------------|----------|
| Hailo-8 | 26 TOPS | Smart cameras |
| Hailo-8L | 13 TOPS | Entry edge |
| Hailo-10 | GenAI capable | Generative edge |

---

## Hailo-10

**Generative AI at edge:**
- Supports LLMs on-device
- GenAI inference
- Launched 2024
- Next-gen architecture

GenAI moving to edge.

---

## Use cases

| Application | Examples |
|-------------|----------|
| Smart cameras | Security, retail |
| [[Automotive]] | ADAS, in-cabin |
| Industrial | Inspection, robotics |
| [[Drones]] | Autonomous navigation |

---

## Customers

| Partner | Application |
|---------|-------------|
| [[BMW]] | [[Automotive]] |
| [[Sony]] | Cameras |
| Various OEMs | Edge devices |

---

## vs Competition

| Competitor | Hailo vs |
|------------|----------|
| NVIDIA (Jetson) | More power efficient |
| Qualcomm | Different architecture |
| [[Intel]] (Movidius) | Better performance/watt |
| [[Google]] (Edge TPU) | More flexible |

---

## Investment case

**Bull:**
- Edge AI growing fast
- Israeli engineering talent
- [[Automotive]] partnerships
- GenAI at edge = new market

**Bear:**
- NVIDIA/Qualcomm competition
- Private (not investable)
- Scale challenges
- Customer concentration

---

## Cap table / Investors

| Round | Date | Amount | Valuation |
|-------|------|--------|-----------|
| Series A | 2018 | $12M | — |
| Series B | 2020 | $60M | — |
| Series C | 2021 | $136M | ~$1B |
| Series D | 2023 | $120M | — |
| **Total** | | **$340M+** | |

**Key investors:**

| Investor | Notes |
|----------|-------|
| Poalim Equity | Israeli bank |
| [[NTT]] Finance | Japanese strategic |
| [[ABB]] | Industrial strategic |
| Qualcomm Ventures | Strategic |
| Latitude Ventures | Israeli VC |
| Entropy | Growth |

**Total raised:** $340M+

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | Private |
| Raised | $340M+ |
| Product | Hailo-8, Hailo-10 |
| Focus | Edge AI chips |

*Updated 2026-01-01*

---

## Related

- [[NVIDIA]] — competitor (Jetson edge chips)
- [[Mobileye]] — peer (Israeli edge AI)
- [[Israel Tech]] — sovereign context
- [[Qualcomm]] — competitor (edge AI)


---
aliases: [Unitree Robotics]
---
#actor #china #robotics #ai #private

Unitree — Hangzhou-based robotics company founded by [[Wang Xingxing]] in 2016. Started with quadruped robots (robot dogs), now the world's highest-volume legged robot manufacturer. [[China]]'s answer to [[Boston Dynamics]] at consumer price points.

---

## Quick stats

| Metric | Value |
|--------|-------|
| HQ | [[Hangzhou]], [[China]] |
| Founded | 2016 |
| Founder | [[Wang Xingxing]] (net worth $2.15B, Bloomberg Feb 2026) |
| Employees | ~500 (early 2025), growing to 1,000+ |
| Status | Private — IPO pending mid-2026, Shanghai [[STAR Market]] |
| Target valuation | ~$7B (50B yuan, Reuters Sep 2025) |
| Underwriter | [[CITIC Securities]] |
| Revenue | Q1 2025: 3.27B yuan (~$450M); FY2024: >1B yuan (~$140M) |
| Net margin | ~14.7% (Q1 2025) |
| Profitable since | 2020 (five consecutive years) |
| Total raised | ~$264M across 10 rounds |

---

## Products

| Model | Type | Price | Use case |
|-------|------|-------|----------|
| Go2 | Quadruped | ~$1,600 | Consumer, education |
| B2 | Quadruped | ~$20,000 | Industrial inspection (IP67) |
| G1 | Humanoid | ~$16,000 | Education, prosumer (43 DOF) |
| R1 | Humanoid | ~$5,900 | Consumer humanoid (Jul 2025) |
| H1 | Humanoid | ~$90,000 | Research |
| H2 | Humanoid | — | 1.8m, sword routines ([[2026 Spring Festival Gala]]) |

Key differentiator: Go2 at $1,600 vs. [[Boston Dynamics]] Spot at $75,000+. 10x cheaper through in-house QDD actuators, injection-molded chassis, and [[Yangtze River Delta]] supply chain density. See [[Unitree Cost Architecture]].

Unit sales (2024): 23,700 quadrupeds (~70% global share), 1,500+ humanoids. G1 shipped 5,000 units in H1 2025. FY2025 total: ~5,500 humanoids shipped.

**2026 production target: 10,000–20,000 humanoid robots** (CEO Wang Xingxing to 36Kr, Feb 2026). Would represent ~4x increase from 2025. Global humanoid shipments expected to reach "tens of thousands" with Unitree accounting for the bulk.

---

## What makes Unitree, Unitree

Three deep dives cover the structural advantages:

[[Unitree Cost Architecture]] — in-house PMSM actuators (~$30-60 each vs. $500-2,000+ for harmonic drives), 90% vertical integration, 284K+ identical motors/year, injection-molded polymer vs. CNC-machined aluminum. Structural cost moat analogous to [[DJI]] in drones.

[[Unitree Quadruped Bridge]] — 7 years of quadrupeds (2016-2023) gave shared RL algorithms, actuator hardware, manufacturing infrastructure, and 50,000+ deployed units of real-world data before building a single humanoid. Went from zero humanoid experience to 5,500+ units shipped in ~2 years — no competitor matched this pace.

[[Unitree GTM Strategy]] — [[Clayton Christensen]] disruption: consumer-first (Go2 at $1,600 for virality and volume), EDU variants ($8-12K, pure software margin), open developer ecosystem (30+ GitHub repos, ROS2 native), then enterprise migration ([[Great Wall Motors]], [[BYD]], [[Geely]], China Mobile). ~50% revenue from overseas.

---

## Spring Festival Gala performances

2025 — "YangBOT": 16 humanoids performing Yangge folk dance. Went viral. [[Wang Xingxing]] met [[Xi Jinping]] at a tech symposium weeks later.

2026 — "Wu BOT": With Henan Tagou Martial Arts School. H1/H2 performing parkour, Drunken Fist, nunchaku, trampoline backflips. Technical firsts: force-controlled weapon strikes, 3m aerial flips, 4 m/s cluster repositioning. Four companies paid ~100M yuan for 2026 partnerships: Unitree, [[MagicLab]], [[Galbot]], [[Noetix]]. Kung fu segment performed "fully autonomously" by G1 robots. See [[2026 Spring Festival Gala]].

---

## Robots making robots (Feb 2026)

Unitree deployed G1 humanoids in its own factory using the [[UnifoLM|UnifoLM-X1-0]] embodied AI model. Robots performing assembly and inspection tasks alongside human workers — a self-referential proof of concept (robots building robots). Strengthens the IPO case by demonstrating real factory utility beyond demos. Mid-2026 IPO timeline on track. See [[UnifoLM]] for the full model family and data engine strategy.

---

## Financials

> *Full analysis: [[Unitree Financials]]*

| Metric | Value |
|--------|-------|
| Gross margin | >50% (stable) |
| Net margin | ~14.7% (Q1 2025) |
| Revenue (Q1 2025) | 3.27B yuan (~$450M) — 3x FY2024 |
| Net profit (Q1 2025) | 480M yuan |
| Institutional forecast | 1.415B yuan net profit (2025), 1.856B (2026), 2.287B (2027) |
| IPO target | ~$7B → P/E ~36x on 2025E, ~27x on 2026E |

Humanoid gross margins ~8pp higher than quadruped. Revenue mix shifting fast: humanoids went from 0% to ~45% of revenue in 2 years. [[Meituan]] is second-largest shareholder (8.24%). Key investors: [[Tencent]], [[Alibaba]], [[Geely]], [[Sequoia]], [[CITIC]].

---

## Peers

| Company | HQ | Focus | Price tier | Status |
|---------|-----|-------|------------|--------|
| Unitree | Hangzhou | Quadruped + humanoid | Low-mid | Private (IPO mid-2026) |
| [[Boston Dynamics]] | US | Quadruped + humanoid | High | [[Hyundai]] subsidiary |
| [[Figure AI]] | US | Humanoid | High | Private ($2.6B raised) |
| [[Tesla]] Optimus | US | Humanoid | TBD | Tesla division |
| [[AGIBOT]] | Shanghai | Humanoid portfolio | Mid | Private (HK IPO rumored) |
| [[UBTech]] | Shenzhen | Humanoid (Walker) | Mid-high | HKEX listed |

---

## Investment case

Bull:
- Profitable robotics company with 10x YoY revenue growth
- Structural cost moat via [[Unitree Cost Architecture|vertical integration + Chinese supply chain]]
- [[Unitree Quadruped Bridge|Quadruped bridge]] gives 2-3 year head start over humanoid-first competitors
- IPO mid-2026 at $7B — potentially conservative by STAR Market standards
- Government-endorsed ([[Xi Jinping]] meeting, gala visibility, [[Pre-IPO tutoring]] completed in 4 months)

Bear:
- Q1 2025 revenue spike may be one-time orders, not sustainable demand
- Humanoid factory use still narrow (inspection/patrol, not assembly)
- Competition intensifies: [[Tesla]] Optimus targeting <$20K, [[AGIBOT]] scaling
- Export restrictions if US-China tensions escalate
- Pre-IPO financials need scrutiny (see [[Unitree Financials]])

---

*Updated 2026-02-19*

---

## Related

- [[Wang Xingxing]] — founder
- [[Unitree Cost Architecture]] — BOM, actuator design, cost moat
- [[Unitree Quadruped Bridge]] — locomotion transfer, timeline advantage
- [[Unitree GTM Strategy]] — consumer-first disruption, enterprise migration
- [[Unitree Financials]] — revenue, margins, IPO valuation
- [[Hangzhou Robotics Cluster]] — Six Little Dragons ecosystem
- [[Pre-IPO tutoring]] — completed Nov 2025 (4 months, CITIC Securities)
- [[STAR Market]] — target listing venue (mid-2026)
- [[2026 Spring Festival Gala]] — "Wu BOT" performance
- [[Boston Dynamics]] — high-cost incumbent
- [[Figure AI]] — US humanoid competitor
- [[Tesla]] — Optimus as scale-based threat
- [[AGIBOT]] — Chinese competitor
- [[MagicLab]] — gala co-performer
- [[Galbot]] — gala co-performer
- [[Noetix]] — gala co-performer
- [[UnifoLM]] — embodied AI model family (WMA, VLA, X1-0)
- [[Robotics]] — sector overview

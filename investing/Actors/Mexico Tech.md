---
aliases: [Nearshoring [[Mexico]]]
---
#actor #sovereign #ai #semiconductor #mexico #nearshoring

**[[Mexico]] Tech** — Nearshoring winner. $9B+ data center investment. US-[[China]] decoupling beneficiary.

---

## Why [[Mexico]] matters

Nearshoring boom:

| Metric | Value |
|--------|-------|
| DC investment (4yr) | $9.2B direct |
| Indirect investment | $27.6B projected |
| New data centers | 73 planned |
| Semiconductor imports | $23.5B (2024) |

---

## Hyperscaler investments

**Major commitments:**

| Company | Investment |
|---------|------------|
| Amazon/AWS | $5B cloud region |
| CloudHQ | $4.8B (Querétaro) |
| Microsoft | $1.3B AI/cloud |
| Google | Expanding |

---

## Querétaro = [[Mexico]]'s data hub

**Concentration:**
- 65% of [[Mexico]]'s DC capacity
- Hyperscale projects
- AWS, Microsoft, Google
- Infrastructure buildout

[[Mexico]]'s Northern Virginia.

---

## Nearshoring thesis

**US-[[China]] decoupling:**
- Manufacturing leaving [[China]]
- [[Mexico]] = [[USMCA]] access
- Proximity to US
- Time zone alignment
- Carlos Slim bullish

---

## Semiconductor opportunity

**Plan [[Mexico]] (Jan 2025):**
- Sheinbaum administration decree
- Incentives for chip nearshoring
- $23.5B import demand
- Domestic production push

---

## IT investment growth

**2024-2027 trajectory:**

| Metric | Value |
|--------|-------|
| 2024 IT growth | +8% |
| 2025-27 private sector | +11%/year |
| AI share of growth | 47% |
| PaaS contribution | 32% |

---

## Key players

| Company | Role |
|---------|------|
| CloudHQ | Largest DC investment |
| AWS | Cloud region |
| Microsoft | AI/cloud |
| [[QSM Semiconductor]] | First chip plant |

---

## Challenges

| Challenge | Impact |
|-----------|--------|
| Energy reliability | 75% of DC costs |
| Grid interconnection | Delays in Querétaro |
| Talent pipeline | Engineering shortage |
| Infrastructure gaps | Roads, ports |

---

## Carlos Slim's view

**Bullish on nearshoring:**
- US-[[China]] vacuum
- Semiconductor opportunity
- Regional supply hub
- Strategic positioning

[[Mexico]]'s richest man is betting big.

---

## Investment exposure

| Company | [[Mexico]] exposure |
|---------|-----------------|
| [[Amazon]] | $5B AWS investment |
| [[Microsoft]] | $1.3B+ |
| [[Google]] | Expanding |
| Industrial [[REITs]] | Warehouses, DCs |

---

## Quick stats

| Metric | Value |
|--------|-------|
| DC investment | $9.2B (direct) |
| New DCs planned | 73 |
| AWS investment | $5B |
| Chip imports | $23.5B |

*Updated 2026-01-01*

---

## Related

- [[Amazon]] — investor ($5B AWS region)
- [[Microsoft]] — investor ($1.3B AI/cloud)
- [[Google]] — investor (expanding)
- [[CloudHQ]] — major DC developer ($4.8B Querétaro)
- [[Querétaro]] — data hub (65% of [[Mexico]] DC capacity)
- [[Brazil AI]] — regional peer (LatAm)
- [[Nearshoring thesis]] — context


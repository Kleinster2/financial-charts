---
aliases: []
---
#actor #company #cybersecurity #usa #acquired

**Symantec** — Historic cybersecurity company. Enterprise business acquired by [[Broadcom]] for $10.7B (2019). Consumer business became NortonLifeLock (now Gen Digital).

---

## Overview

| Metric | Value |
|--------|-------|
| Founded | 1982 |
| HQ | Mountain View, CA |
| Peak status | World's largest cybersecurity company |
| Fate | Split (2019) |

---

## 2019 breakup

| Part | Acquirer | Value | Now |
|------|----------|-------|-----|
| Enterprise security | [[Broadcom]] | $10.7B | Broadcom Symantec division |
| Consumer (Norton) | Spun off | — | Gen Digital (NLOK → GEN) |

**Enterprise revenue (FY19):** $2.3B (~50% of total)

---

## Enterprise products (now Broadcom)

| Product | Category |
|---------|----------|
| Endpoint security | EDR, antivirus |
| Web security | Proxy, filtering |
| Cloud security | CASB |
| Data loss prevention | DLP |
| Email security | Gateway |

---

## Key acquisitions (pre-Broadcom)

| Year | Company | Focus | Value |
|------|---------|-------|-------|
| 2019 | [[Luminate Security]] | Zero trust | ~$250M |
| 2017 | Fireglass | Web isolation | — |
| 2016 | Blue Coat | Web security | $4.7B |
| 2014 | Lifelock | Identity theft | $2.3B |

---

## Broadcom integration

Under Broadcom's Symantec Enterprise division:
- Led by Art Gilliland (SVP/GM)
- Part of Broadcom's software strategy (alongside CA, VMware)
- Cost-cutting focus under Hock Tan model

---

## Consumer business (Gen Digital)

| Metric | Value |
|--------|-------|
| Former name | NortonLifeLock |
| Ticker | GEN (NASDAQ) |
| Products | Norton, Avira, LifeLock, Avast |
| Revenue | ~$4B |

---

*Created 2026-01-17*

---

## Related

- [[Broadcom]] — acquirer (enterprise, $10.7B)
- [[Luminate Security]] — acquisition (zero trust)
- [[Torq]] — founder Ofer Smadari from Luminate
- [[Cybersecurity]] — sector

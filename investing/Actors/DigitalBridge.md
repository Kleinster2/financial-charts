---
aliases: [DBRG]
---
#actor #finance #infrastructure #usa

**DigitalBridge** — Pure-play digital infrastructure asset manager. $108B AUM. **Being acquired by [[SoftBank]] for $4B (Dec 2025).**

![[dbrg-price-chart.png]]
*DBRG (blue) massively underperformed diversified peers through 2024, then spiked on SoftBank takeout news (Dec 2025). Colony Capital legacy overhang finally resolved. vs [[Blackstone|BX]] · [[Brookfield|BAM]] · [[Blue Owl|OWL]]*

![[dbrg-fundamentals.png]]
*Asset manager economics: Revenue swings with carried interest realizations and asset sales. Q2 2023 loss from Wafra settlement. Q4 2023 gain from disposition. Revenue declining as DBRG transitions to pure fee-based model.*

---

## SoftBank acquisition (Dec 2025)

| Detail | Value |
|--------|-------|
| Acquirer | [[SoftBank]] |
| Price | **$4B** |
| Market cap (Jan 2026) | **$2.8B** |
| Premium | **43%** over market cap |
| 2025 stock performance | **+35%** YTD |
| Expected close | H2 2026 |
| Post-acquisition | Operates as separate platform, Marc Ganzi continues as CEO |

**Strategic rationale:**
- SoftBank needs DC infrastructure for AI ambitions
- DigitalBridge's Vantage building Stargate Wisconsin (~1GW)
- Deep LP relationships — SoftBank (proven capital raiser) + DigitalBridge investor network = fundraising synergies

**SoftBank also:** In talks to acquire Switch separately at ~$50B. Sold entire [[NVIDIA]] stake ($5.83B) to fund AI infrastructure.

**Why only $4B for $108B AUM?** Asset manager math — DBRG doesn't own the $108B, they manage it for LPs:

| Metric | Value |
|--------|-------|
| AUM | $108B (LP money, not DBRG's) |
| FEEUM | $40.7B (fee-earning portion) |
| Fee revenue | ~$400M/year (~1% mgmt fee) |
| $4B deal | **~10x fee revenue** |

10x fee revenue is reasonable for an asset manager. [[Blackstone]] trades at 15-20x but has diversified strategies, longer track record, higher carry realization, and a working stock price. DBRG was down 50%+ pre-deal — Colony Capital legacy overhang. The 65% premium to pre-leak price tells you how broken the stock was.

SoftBank is buying the **fee stream + GP economics** (management fees + 20% carried interest on future appreciation), not the underlying assets.

---

## Why DigitalBridge matters

DigitalBridge is the only large pure-play digital infrastructure investor:

| Metric | Value |
|--------|-------|
| AUM | **$108B** (Sept 2025) |
| DC capacity | **5.4GW** in development/operation |
| Focus | 100% digital infrastructure |
| Segments | Data centers, towers, fiber, edge |
| Structure | Asset manager (fee income) |

---

## Pure-play advantage

**Unlike diversified competitors:**
- [[Blackstone]]: Data centers are subset of $1T+ AUM
- [[Brookfield]]: DC part of broader infrastructure
- [[Blue Owl]]: Digital infra is one strategy

**DigitalBridge:** 100% focused on digital infrastructure. Specialized expertise.

---

## Portfolio companies

| Company | Segment | Notes |
|---------|---------|-------|
| Vantage [[Data Centers]] | Hyperscale DC | Building Stargate Wisconsin (~1GW) |
| DataBank | Edge/enterprise DC | — |
| [[Switch]] | Hyperscale DC | **SoftBank in separate talks at ~$50B** (acquired 2022 for $11B = 4.5x) |
| Scala [[Data Centers]] | Latin America DC | — |
| AtlasEdge | Edge DC | — |
| AIMS | [[Asia]]-Pacific DC | — |
| JTOWER | [[Japan]] towers | Acquired 76% stake for $466M (Oct 2024), taking private |
| Yondr Group | Hyperscale DC | Acquired Oct 2024, 420MW capacity, 1GW planned |
| Various tower cos | Cell towers | — |
| Fiber assets | Connectivity | — |

---

## Data center strategy

**Focus areas:**
- Hyperscale build-to-suit
- Edge data centers
- International expansion
- AI-ready facilities

**Differentiation:**
- Operator expertise (not just financing)
- Global platform
- Full stack (power, cooling, connectivity)

---

## Colony Capital transformation

| Year | Event |
|------|-------|
| 1991 | [[Thomas Barrack]] founds Colony Capital — opportunistic real estate |
| 2013 | Marc Ganzi founds Digital Bridge Holdings (towers, then DC) |
| 2019 | Colony acquires Digital Bridge Holdings for $325M |
| 2020 | Ganzi replaces Barrack as CEO, begins asset rotation |
| 2021 | Rebrands to DigitalBridge, completes pivot to pure-play |
| 2022 | Acquires [[Switch]] for $11B |
| 2025 | SoftBank acquires DBRG for $4B |

**Ganzi rotated $73B of assets** — sold $40B legacy real estate (hotels, offices, healthcare, retail), built $33B digital holdings.

---

#### CEO: [[Marc Ganzi]]

**30+ years** in digital infrastructure. Operator-turned-investor.

| Year | Role |
|------|------|
| 1993 | Wharton BS |
| 2000 | Co-founded Apex Site Management (wireless sites) → merged with SpectraSite |
| 2003 | Founded Global Tower Partners |
| 2013 | **Sold GTP to [[American Tower]] for $4.8B** |
| 2013 | Founded Digital Bridge Holdings |
| 2019 | Merged into Colony Capital |
| 2020 | CEO of combined company |

Builder who has exited to strategics before. Knows the playbook.

---

## Investment case

**Bull:**
- Pure-play = levered to digital growth
- AI demand tailwind
- Specialized expertise vs generalists
- Public stock (DBRG)
- Management fee stability

**Bear:**
- Smaller than Blackstone/Brookfield
- Less balance sheet firepower
- DC oversupply risk
- Interest rate sensitivity

---

## Financials

| Year | Revenue | Net Income | Notes |
|------|---------|------------|-------|
| 2024 | $607M | $71M | Transition to pure fee model |
| 2023 | $821M | $65M | Wafra settlement loss offset by dispositions |
| 2022 | $695M | — | Switch acquisition ($11B) |
| 2021 | $388M | — | Digital pivot complete, Colony renamed |
| 2020 | $416M | — | Ganzi becomes CEO, asset rotation begins |
| 2019 | $61M | — | Colony acquires Digital Bridge Holdings |

*Asset manager revenue = management fees + carried interest + investment income. Volatile by nature — swings with realizations and mark-to-market. Fee-related earnings (FRE) is the stable metric.*

---

## Leadership

| Role | Name | Background |
|------|------|------------|
| CEO | Marc Ganzi | Founder Digital Bridge Holdings (2013). Sold Global Tower Partners to [[American Tower]] for $4.8B (2013). Colony CEO since 2020 |
| CFO | Jacky Wu | Previously Vista Equity Partners |
| President | Ben Jenkins | Joined 2021, previously Macquarie Infrastructure |

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | DBRG |
| AUM | **$108B** (Sept 2025) |
| FEEUM | **$40.7B** (Q3 2025, ahead of target) |
| DC capacity | **5.4GW** built/under construction |
| Power bank | **20GW** secured |
| HQ | Boca Raton, FL |
| CEO | Marc Ganzi |
| Status | **Being acquired by SoftBank ($16/share, ~$4B)** |

*Updated 2026-01-29*

---

## Related

- [[SoftBank]] — acquirer ($4B, Dec 2025)
- [[Masayoshi Son]] — SoftBank CEO driving acquisition
- [[Project Stargate]] — Vantage building Wisconsin DC
- [[Thomas Barrack]] — Colony Capital founder, replaced by Ganzi 2020
- [[American Tower]] — bought Ganzi's Global Tower Partners for $4.8B (2013)
- [[Switch]] — portfolio company (acquired 2022 for $11B, SoftBank in talks for ~$50B)
- [[Scala Data Centers]] — portfolio company (LatAm)
- [[Blue Owl]] — competitor (AI infrastructure financing)
- [[Blackstone]] — competitor ([[QTS]], diversified)
- [[Brookfield]] — competitor (infrastructure, diversified)
- [[Equinix]] — DC operator (investee type)
- [[Digital Realty]] — DC operator (investee type)
- [[AI infrastructure financing]] — thesis context
- [[Digital infrastructure]] — asset class overview
- [[Alternative asset manager valuation]] — why $4B for $108B AUM

#actor #fintech #private-markets #usa

**Republic** — Private investment platform. Equity crowdfunding, tokenized assets. Launched [[SpaceX]] token for retail investors.

---

## Why Republic matters

Republic bridges retail investors to private markets:

| Offering | Description |
|----------|-------------|
| Equity crowdfunding | Invest in startups from $50 |
| Tokenized assets | [[SpaceX]] token, real estate |
| Secondary markets | [[Trade]] private shares |
| Reg A+ offerings | Larger private raises |

---

## [[SpaceX]] token

**Flagship tokenized offering:**
- Token mirrors performance of [[SpaceX]] private shares
- Retail can gain exposure without accredited status
- Not actual [[SpaceX]] equity — derivative instrument
- [[SpaceX]] did not issue or endorse

**Structure:**
- Republic acquires [[SpaceX]] shares
- Issues tokens representing exposure
- Tokens tradeable on Republic platform
- Should converge at liquidity event (IPO)

See [[Tokenized private shares]] for risks and context.

---

## Business model

| Revenue stream | Description |
|----------------|-------------|
| Platform fees | % of capital raised |
| Token issuance | Fees on tokenized assets |
| Secondary trading | Transaction fees |
| Republic Note | Investment in Republic itself |

---

## Regulatory framework

**Using existing exemptions:**
- Reg CF (Crowdfunding) — up to $5M raises
- Reg A+ — up to $75M raises
- Reg D — accredited investors
- Tokenization — regulatory gray area

**Risk:** Regulations haven't caught up with innovation.

---

## Cap table / Investors

| Round | Date | Amount | Valuation |
|-------|------|--------|-----------|
| Seed-Series A | 2016-2019 | $20M+ | — |
| Series B | 2021 | $150M | $800M+ |
| **Total** | | **$170M+** | |

**Key investors:**

| Investor | Notes |
|----------|-------|
| Galaxy Digital | Lead (Series B) |
| [[Valor Equity Partners]] | Growth |
| Tribe Capital | Growth |
| CoinFund | [[Crypto]] focus |

**Total raised:** $170M+

## Platform stats

| Metric | Value |
|--------|-------|
| Status | Private |
| Users | [[3M]]+ |
| Capital deployed | $2B+ |
| Offerings | 1,500+ |
| Founded | 2016 |
| HQ | New York |

---

## Investment case

**Bull:**
- First-mover in tokenized private assets
- Growing demand for private market access
- Regulatory tailwinds (SEC modernization)
- Platform network effects
- Real estate + startup diversification

**Bear:**
- Regulatory uncertainty
- Tokenized assets untested legally
- Adverse selection (best companies don't need retail)
- Illiquidity risk
- Company is private (can't invest in Republic itself easily)

---

## Competitive landscape

| Competitor | Focus |
|------------|-------|
| [[Robinhood]] | Brokerage + tokenized shares |
| [[AngelList]] | Accredited investor syndicates |
| Wefunder | Equity crowdfunding |
| StartEngine | Reg CF/A+ offerings |
| Forge | Secondary private shares |

Republic = broadest retail private markets access.

---

## Key differentiator

**Republic vs Robinhood on tokenization:**

| Aspect | Republic | Robinhood |
|--------|----------|-----------|
| Focus | Private markets native | Brokerage adding tokens |
| [[SpaceX]] | Token launched | Token launched |
| OpenAI | Not yet | [[Europe]] (June 2025) |
| User base | Private market seekers | Retail traders |
| Regulation | Reg CF/A+ expertise | Brokerage license |

---

## Related

- [[Tokenized private shares]] — core business
- [[Robinhood]] — competitor/peer
- [[OpenAI]] — potential future token
- [[Private markets]] — industry context


#etf #commodity #broad

**iShares S&P GSCI Commodity-Indexed Trust** — tracks the S&P GSCI, the oldest and most widely-followed commodity index.

| Metric | Value |
|--------|-------|
| AUM | $1B |
| Expense ratio | 0.75% |
| Exposure | S&P GSCI futures |
| Inception | Jul 2006 |

---

## Sector correlation

| Sector | ETF | Correlation |
|--------|-----|-------------|
| Oil & Gas E&P | XOP | 0.72 |
| [[Energy and Utilities\|Energy]] | XLE | 0.70 |
| Technology | XLK | 0.44 |
| *S&P 500* | *SPY* | *0.46* |

GSG trades as a core Oil & Gas E&P name (XOP r = 0.72).

---

## Composition

Very energy-heavy: ~60% energy (WTI dominant), ~15% agriculture, ~15% industrial metals, ~10% precious metals and livestock. More energy-weighted than [[DBC]].

## Use Case

Alternative [[Commodities]] benchmark. GSG vs [[DBC]] in the dashboard — GSG is more energy-tilted, so divergence signals energy vs non-energy commodity dynamics.

## Related

- [[Commodities]] — sector
- [[DBC]] — Invesco broad commodity ETF
- [[BlackRock]] — issuer

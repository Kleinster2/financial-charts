---
aliases: [MARA, [[Marathon]]]
---
#actor #crypto #usa #public

**[[Marathon]] Digital (MARA)** — Bitcoin pure-play. Underperforming AI-pivot peers. BTC holdings focus.

---

## Why [[Marathon]] matters

Largest Bitcoin miner staying Bitcoin-focused:

| Metric | Value |
|--------|-------|
| Ticker | MARA |
| Strategy | Bitcoin pure-play |
| 2025 performance | Underperforming |
| Approach | BTC accumulation |

---

## Bitcoin-first strategy

**Not pivoting (yet):**
- Focus on BTC mining
- Accumulating Bitcoin on balance sheet
- Not chasing AI deals
- Betting on Bitcoin appreciation

---

## Underperformance

**Lagging AI-pivot peers:**

| Company | 2025 YTD | Strategy |
|---------|----------|----------|
| [[IREN]] | +300% | AI pivot |
| [[Cipher Mining]] | +230% | AI pivot |
| [[Hut 8]] | +139% | AI pivot |
| **[[Marathon]]** | Lagging | Bitcoin focus |

Market rewarding AI pivots, not BTC holdings.

---

## The debate

**Bull case for pure-play:**
- Bitcoin could outperform
- Simpler business model
- No AI execution risk
- BTC holdings = option value

**Bear case:**
- Missing AI opportunity
- Lower revenue per MW
- Peers getting hyperscaler contracts
- Market values AI infrastructure

---

## Comparison to Bitdeer

| Company | Strategy | Performance |
|---------|----------|-------------|
| **[[Marathon]]** | BTC pure-play | Lagging |
| [[Bitdeer]] | ASIC focus | -50% |

Both underperforming AI-focused peers.

---

## Investment case

**Bull:**
- Largest Bitcoin miner
- BTC on balance sheet
- If Bitcoin rallies, wins
- Optionality to pivot later

**Bear:**
- Missing $38B AI opportunity
- Peers signing $B+ deals
- Market rewarding AI pivot
- Pure-play out of favor

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | MARA |
| Strategy | Bitcoin pure-play |
| 2025 | Underperforming |
| BTC holdings | Significant |

*Updated 2026-01-01*

---

## Related

- [[IREN]] — competitor (outperforming via AI pivot)
- [[Hut 8]] — competitor (outperforming via AI pivot)
- [[Bitdeer]] — peer (also underperforming)
- [[Riot Platforms]] — peer (crypto-focused)
- [[Crypto-to-AI pivot]] — thesis context (not pivoting)

---
aliases: [ADI]
---
#actor #semiconductor #analog #usa #public

**Analog Devices** — #2 analog chipmaker globally. Mixed-signal leader. Industrial, auto, communications exposure.

---

## Why Analog Devices matters

| Metric | Value |
|--------|-------|
| Ticker | ADI (NASDAQ) |
| Market cap | ~$149B |
| Position | #2 analog (after Texas Instruments) |
| FY2025 revenue | $11.0B |

---

## Price vs peers

![[adi-vs-peers.png]]

ADI has significantly outperformed analog peers over 7 years, driven by premium positioning in high-performance mixed-signal. The Maxim acquisition (2021) created a step-change in scale.

---

## 10-year financials

| FY | Revenue ($B) | Net Income ($B) | Margin |
|----|-------------|-----------------|--------|
| 2025 | 11.02 | 2.27 | 20.6% |
| 2024 | 9.43 | 1.64 | 17.4% |
| 2023 | 12.31 | 3.32 | 27.0% |
| 2022 | 12.01 | 2.75 | 22.9% |
| 2021 | 7.32 | 1.39 | 19.0% |
| 2020 | 5.60 | 1.22 | 21.8% |
| 2019 | 5.99 | 1.36 | 22.7% |
| 2018 | 6.23 | 1.50 | 24.1% |
| 2017 | 5.25 | 0.81 | 15.4% |
| 2016 | 3.42 | 0.86 | 25.1% |
| 2015 | 3.44 | 0.70 | 20.3% |

**Key observations:**
- Revenue 3.2x growth over 10 years (CAGR ~12%)
- 2021-2022 jump reflects Maxim acquisition ($21B, closed Aug 2021)
- FY2023 peak margins compressed in FY2024 cyclical downturn
- FY2025 recovery underway with 17% revenue growth YoY

---

## FY2025 results

**Q4 FY2025:**

| Metric | Value | YoY |
|--------|-------|-----|
| Revenue | $3.08B | +26% |
| EPS (adj) | $2.26 | +35% |
| Operating cash flow | $4.8B (FY) | — |
| [[Free cash flow]] | $4.3B (FY) | 39% margin |

Beat estimates on both revenue and EPS.

---

## FY2026 outlook

**Q1 FY2026 guidance:**

| Metric | Guidance |
|--------|----------|
| Revenue | $3.1B ± $100M |
| Operating margin (adj) | ~43.5% |
| EPS (adj) | $2.29 ± $0.10 |

Guidance 4.4% above consensus — cycle recovery underway.

---

## Price increases (Feb 2026)

**Announced hikes:**

| Category | Increase |
|----------|----------|
| Commercial-grade | 10-15% |
| Industrial-grade | ~15% |
| Military-spec | Up to 30% |

Following Texas Instruments lead — industry pricing power.

---

## End markets

| Market | Exposure | Trend |
|--------|----------|-------|
| Industrial | ~50% | Recovering |
| [[Automotive]] | ~25% | EV/ADAS growth |
| Communications | ~15% | [[5G]], data center |
| [[Consumer]] | ~10% | Stable |

Industrial automation + automotive = key drivers.

---

## Competitive position

| vs | ADI strength | ADI weakness |
|----|--------------|--------------|
| Texas Instruments | Higher-end mixed-signal | Scale |
| NXP | Analog breadth | Auto focus |
| Infineon | Industrial/comms | Power semis |

ADI = premium analog, higher margins.

---

## Investment case

**Bull:**
- Cyclical recovery underway
- Pricing power (10-30% hikes)
- Industrial automation tailwind
- EV/ADAS content growth
- 39% FCF margin

**Bear:**
- Cyclical exposure
- Industrial slowdown risk
- [[China]] revenue exposure
- Competition from TI
- Valuation elevated

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | ADI |
| Market cap | ~$149B |
| FY2025 revenue | $11.0B |
| FCF margin | 39% |
| Position | #2 analog |

*Updated 2026-01-28*

---

## Related

- [[Texas Instruments]] — competitor (#1 analog)
- [[NXP]] — competitor (auto focus)
- [[Infineon]] — competitor (power)
- [[Automotive semiconductors]] — end market
- [[Renesas]] — competitor (MCUs)

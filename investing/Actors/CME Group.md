---
aliases: [CME, Chicago Mercantile Exchange, CBOT, NYMEX, COMEX]
---
#actor #finance #exchange #derivatives #usa #public

**CME Group** — World's largest derivatives exchange. Futures, options on rates, commodities, equities.

---

## Why CME matters

Derivatives monopoly:

| Metric | Value |
|--------|-------|
| Market cap | ~$85B |
| Ticker | CME |
| Daily volume | 25M+ contracts |
| CEO | Terry Duffy |
| Founded | 1898 (CBOT) |

---

## Exchange portfolio

| Exchange | Products |
|----------|----------|
| **CME** | Rates, FX, equities |
| **CBOT** | Grains, Treasuries |
| **NYMEX** | Energy (WTI crude) |
| **COMEX** | Metals (gold, silver, copper) |

Owns the benchmarks.

---

## Key products

| Product | Significance |
|---------|--------------|
| **Eurodollar/SOFR futures** | Interest rate benchmark |
| **E-mini S&P 500** | Most liquid equity futures |
| **WTI Crude** | US oil benchmark |
| **Gold futures** | COMEX = price discovery |
| **Treasury futures** | Rate hedging |

---

## Business model

**Transaction fees + data:**
- Per-contract fees
- Clearing fees
- Market data licensing
- Colocation services
- 85%+ operating margins

Volume × volatility = revenue.

---

## Volatility correlation

| Market condition | CME impact |
|------------------|------------|
| High volatility | Revenue surge |
| Rate uncertainty | Treasury volume up |
| Commodity swings | Energy/metals volume up |
| Low volatility | Volume decline |

CME benefits from uncertainty.

---

## Investment case

**Bull:**
- Monopoly on key benchmarks
- Volatility = tailwind
- High margins (85%+)
- Network effects
- Rate uncertainty persists

**Bear:**
- Volume-dependent
- Low volatility risk
- Crypto competition
- Regulatory scrutiny

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | CME |
| Market cap | ~$85B |
| Key products | Rates, commodities |
| Sector | Financial infrastructure |

*Created 2026-01-08*

---

## Related

- [[Intercontinental Exchange]] — competitor (ICE Futures)
- [[Nasdaq]] — peer (equities)
- [[CBOE]] — competitor (options, VIX)
- [[OPEC]] — CME trades oil futures


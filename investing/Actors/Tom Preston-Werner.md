# Tom Preston-Werner

#actor #individual #founder #investor #developer-tools

**Tom Preston-Werner** — GitHub co-founder (2008), sold to Microsoft. Creator of Jekyll, Semantic Versioning (SemVer), TOML. Now runs Preston-Werner Ventures — 175+ investments including early checks in Stripe, Cursor, [[Supabase]], [[PlanetScale]], Netlify.

---

## Key facts

| Detail | Value |
|--------|-------|
| Known for | GitHub co-founder |
| Current role | GP, Preston-Werner Ventures |
| Investments | 175+ startups |
| Notable creations | Jekyll, SemVer, TOML |

---

## Career

### GitHub (2008-2014)

| Detail | Value |
|--------|-------|
| Role | Co-founder, CEO |
| Co-founders | Chris Wanstrath, P.J. Hyett |
| Founded | 2008 |
| Departed | 2014 |
| Acquisition | Microsoft ($7.5B, 2018) |

**Departure:** Resigned in 2014 following internal investigation. Sold shares to Microsoft in 2018 acquisition.

### Pre-GitHub

| Company | Role | Outcome |
|---------|------|---------|
| Gravatar | Founder (2004) | Sold to Automattic (2007) |
| Powerset | Employee | Acquired by Microsoft |

Declined $300K Microsoft bonus + stock options to focus on GitHub.

### Technical contributions

| Creation | Impact |
|----------|--------|
| **Jekyll** | Static site generator, powers millions of sites |
| **Semantic Versioning (SemVer)** | Universal standard for software versions |
| **TOML** | Configuration format used across industries |

---

## Preston-Werner Ventures

Founded 2019 with wife Theresa and partners David Price, David Thyresson.

### Investment track record

| Category | Examples |
|----------|----------|
| **Early wins** | Stripe, Cursor |
| **Developer tools** | [[Supabase]], [[PlanetScale]], Netlify, Retool, Snyk |
| **Total investments** | 175+ |

### Fund details

| Detail | Value |
|--------|-------|
| Founded | 2019 |
| Target | $100M fund (reported) |
| Focus | Early-stage tech startups |
| Board seats | Netlify (since 2020) |

---

## RedwoodJS

Currently leading development of **RedwoodJS** — full-stack JavaScript framework:

| Detail | Value |
|--------|-------|
| Type | Open source framework |
| Focus | Full-stack React apps |
| Philosophy | Opinionated, batteries-included |

Reflects continued commitment to developer tools and open source.

---

## Related

- [[Supabase]] — angel investor
- [[PlanetScale]] — angel investor
- [[Vercel]] — peer in developer tools ecosystem
- [[Nat Friedman]] — successor as GitHub leader (CEO 2018-2021)
- Microsoft — acquired GitHub

---

Sources:
- [Wikipedia - Tom Preston-Werner](https://en.wikipedia.org/wiki/Tom_Preston-Werner)
- [Preston-Werner Ventures](https://pwv.com/)
- [PitchBook - PWV fund](https://pitchbook.com/news/articles/github-co-founder-raising-100m-fund-early-stage-tech-startups)

*Created 2026-01-14*

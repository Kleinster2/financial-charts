---
aliases: []
---
#actor #company #socialmedia #gaming #usa #private

**Discord** — Community platform. 200M+ MAUs. [[Gaming]] roots, expanding. ~$15B valuation.

---

## Why Discord matters

| Metric | Value |
|--------|-------|
| Valuation | ~$15B |
| MAUs | 200M+ |
| Revenue | ~$600M |
| HQ | San Francisco |
| Status | Private |

De facto community platform for internet culture.

---

## Product

| Feature | Description |
|---------|-------------|
| **Servers** | Community spaces |
| **Voice** | Low-latency chat |
| **Text** | Channels, threads |
| **Video** | Screen share, streaming |
| **Stage** | Audio events |

---

## Business model

| Revenue | Source |
|---------|--------|
| **Nitro** | Premium subscriptions |
| **Server Boosts** | Community features |
| **Apps** | Developer platform |

No ads (differentiator).

---

## User base

| Segment | Presence |
|---------|----------|
| [[Gaming]] | Origin, still core |
| [[Crypto]]/Web3 | Major communities |
| Education | Study groups |
| Creators | Fan communities |
| Brands | Customer communities |

Expanded beyond gaming.

---

## Acquisition history

| Year | Suitor | Outcome |
|------|--------|---------|
| 2021 | [[Microsoft]] | Rejected $12B offer |
| 2022 | Various | IPO speculation |

Chose independence.

---

## Competition

| Platform | Overlap |
|----------|---------|
| Slack | Work communication |
| Teams | Enterprise |
| Telegram | Communities |
| [[Reddit]] | Async communities |

Differentiated by real-time + voice.

---

## Investment case (if IPO)

**Bull:**
- Community moat
- No-ads differentiation
- Platform expansion
- Cultural relevance

**Bear:**
- Monetization limits
- Moderation challenges
- Enterprise skepticism
- Valuation expectations

---

## Quick stats

| Metric | Value |
|--------|-------|
| Valuation | ~$15B |
| MAUs | 200M+ |
| Status | Private |
| Focus | Community platform |

---

## Related

- [[Roblox]] — gaming peer
- [[Snap]] — youth social
- [[Media]] — sector
- [[Slack]] — enterprise competitor
- [[Teams]] — enterprise competitor
- [[Telegram]] — community competitor
- [[Clawdbot]] — integrates with Discord

---

*Created 2026-01-16*

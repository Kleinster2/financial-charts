---
aliases: [SMCI, Supermicro]
---
#actor #datacenter #servers #cooling #usa #public

**Super Micro Computer** — AI server leader. Liquid cooling pioneer. Accounting concerns. Volatile.

---

## Why Supermicro matters

#1 AI server company (with drama):

| Metric | Value |
|--------|-------|
| Ticker | SMCI |
| Market cap | ~$20B (volatile) |
| Revenue | ~$15B (FY24) |
| Growth | 100%+ |
| Issue | Accounting concerns |

---

## AI server dominance

**NVIDIA's best friend:**
- First to market with H100 racks
- Liquid cooling integrated
- Direct-to-chip solutions
- NVIDIA partnership

Gets AI servers out fastest.

---

## Liquid cooling lead

**Why Supermicro leads:**

| Advantage | Details |
|-----------|---------|
| First mover | Liquid-cooled racks early |
| Integration | Full rack solutions |
| Density | More GPUs per rack |
| Efficiency | PUE improvements |

Liquid cooling = competitive moat.

---

## Product portfolio

| Product | Application |
|---------|-------------|
| GPU servers | AI training/inference |
| Liquid cooling | AI thermal management |
| Storage | High-density |
| Edge | 5G, IoT |

---

## Accounting controversy

**2024 issues:**
- Short seller report (Hindenburg)
- Delayed 10-K filing
- Auditor resigned (EY)
- DOJ inquiry reported
- Stock volatile

---

## The bull case

**Despite controversy:**
- Real products, real revenue
- NVIDIA relationship intact
- AI demand undeniable
- Liquid cooling differentiation
- Cheap if concerns overblown

---

## The bear case

**Serious concerns:**
- Accounting credibility
- Auditor departure = red flag
- DOJ inquiry
- Delisting risk
- Governance questions

---

## Competition

| Competitor | Supermicro vs |
|------------|---------------|
| Dell | Faster to market |
| HPE | More innovative |
| Lenovo | Liquid cooling lead |
| Inspur (China) | US alternative |

---

## Investment case

**Bull:**
- AI server leader
- Liquid cooling moat
- Cheap on accounting fears
- NVIDIA partnership
- Demand undeniable

**Bear:**
- Accounting red flags
- Auditor resignation
- Potential fraud
- Delisting risk
- Governance concerns

High risk/high reward.

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | SMCI |
| Market cap | ~$20B (volatile) |
| Growth | 100%+ |
| Issue | Accounting concerns |

*Updated 2026-01-01*

---

## Related

- [[NVIDIA]] — partner (first to market with H100 racks)
- [[Dell]] — competitor (server market)
- [[Vertiv]] — supplier (DC infrastructure)
- [[CoreWeave]] — customer type (GPU cloud)


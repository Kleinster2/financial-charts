---
aliases: [BLDP, Ballard Power]
---
#actor #energy #hydrogen #canada #public

**Ballard** — Fuel cell technology company. Focus on heavy-duty transport (buses, trucks, trains, marine). Canadian, more focused than Plug Power.

---

## Why Ballard matters

Ballard is the focused fuel cell play:

| Metric | Value |
|--------|-------|
| Focus | Heavy-duty mobility fuel cells |
| Markets | Bus, truck, rail, marine |
| Geography | Global ([[China]], [[Europe]], NA) |
| Strategy | Technology + partnerships |

---

## Business model

| Segment | Description |
|---------|-------------|
| Heavy-duty motive | Buses, trucks |
| Rail | Hydrogen trains |
| Marine | Ship fuel cells |
| Stationary | Backup power |

**Focus:** Transportation where batteries don't work well (heavy, long-range).

---

## [[Target]] markets

| Market | Why hydrogen |
|--------|--------------|
| Buses | City fleets, range anxiety |
| Trucks | Long-haul, heavy loads |
| Trains | Non-electrified routes |
| Ships | Marine decarbonization |

**Thesis:** [[Batteries]] for cars, hydrogen for heavy transport.

---

## [[China]] exposure

**Significant [[China]] business:**
- Weichai Power partnership
- [[China]] pushing hydrogen buses
- Joint ventures
- Risk: Geopolitics, competition

---

## AI/Data center relevance

**Limited:**
- Heavy transport focus, not stationary
- Some backup power applications
- Not core to data center story

---

## Financial position

| Metric | Status |
|--------|--------|
| Revenue growth | Slow |
| Profitability | Not yet |
| Cash position | Better than Plug Power |
| Burn rate | Manageable |

**Better than Plug:** More focused, less cash burn.

---

## Investment case

**Bull:**
- Focused strategy (transport)
- Technology leadership
- Partnership model (asset-light)
- Heavy transport decarbonization mandate
- Better financial position than peers

**Bear:**
- Hydrogen infrastructure slow to build
- Battery trucks improving ([[Tesla]] Semi)
- [[China]] revenue risk
- Still unprofitable
- Long timeline to scale

---

## Financials

| Metric | Value |
|--------|-------|
| Ticker | BLDP (NASDAQ/TSX) |
| Market cap | ~$800M |
| Revenue (TTM) | ~$100M |
| Net income | Losses (smaller than Plug) |
| Cash | Adequate for near-term |
| HQ | Vancouver, [[Canada]] |

---

## vs Plug Power

| Aspect | Ballard | [[Plug Power]] |
|--------|---------|----------------|
| Focus | [[Transport]] | Ecosystem |
| Financial health | Better | Worse |
| Strategy | Partnerships | Vertical |
| Revenue | Smaller | Larger |
| Risk | Lower | Higher |

---

## Related

- [[Plug Power]] — peer (more distressed)
- [[Bloom Energy]] — peer (stationary)
- [[Clean energy for AI]] — context
- [[Toyota]] — hydrogen vehicle partner


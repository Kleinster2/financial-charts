---
aliases: [旷视科技, Face++, Megvii Technology]
---
#actor #china #ai #cv #public

**Megvii (Face++)** — [[China]] computer vision pioneer. Tsinghua spinoff. Face++ platform. Public (Shanghai: 688256). On US Entity List.

---

## Why Megvii matters

Early [[China]] AI success story, now facing headwinds:

| Metric | Value |
|--------|-------|
| HQ | Beijing |
| Founded | 2011 |
| Listing | Shanghai STAR: 688256 (2021) |
| Focus | Face recognition, computer vision |
| Status | US Entity List (2019) |
| Famous for | Face++ API (early viral success) |

---

## Founding story

Three Tsinghua classmates:

| Founder | Role |
|---------|------|
| Yin Qi | CEO |
| Tang Wenbin | CTO |
| Yang Mu | CPO |

**Origins:** Started as Tsinghua undergrad project. Face++ API launched 2012, went viral with developers globally.

---

## Face++ legacy

The product that put Megvii on the map:

| Aspect | Details |
|--------|---------|
| Launch | 2012 |
| Model | API-based face recognition |
| Users | [[Alipay]], Didi, [[Lenovo]], others |
| Scale | Billions of API calls |
| Impact | Proved [[China]] CV could compete globally |

**[[Alipay]] deal:** Face++ powers [[Alipay]]'s face verification. Massive scale validation.

---

## Business segments

| Segment | Description | Revenue share |
|---------|-------------|---------------|
| IoT (City) | Smart city, surveillance | ~60% |
| IoT (Supply Chain) | Logistics, robotics | ~25% |
| [[Consumer]] | Mobile, fintech | ~15% |

**Similar to [[SenseTime]]:** Heavy exposure to surveillance/smart city.

---

## Technology

| Capability | Details |
|------------|---------|
| Face recognition | Core competency |
| Body recognition | Full-body tracking |
| Text recognition | OCR, document processing |
| Brain++ | Internal AI training platform |

---

## Entity List impact

Added alongside SenseTime in 2019:

| Effect | Details |
|--------|---------|
| IPO delay | Hong Kong IPO cancelled |
| Relisting | Shanghai STAR instead |
| Chip access | [[NVIDIA]] restrictions |
| Valuation | Compressed |

---

## Competitive position

| vs Competitor | Megvii advantage | Disadvantage |
|---------------|------------------|--------------|
| [[SenseTime]] | More focused | Smaller |
| Hikvision | Software purity | Hardware integration |
| CloudWalk | First mover | Less government ties |
| Yitu | Face++ brand | Similar size |

**The "Four Dragons of CV":** Megvii, SenseTime, CloudWalk, Yitu — all Tsinghua/academic spinoffs, all Entity Listed.

---

## Financial snapshot

| Metric | Recent | Trend |
|--------|--------|-------|
| Revenue | ~$1.5B RMB | Slower growth |
| Gross margin | ~55% | Stable |
| Net income | Negative | Struggling |
| R&D spend | High | Maintaining |

**Challenges:** Growth slowing, profitability elusive.

---

## Investment implications

**Investable:** Shanghai STAR: 688256 (harder for foreign investors)

**Bull case:**
- Face++ brand recognition
- [[Alipay]] relationship
- [[Robotics]]/logistics pivot
- Depressed valuation

**Bear case:**
- Entity List overhang
- Smaller than SenseTime
- Smart city concentration
- Profitability challenges

---

*Updated 2026-01-03*

---

## Related

- [[SenseTime]] — competitor/peer
- [[China university-AI pipeline]] — origin (Tsinghua spinoff)
- [[Sinovation Ventures]] — investor
- [[Alibaba]] — customer ([[Alipay]])
- [[Export controls]] — context (Entity List)

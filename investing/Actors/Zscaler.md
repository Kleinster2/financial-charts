---
aliases: [ZS]
---
#actor #cybersecurity #usa #public

**Zscaler** — Zero trust security leader. Cloud-native architecture. Replacing legacy firewalls.

---

## Sector correlation

| Sector | ETF | Correlation |
|--------|-----|-------------|
| Software | IGV | 0.72 |
| Technology | XLK | 0.53 |
| [[Banks|Regional Banks]] | KRE | 0.52 |
| *S&P 500* | *SPY* | *0.52* |

ZS trades as a core Software name (IGV r = 0.72).

---

## Why Zscaler matters

| Metric | Value |
|--------|-------|
| Ticker | ZS (NASDAQ) |
| Market cap | ~$30B |
| Position | \#1 zero trust/SASE |
| Key product | Zero Trust Exchange |

---

## Business model

Zero Trust Exchange platform:

| Product | Function |
|---------|----------|
| ZIA | Secure Internet Access |
| ZPA | Zero Trust Network Access |
| ZDX | Digital Experience |
| Workload protection | Cloud security |

Architecture: All traffic through Zscaler cloud — no on-prem appliances.

---

## Why zero trust

Legacy model broken:
- Perimeter security assumes trusted network
- Remote work killed the perimeter
- Cloud apps bypass traditional firewalls

Zero trust model:
- Never trust, always verify
- User-to-application, not network-to-network
- Cloud-native, scalable

---

## Competitive position

| Competitor | Approach |
|------------|----------|
| Zscaler | Pure-play zero trust |
| [[Palo Alto Networks]] | Prisma SASE (platform) |
| [[Cisco]] | Umbrella (legacy vendor) |
| Netskope | CASB + SASE |

---

## Investment case

Bull:
- Zero trust adoption accelerating
- Cloud-native architecture advantage
- Large enterprise wins
- Firewall replacement cycle

Bear:
- Palo Alto platformization threat
- Growth slowing from peak
- Premium valuation
- Macro sensitivity (IT budgets)

---


![[zs-employees-chart.png]]
*Headcount: 7,923 (2025) — up 7.8% YoY*

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | ZS |
| Market cap | ~$30B |
| Founded | 2007 |
| CEO | Jay Chaudhry |

*Created 2026-01-06*

---

## Q2 FY2026 (reported Feb 26, 2026)

| Metric | Q2 FY26 | Y/Y |
|--------|---------|-----|
| Revenue | $815.8M | +26% |
| Non-GAAP EPS | $1.01 | Beat ($0.89 est.) |
| ARR | $3,359M | +25% |
| Net new ARR | $155.5M | — |
| Non-GAAP op margin | >22% | — |
| AI Security ARR | $400M+ | +80% Y/Y, hit FY26 target 3 quarters early |
| Zero Trust Everywhere | 450+ enterprises | Hit FY26 target 3 quarters early |

FY2026 guidance raised: ARR growth to 24%. Non-GAAP operating income $742-748M (28-29% growth).

Stock -10% AH despite clean beat — billings/deferred revenue metrics disappointed. In the [[February 2026 AI Disruption Cascade]] environment, cybersecurity SaaS gets no benefit of the doubt. ZS is -37% YTD despite accelerating fundamentals. The AI Security pivot is the bright spot but market questions whether zero trust becomes commoditized by AI agents.

---

## Related

- [[Palo Alto Networks]] — competitor (Prisma SASE)
- [[CrowdStrike]] — peer (endpoint complement)
- [[Cybersecurity consolidation]] — thesis context

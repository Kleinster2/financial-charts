---
aliases: [Argentine]
---
#actor #sovereign #latam #datacenter

**Argentina** — Emerging AI infrastructure destination. Stargate's first LATAM site ($25B). Milei government aggressively courting tech with RIGI incentives.

---

## Why Argentina matters for AI

| Factor | Details |
|--------|---------|
| Stargate deal | $25B data center in Patagonia |
| Government | Milei actively pro-tech, pro-US |
| Incentives | RIGI program for $200M+ investments |
| Energy | Cheap hydro/wind in Patagonia |
| Adoption | High ChatGPT usage per capita |

---

## Stargate Argentina

| Aspect | Details |
|--------|---------|
| Investment | $25B |
| Location | Patagonia |
| Capacity | 500MW (largest in LATAM) |
| Partner | Sur Energy |
| Timeline | Construction 2026, first 100MW 2027 |
| Status | Letter of Intent signed (Oct 2025) |

See [[Project Stargate]] for full context.

---

## RIGI incentive program

**Régimen de Incentivo a las Grandes Inversiones** (Large Investment Incentive Regime):

| Benefit | Details |
|---------|---------|
| Threshold | $200M+ in foreign currency |
| Import taxes | Exemptions |
| FX benefits | Favorable exchange treatment |
| Tax holidays | Corporate tax reductions |
| Duration | 30-year stability guarantee |

Designed to attract hyperscale infrastructure investment.

---

## Milei's tech positioning

| Action | Signal |
|--------|--------|
| Met with OpenAI delegation | Personal involvement |
| RIGI program launch | Structural incentives |
| Pro-US alignment | Geopolitical positioning |
| Deregulation push | Faster permitting |

Milei views AI infrastructure as economic transformation lever.

---

## Energy advantage

Patagonia offers:

| Resource | Advantage |
|----------|-----------|
| Wind | Some of world's best wind resources |
| Hydro | Existing hydroelectric capacity |
| Solar | Northern regions |
| Cost | Below US/EU electricity prices |
| Grid | Less congested than US markets |

**The pitch**: Cheap, clean power for compute-intensive AI workloads.

---

## vs Other LATAM

| Country | Data center status |
|---------|-------------------|
| **Argentina** | Stargate ($25B, 500MW) — largest |
| Brazil | Existing hyperscaler presence |
| Chile | Growing, good connectivity |
| Mexico | Nearshoring beneficiary |

Argentina leap-frogging with single mega-project.

---

## Risks

| Risk | Severity |
|------|----------|
| Political instability | Medium — Milei polarizing |
| Currency volatility | High — peso history |
| Execution | Medium — infrastructure gaps |
| Grid reliability | Medium — upgrades needed |
| Regulatory change | Low — RIGI has 30-year lock |

RIGI's 30-year stability clause designed to mitigate political risk.

---

## Investment implications

**Direct beneficiaries:**
- Sur Energy (Stargate partner)
- Argentine utilities
- Construction/infrastructure

**Indirect beneficiaries:**
- Regional connectivity providers
- LATAM cloud expansion plays

**Watch for:**
- Stargate Argentina construction milestones
- Additional hyperscaler announcements
- RIGI program uptake

---

## Related

- [[Project Stargate]] — Parent initiative
- [[OpenAI]] — Stargate anchor tenant
- [[Masayoshi Son]] — Stargate lead investor
- [[Brazil AI]] — Regional comparison
- [[Mexico Tech]] — LATAM peer

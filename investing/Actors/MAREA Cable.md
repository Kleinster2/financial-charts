---
aliases: []
---
#actor #infrastructure #submarine #cable

**MAREA Cable** — Meta + Microsoft trans-Atlantic. 210 Tbps. Virginia to Spain.

---

## Why MAREA matters

Hyperscaler alliance:

| Metric | Value |
|--------|-------|
| Route | Virginia Beach → Bilbao, Spain |
| Length | 6,600 km (4,100 miles) |
| Capacity | 210 Tbps |
| Operational | 2018 |
| Owners | Meta, Microsoft, Telxius |

---

## Ownership

| Partner | Role |
|---------|------|
| [[Meta]] | Co-owner, capacity |
| [[Microsoft]] | Co-owner, Azure traffic |
| Telxius (Telefónica) | Operator |

First major hyperscaler-owned trans-Atlantic.

---

## Technical specs

| Spec | Value |
|------|-------|
| Fiber pairs | 8 |
| Design capacity | 210 Tbps |
| Depth | Up to 5,180m |
| Technology | Open cable design |

---

## Strategic importance

**Why Meta + Microsoft built it:**
- Control over capacity
- Avoid consortium politics
- Direct US-Europe link
- Azure + Facebook traffic

---

## Quick stats

| Metric | Value |
|--------|-------|
| Route | US-Spain |
| Capacity | 210 Tbps |
| Operational | 2018 |
| Owners | Meta, Microsoft, Telxius |

*Updated 2026-01-01*

---

## Related

- [[Meta]] — co-owner
- [[Microsoft]] — co-owner (Azure traffic)
- [[Amitié Cable]] — peer (trans-Atlantic)
- [[Submarine Cables]] — infrastructure context


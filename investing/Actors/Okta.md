---
aliases: [OKTA]
---
#actor #saas #identity #security #usa #public

**Okta** — Identity and access management leader. $2.8B revenue. AI agent identity emerging. $1B buyback.

---

## Why Okta matters

| Metric | Value |
|--------|-------|
| Ticker | OKTA ([[Nasdaq]]) |
| Revenue | **$2.84B** (LTM) |
| FY26 guidance | $2.91B (+11%) |
| Subscription % | **98%** |
| FCF margin | **29%** |

---

## Products

| Product | Function |
|---------|----------|
| Workforce Identity | Employee SSO, MFA |
| Customer Identity | B2C authentication |
| Identity Governance | Access management |
| Privileged Access | Admin security |
| **Okta for AI Agents** | Non-human identity |

---

## AI agent identity opportunity

**CEO McKinnon (2025):**
> "The opportunity to be the access and identity layer for [[AI agents]] and enterprises is probably bigger than the customer and workforce identity businesses."

**Oktane 2025 announcements:**
- Identity Security Fabric (ISF)
- Okta for AI Agents — govern non-human identities
- 100+ customers already engaged

---

## Financial performance

| Metric | Value |
|--------|-------|
| Q3 FY26 revenue | $742M (+12% YoY) |
| Q4 guidance | $748-750M |
| Operating margin | 24-26% (non-GAAP) |
| FCF margin | 29% |
| Buyback | **$1B** authorized |

---

## Profitability transformation

| Metric | Then | Now |
|--------|------|-----|
| Operating margin | Single digits | 24-26% |
| FCF generation | Breakeven | $210M+/quarter |
| Capital return | None | $1B buyback |

---

## Competitive landscape

| Competitor | Focus |
|------------|-------|
| [[Microsoft]] Entra | Enterprise identity |
| CrowdStrike | Endpoint + identity |
| Ping Identity | Workforce IAM |
| Auth0 (Okta) | Developer identity |

---

## Investment case

**Bull:**
- AI agent identity = new TAM
- Profitability transformation
- 98% subscription revenue
- $1B buyback signal

**Bear:**
- Growth slowing to 11%
- [[Microsoft]] Entra competition
- Past security incidents
- Valuation vs growth

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | OKTA |
| Exchange | [[Nasdaq]] |
| Revenue | ~$2.8B |
| Focus | Identity/IAM |

---

## Related

- [[CrowdStrike]] — security peer
- [[SaaS]] — sector

---

## Sources

- [Okta investor relations](https://investor.okta.com/)
- [Diginomica: AI security](https://diginomica.com/okta-earnings-beat-expectations-company-removes-macro-caution-bets-big-ai-security-market)

*Created 2026-01-09*

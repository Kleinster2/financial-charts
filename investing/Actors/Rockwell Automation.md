---
aliases: [ROK, Rockwell]
---
#actor #industrial #automation #usa #public

**Rockwell Automation** — #1 industrial automation pure-play in North America. Factory AI, PLCs, robotics integration. Benefits from reshoring.

---

## Why Rockwell Automation matters

| Metric | Value |
|--------|-------|
| Ticker | ROK (NYSE) |
| Market cap | ~$35B |
| Position | #1 North American automation |
| Focus | Discrete manufacturing |

---

## Business segments

| Segment | Revenue | Focus |
|---------|---------|-------|
| Intelligent Devices | ~55% | PLCs, drives, sensors |
| Software & Control | ~25% | FactoryTalk, MES |
| Lifecycle Services | ~20% | Integration, support |

---

## Product portfolio

| Category | Products |
|----------|----------|
| Controllers | Allen-Bradley PLCs |
| Drives | PowerFlex VFDs |
| [[Sensors]] | Machine vision, safety |
| Software | FactoryTalk suite |
| [[Robotics]] | Partnerships (not OEM) |

Allen-Bradley brand dominant in North American factories.

---

## AI/Factory automation

**Industrial AI applications:**

| Use Case | Technology |
|----------|------------|
| Predictive maintenance | ML on sensor data |
| Quality inspection | Machine vision AI |
| Process optimization | Digital twin |
| Autonomous operations | Adaptive control |

Plex acquisition added cloud MES capabilities.

---

## Reshoring tailwind

**US manufacturing investment:**

| Driver | Impact |
|--------|--------|
| [[CHIPS Act]] | Semiconductor fabs |
| IRA | Clean energy manufacturing |
| Supply chain security | Nearshoring |
| EV battery plants | Greenfield builds |

Every new US factory needs automation systems.

---

## Competitive position

| Competitor | Geography | Focus |
|------------|-----------|-------|
| [[Siemens]] | Global | Broader industrial |
| [[ABB]] | Global | [[Robotics]], power |
| [[Fanuc]] | [[Japan]] | CNC, robotics |
| Emerson | US | Process industries |

Rockwell strongest in discrete manufacturing, North America.

---

## Investment case

**Bull:**
- Reshoring super-cycle
- Software/recurring revenue growth
- Factory AI adoption
- Installed base moat
- EV/battery plant wins

**Bear:**
- Cyclical capital spending
- Valuation premium
- International weakness
- Siemens competition

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | ROK |
| Market cap | ~$35B |
| Dividend yield | ~1.8% |
| HQ | Milwaukee |

*Created 2026-01-06*

---

## Related

- [[Honeywell]] — peer (industrial automation)
- [[Siemens]] — competitor (factory automation)
- [[ABB]] — competitor (robotics)
- [[Fanuc]] — competitor (CNC, robotics)
- [[TSMC]] — customer type (fab automation)

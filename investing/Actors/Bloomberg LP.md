---
aliases: [Bloomberg, Bloomberg Terminal]
---
#actor #finance #media #data #usa #private

**Bloomberg LP** — Financial data, software, and media company. The Bloomberg Terminal dominates financial data with ~33% market share. Michael Bloomberg owns 88%.

---

## Overview

| Metric | Value |
|--------|-------|
| Founded | 1981 |
| Founder | [[Michael Bloomberg]] |
| HQ | New York, NY |
| Ownership | 88% Michael Bloomberg, 12% Merrill Lynch (original) |
| Revenue (2024 est.) | ~$13B |
| Terminal subscribers | 325,000 (2022) |
| Market share | 33% of $40B financial data market |

---

## History

**1981 — The severance bet:**
Michael Bloomberg, a general partner at [[Salomon Brothers]], was pushed out after the firm sold to Phibro Corporation. At age 39, he took his **$10M severance** and founded Innovative Market Systems (later Bloomberg LP) with Thomas Secunda, Duncan MacMillan, and Charles Zegar.

**1982 — Merrill Lynch contract:**
Bloomberg secured a **$30M contract** from Merrill Lynch, which also took a 12% ownership stake. The first terminal shipped in December 1982.

**Growth milestones:**

| Year | Terminals | Revenue | Notes |
|------|-----------|---------|-------|
| 1982 | 1 | — | First terminal to Merrill Lynch |
| 1987 | — | — | London office opened |
| 1990 | 10,000 | — | Bloomberg News launched |
| 1992 | 20,000 | — | Rapid expansion |
| 1993 | 31,000 | $370M | [[Media]] ventures drove terminal sales |
| 2012 | — | — | Surpassed [[Thomson Reuters]] in terminal revenue |
| 2022 | 325,000 | — | Current subscriber base |
| 2024 | — | ~$13B | Estimated revenue |

**Key insight:** Bloomberg discovered that financial professionals would pay premium prices ($20-30K/year) for a terminal that combined data, analytics, news, and messaging in one system. The chat function ("Bloomberg Message") created network effects — traders needed terminals to talk to other traders.

---

## Business segments

| Segment | Revenue share | Description |
|---------|---------------|-------------|
| **Bloomberg Terminal** | ~85% | Data, analytics, trading, messaging |
| **Enterprise data** | ~10% | Licensing data to other platforms |
| **Bloomberg News/[[Media]]** | ~5% | News, TV, radio, digital |

**Terminal pricing:** ~$25,000-30,000/user/year (vs. Refinitiv at ~$20,000)

---

## What makes Bloomberg, Bloomberg

**The flywheel:**
```
More subscribers → Better chat network → More essential → Higher prices → More R&D → Better product → More subscribers
```

**Moats:**
- **[[Network effects]]:** Bloomberg Message is how Wall Street communicates. Can't leave if counterparties are on Bloomberg.
- **Switching costs:** Workflows, keyboard shortcuts, muscle memory built over decades
- **Data breadth:** 35M+ instruments, proprietary indices, historical depth
- **Bundling:** News + data + analytics + trading + chat in one terminal

**The math:** 325,000 terminals × $25K/year = $8B+ just from subscriptions. Add enterprise data licensing and media, and you get to $13B.

---

## Leadership

| Role | Name | Tenure |
|------|------|--------|
| Founder/Owner (88%) | [[Michael Bloomberg]] | 1981–present |
| CEO | Michael Bloomberg | 1981–2001, 2014–2023 |
| Current CEO | — | (verify current) |

Bloomberg stepped away 2002–2013 to serve as NYC Mayor.

---

## Competitors

| Company | Market share | Notes |
|---------|--------------|-------|
| **Bloomberg** | ~33% | Dominant in trading, FI |
| **Refinitiv** (LSEG) | ~25% | Cheaper, Eikon terminal |
| **FactSet** | ~5% | Strong in buy-side research |
| **S&P Capital IQ** | ~5% | Corporate finance focus |
| **[[Morningstar]]** | — | Retail/advisor focus |

**Competitive dynamics:** Refinitiv gaining share at lower price point. But Bloomberg's network effects in chat/messaging create stickiness that pure data competitors can't match.

---

## Related

- [[Michael Bloomberg]] — founder, 88% owner, former NYC mayor
- [[Refinitiv]] — main competitor (owned by LSEG)
- [[FactSet]] — competitor (public: FDS)
- [[S&P Global]] — competitor (owns Capital IQ)
- [[Financial data industry]] — sector context

*Sources: [Wikipedia](https://en.wikipedia.org/wiki/Bloomberg_L.P.), [Statista](https://www.statista.com/statistics/1387634/bloomberg-lp-revenue/), [Fast Company](https://www.fastcompany.com/3051883/the-bloomberg-terminal)*

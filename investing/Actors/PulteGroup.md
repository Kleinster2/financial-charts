---
aliases: [Pulte Homes, PHM]
---
#actor #housing #homebuilder #usa #public

**PulteGroup** — Major US homebuilder. Move-up buyer focus. Atlanta-based.

---

## Why PulteGroup matters

Third largest public homebuilder:

| Metric | Value |
|--------|-------|
| Market cap | ~$25B |
| Ticker | PHM |
| Founded | 1950 |
| HQ | Atlanta, GA |
| Homes closed (FY24) | ~30,000 |

---

## Business model

**Move-up focus:**
- Higher price points than DR Horton
- Move-up and active adult buyers
- Less rate-sensitive than entry-level
- Quality over volume

---

## Brands

| Brand | Segment |
|-------|---------|
| Pulte Homes | Move-up |
| Del Webb | 55+ active adult |
| Centex | Entry-level |

Del Webb = major 55+ brand (aging demographics tailwind).

---

## Geographic footprint

**National presence:**
- Florida
- Texas
- Arizona
- California
- Midwest
- 25+ states

---

## Competitive position

| Builder | FY24 Closings | Segment |
|---------|---------------|---------|
| [[DR Horton]] | ~90,000 | Entry-level |
| [[Lennar]] | ~80,000 | Diversified |
| **PulteGroup** | ~30,000 | Move-up |
| [[Toll Brothers]] | ~10,000 | [[Luxury]] |

---

## Del Webb advantage

**55+ active adult:**
- Baby boomer demand
- Less rate-sensitive (cash buyers)
- Higher margins
- Amenity-rich communities
- Growing demographic

---

## Trump housing ban (Jan 7, 2026)

Stock dipped on institutional homebuying ban announcement:
- Less exposed than entry-level builders
- Move-up buyers less institutional
- See [[Institutional homebuying ban]]

---

## Investment case

**Bull:**
- Move-up = less rate-sensitive
- Del Webb demographics
- Higher margins
- Less commodity exposure
- Baby boomer tailwind

**Bear:**
- Smaller scale than DHI/LEN
- Still cyclical
- Move-up buyers need equity
- Labor/material costs

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | PHM |
| Market cap | ~$25B |
| Homes/year | ~30,000 |
| Segment | Move-up, 55+ |

*Created 2026-01-08*

---

## Related

- [[Housing]] — sector hub
- [[DR Horton]] — peer (#1, entry-level)
- [[Lennar]] — peer (#2, diversified)
- [[Toll Brothers]] — peer (luxury)
- [[Single-family rental]] — build-to-rent context
- [[Institutional homebuying ban]] — Trump policy (Jan 2026)


---
aliases: [COR, CoreSite Realty]
---
#actor #datacenter #reit #usa #public

**Coresite** — Data center REIT focused on interconnection. Acquired by [[American Tower]] (2021). Colocation and cloud on-ramps.

---

## Why Coresite matters

| Metric | Value |
|--------|-------|
| Status | Subsidiary of [[American Tower]] |
| Acquired | 2021 (~$10B) |
| Focus | Colocation, interconnection |
| Markets | Major US metros |

---

## Business model

| Service | Description |
|---------|-------------|
| Colocation | Rack space, power, cooling |
| Interconnection | Cross-connects, cloud on-ramps |
| Cloud on-ramps | Direct connects to AWS, Azure, GCP |
| Network density | High interconnection value |

**Differentiation:** Dense interconnection hubs — valuable for low-latency connections.

---

## Acquisition by [[American Tower]]

**2021 deal:**
- [[American Tower]] acquired Coresite for ~$10B
- Tower company → data center expansion
- Coresite operates as subsidiary
- No longer separately traded

---

## AI/cloud relevance

| Factor | Description |
|--------|-------------|
| Cloud on-ramps | Enterprises connect to hyperscalers |
| Hybrid cloud | Bridge between private and public cloud |
| AI inference | Low-latency edge locations |
| Network density | GPU clusters need high bandwidth |

---

## Data center REIT landscape

| Company | Focus | Status |
|---------|-------|--------|
| [[Equinix]] | Largest, global | Public |
| [[Digital Realty]] | Hyperscale + colo | Public |
| **Coresite** | Interconnection | [[American Tower]] subsidiary |
| [[QTS]] | Hyperscale | Acquired by [[Blackstone]] |
| CyrusOne | Enterprise | Acquired by [[KKR]] |

**Trend:** Private equity consolidating data center [[REITs]].

---

## Geographic footprint

| Market | Presence |
|--------|----------|
| Northern Virginia | Major hub |
| Los Angeles | West Coast |
| Denver | Mountain region |
| Silicon Valley | Tech core |
| Chicago | Midwest |
| Others | Boston, Miami, NYC |

---

## Investment implication

**Coresite itself:** Not separately investable ([[American Tower]] subsidiary).

**Exposure via:**
- [[American Tower]] (AMT) — parent company
- [[Equinix]], [[Digital Realty]] — public data center peers

---

## Related

- [[Equinix]] — largest data center REIT
- [[Digital Realty]] — peer
- [[Power constraints]] — data center power demand
- [[AI hyperscalers]] — customers


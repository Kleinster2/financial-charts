#actor #fintech #private #crypto

**Polymarket** — Largest crypto-based [[prediction markets|prediction market]]. Founded 2020 by [[Shayne Coplan]]. Offshore (US users technically prohibited). $9B valuation (Oct 2025) after [[ICE]] $2B investment. 2024 election validated platform — Trump probability diverged from polls.

---

## Quick stats

| Metric | Value |
|--------|-------|
| **Valuation** | $9B (Oct 2025) |
| **Total raised** | $2.3B+ |
| **Founded** | 2020 |
| **HQ** | Manhattan, New York |
| **Founder stake** | ~11% |
| **Blockchain** | Polygon (Ethereum L2) |
| **2024 election volume** | $3.7B |

---

## What makes Polymarket, Polymarket

**Crypto-native, globally accessible.** While [[Kalshi]] went the regulated route, Polymarket built on blockchain — anyone with a crypto wallet can trade. US users technically prohibited after 2022 CFTC settlement, but enforcement is limited.

**2024 election moment:** Polymarket showed Trump 60-65% when polls showed toss-up. Trump won. This was prediction markets' mainstream validation — media started citing Polymarket probabilities alongside polls.

**The trade-off:**
- **Advantage:** Global access, no KYC friction, deeper liquidity on major events
- **Disadvantage:** No US regulatory license, can't onboard US users legally, vulnerable to enforcement

---

## Founder

**[[Shayne Coplan]]** (b. 1998)
- Upper West Side NYC, dropped out of NYU
- Early Ethereum investor (2014 ICO)
- Founded Polymarket 2020 (age 22)
- Net worth: ~$1B (11% of $9B valuation)
- **Youngest self-made billionaire** (Oct 2025, age 27)

Origin story: At 14, emailed SEC asking how to create new marketplaces. Founded Polymarket to combat "rampant misinformation" — let markets discover truth.

---

## Funding history

| Round | Date | Amount | Valuation | Lead |
|-------|------|--------|-----------|------|
| Seed | 2020 | — | — | — |
| Series A | 2022 | $25M | — | Polychain |
| Series B | May 2024 | $70M | — | Founders Fund (Thiel) |
| Strategic | Oct 2025 | $2B | $9B | [[ICE]] |

**Key investors:** [[ICE]] (NYSE parent), [[Founders Fund]], General Catalyst, Polychain, Blockchain Capital, [[Point72]], [[Vitalik Buterin]], [[Coinbase]]

**ICE investment (Oct 2025):** Intercontinental Exchange (owns NYSE) invested $2B — signals traditional finance validation of prediction markets.

---

## Regulatory history

| Date | Event |
|------|-------|
| **2022** | CFTC $1.4M settlement — operating unregistered exchange |
| **Post-2022** | Blocks US users (geo-fencing) |
| **Jul 2025** | DOJ/CFTC probe closed — no further action |

**Current status:** Operates offshore, US users prohibited. But enforcement is minimal — VPNs common. This is the legal grey zone.

---

## 2024 election

**The moment prediction markets went mainstream:**

| Metric | Polymarket | Polls |
|--------|------------|-------|
| **Final Trump probability** | 60-65% | ~50% (toss-up) |
| **Election volume** | $3.7B | — |
| **Outcome** | Trump won | — |

Media started displaying Polymarket probabilities alongside polls. CNN, Bloomberg, others cited the platform. Prediction markets proved more accurate than traditional polling.

---

## vs Kalshi

| | Polymarket | [[Kalshi]] |
|---|------------|---------|
| **Regulatory status** | Offshore (CFTC-settled) | CFTC DCM license |
| **US users** | Prohibited (but accessible) | Legal |
| **Infrastructure** | Blockchain (Polygon) | Traditional exchange |
| **KYC** | Minimal | Full compliance |
| **State lawsuit risk** | None (offshore) | Major (90% sports volume) |
| **Valuation** | $9B | $11B |

---

## Ultra-short-term crypto contracts (2025-2026)

[[Polymarket]] launched 15-minute "up-down" bets on [[Bitcoin]], [[Ethereum]], [[Solana]], and [[XRP]] in late 2025, then added 5-minute contracts. These are binary bets on whether token prices will be higher or lower at expiry.

**Volume:** Short-term crypto markets now represent more than half of all crypto trading on the platform, reaching ~$100M+ daily (7-day rolling average) by March 2026.

![[polymarket-short-term-crypto-volume-ft-mar26.png]]
*Short-term crypto volume grew rapidly despite January fee introduction. Source: Polymarket, FT Analysis (Mar 13, 2026)*

**Fee introduction:** Added per-trade fees on 15-minute crypto bets in January 2026, which helped deter bot-driven arbitrage. Subsequently expanded fees up to 1.56% to all crypto contracts.

**Arbitrage:** [[Keyrock]] researcher Amir Hajian reported latency arbitrage was "rampant" on 15-minute markets before the fee introduction — sophisticated traders exploiting time lags between Polymarket and [[Binance]]. Five-minute markets still exhibit "microstructure inefficiencies" targeted by HFT firms.

Source: FT (Mar 13, 2026)

---

## The bull case

1. **Global access** — no geographic restrictions (except US)
2. **Crypto-native** — self-custody, permissionless
3. **ICE backing** — traditional finance validation
4. **Liquidity** — deepest markets on major events
5. **2024 proved value** — mainstream credibility

## The bear case

1. **No US license** — can't legally serve largest market
2. **Regulatory risk** — could face enforcement
3. **Kalshi competition** — if Kalshi gets federal preemption
4. **Manipulation concerns** — Polymarket faced questions about whale activity

---

## Related

- [[Shayne Coplan]] — founder
- [[Prediction markets]] — concept
- [[Kalshi]] — regulated competitor
- [[ICE]] — strategic investor (NYSE parent)
- [[CFTC]] — settled 2022
- [[Crypto]] — infrastructure
- [[Founders Fund]] — investor

---
aliases:
  - CF Industries Holdings
  - CF
tags:
  - actor
  - company
  - public
  - chemicals
  - fertilizer
  - nitrogen
  - ammonia
---
	
# CF Industries

The world's largest ammonia producer and the S&P 500's #1 performer in March 2026. CF's structural cost advantage — North American natural gas feedstock at ~$3.34/MMBtu vs multiples of that in Europe/Asia — turned the [[Strait of Hormuz]] disruption into a windfall: stock hit an all-time high of $136 on March 12 before settling at $129.57 on March 13, up ~77% YTD. Revenue surged to $7.08B in FY2025 (+19.1%), with adj. EBITDA of $2.89B. The company is also building a second act through blue ammonia — the $4B Blue Point Complex JV with [[JERA]] and [[Mitsui]] targets 1.4M mt/year of low-carbon ammonia by 2029. The bull case: irreplaceable domestic nitrogen capacity in a world where ~20% of European ammonia production is curtailed. The bear case: geopolitical premium fades, Senator Hawley's price-gouging accusations introduce regulatory risk, and nat gas input costs aren't zero.

---

## Sector correlation

> [!warning] Sector Orphan
> CF does not trade tightly with any sector ETF (max r = 0.48 with XLE).

| Sector | ETF | Correlation |
|--------|-----|-------------|
| [[Energy and Utilities\|Energy]] | XLE | 0.48 |
| Oil & Gas E&P | XOP | 0.48 |
| Utilities | XLU | 0.26 |
| *S&P 500* | *SPY* | *0.16* |

CF trades between Energy and Oil & Gas E&P without a tight sector fit. Moves on nitrogen/fertilizer supply-demand dynamics, natural gas spreads, and geopolitical disruption — not energy writ large.

---

![[cf-vs-mos-vs-xle-price-chart.png]]
*CF (blue) dramatically outperforming peer [[Mosaic]] and the energy sector benchmark XLE since early 2026, driven by the Iran/Hormuz nitrogen supply disruption. The 2022 spike reflects the prior energy crisis premium.*

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | CF (NYSE) |
| Market cap | ~$19B (Mar 2026) |
| Stock price | $129.57 (Mar 13, 2026) |
| YTD performance | +77% |
| Revenue (FY2025) | $7.08B (+19.1%) |
| Net earnings (FY2025) | $1.46B ($8.97/share) |
| Adj. EBITDA (FY2025) | $2.89B |
| Cash | ~$2.2B |
| HQ | Deerfield, IL |
| Founded | 1946 |
| CEO | Chris Bohn (from Jan 2026) |
| Employees | ~3,000 |
| Ammonia capacity | ~10.4 Mt/year |
| Key products | [[Ammonia]], [[urea]], UAN, ammonium nitrate, DEF |

---

## Leadership

| Role | Name | Background |
|------|------|------------|
| CEO | Chris Bohn | Former EVP; succeeded Tony Will Jan 4, 2026 |
| Former CEO | Tony Will | CEO 2014-2025; retired Jan 4, 2026; disciplined capital allocator, pivoted CF from broad ag to pure-play nitrogen |
| Board Chair | Stephen Hagge | |

---

## Evolution

The story of CF Industries is the story of a farm cooperative that became a geopolitical beneficiary — twice.

- 1946-2004: Founded as Central Farmers Fertilizer Company, a federation of regional agricultural cooperatives serving Midwest farmers. For nearly six decades, it operated as a collective purchasing and distribution model — large but not particularly exciting.

- 2005: IPO marked the transition to an independent public nitrogen producer. The listing unlocked capital for what would become a decade of aggressive transformation.

- 2010: The $4.7B acquisition of [[Terra Industries]] was the defining move — it consolidated CF's position as one of the world's largest nitrogen producers and gave it the scale to compete globally. The deal roughly doubled capacity.

- 2014: Tony Will became CEO and began a strategic focus on pure-play nitrogen. CF divested its phosphate business to [[Mosaic]], shedding complexity to concentrate on what it does best: converting cheap North American natural gas into ammonia and its derivatives. This bet on feedstock cost advantage over product diversification would prove prescient.

- 2016: New ammonia plant at Donaldsonville, Louisiana came online — 3,600 tons/day nameplate capacity, making the complex the single largest ammonia production facility in the world (~8M tons nitrogen products/year depending on product mix).

- 2022: First geopolitical windfall. Russia's invasion of Ukraine and the European energy crisis sent global nitrogen prices to historic highs. CF's North American gas advantage created massive margins as European competitors paying 5-10x for gas either curtailed or shut down entirely. Revenue peaked at $11.19B — nearly double the prior year.

- 2023-2024: The hangover. As energy prices normalized, revenue fell 40.7% to $6.63B in 2023 and another 10.5% to $5.94B in 2024. CF used the downcycle to buy back stock aggressively — retiring ~30% of float over the decade — and to build its clean energy platform.

- 2025: Blue ammonia strategy took shape. CF shipped its first certified low-carbon ammonia cargoes to Envalior in Belgium (Q3 2025). The Yazoo City CCS project with [[Exxon]] was announced ($100M investment, 500K mt CO2/year, startup 2028). Section 45Q carbon sequestration tax credits ($85/mt CO2) began appearing on the bottom line. Revenue recovered to $7.08B (+19.1%) as nitrogen markets tightened. Tony Will retired Jan 4, 2026; Chris Bohn succeeded him.

- 2026: Second geopolitical windfall. Iran conflict disrupted the [[Strait of Hormuz]], through which a meaningful share of global fertilizer flows. ~20% of European ammonia capacity and ~25% of urea capacity curtailed. Urea prices surged ~32% in 12 days ($516 → $683/mt). CF hit an all-time high of $136, becoming the S&P 500's #1 performer in March.

---

## Financial performance

| Year | Revenue ($B) | Growth | Net Earnings ($B) | EPS | Adj. EBITDA ($B) |
|------|-------------|--------|-------------------|-----|-----------------|
| 2022 | 11.19 | — | — | — | — |
| 2023 | 6.63 | -40.7% | 1.53 | — | — |
| 2024 | 5.94 | -10.5% | 1.22 | $6.74 | 2.28 |
| 2025 | 7.08 | +19.1% | 1.46 | $8.97 | 2.89 |

### Q4 2025 (reported late Feb 2026)

| Metric | Q4 2025 |
|--------|---------|
| Net earnings | $404M ($2.59/share, beat $2.48 consensus by $0.11) |
| EBITDA | $731M |
| Adj. EBITDA | $821M |
| Gross margin | 38.5% (vs 34.6% prior year) |
| Avg nat gas cost | ~$3.34/MMBtu |

### Cash flow and capital return

| Metric | FY2025 |
|--------|--------|
| Operating cash flow | $2.75B |
| Free cash flow | $1.79B |
| Shares repurchased | 16.6M (~10% of float) |
| Buyback spend | $1.34B |
| New buyback authorization | $2B (late 2025) |
| Cash on hand | ~$2.2B |

Over the past decade, CF has retired ~30% of its outstanding shares through buybacks — a core part of the capital allocation strategy.

---

## Business model

CF is a pure-play nitrogen manufacturer. The Haber-Bosch process that produces ammonia is energy-intensive, and CF's entire competitive advantage rests on North American natural gas being structurally cheaper than what European and Asian producers pay. When the gas spread widens — as during geopolitical disruptions — CF's margins expand mechanically.

Revenue mix:

| Product | Use |
|---------|-----|
| [[Ammonia]] | Direct fertilizer + feedstock for other products |
| Granular [[urea]] | Solid fertilizer, high nitrogen content |
| UAN (urea ammonium nitrate) | Liquid fertilizer, precise application |
| Ammonium nitrate | Agriculture + industrial explosives |
| DEF (diesel exhaust fluid) | Emissions reduction for diesel engines |
| Blue ammonia (emerging) | Low-carbon fuel for shipping/power |

Key facilities:

| Site | Location | Capacity |
|------|----------|----------|
| Donaldsonville | Louisiana | ~8M mt nitrogen products/year (world's largest ammonia complex) |
| Port Neal | Iowa | Major nitrogen complex |
| Yazoo City | Mississippi | Ammonia + CCS with [[Exxon]] (2028) |
| Blue Point Complex | Louisiana | 1.4M mt/year blue ammonia (2029, JV) |

Logistics moat: Mississippi River system + NuStar ammonia pipeline + rail/truck/barge/deepwater vessel access from Donaldsonville directly into the US Corn Belt.

---

## Clean energy platform

### Blue Point Complex

| Detail | Value |
|--------|-------|
| JV partners | CF Industries (40%), [[JERA]] (35%), [[Mitsui]] (25%) |
| Total investment | ~$4B |
| Capacity | 1.4M mt/year low-carbon ammonia |
| Technology | Autothermal reforming (ATR) with CO2 capture |
| Location | Ascension Parish, Louisiana (RiverPlex MegaPark) |
| Target production | 2029 |
| Offtake | JERA takes portion for maritime fuel/power generation in Asia |

FID was reached in April 2025. The facility will produce ammonia while capturing CO2 for sequestration, enabling "blue" certification at premium pricing.

### Yazoo City CCS project

| Detail | Value |
|--------|-------|
| Partner | [[Exxon]] (transport + sequestration) |
| CF investment | ~$100M |
| CO2 sequestered | Up to 500K mt/year |
| Emissions reduction | ~50% of Yazoo City site |
| Target startup | 2028 |
| Tax credits | Section 45Q at $85/mt CO2 |

### First blue ammonia sales

CF shipped its first certified low-carbon ammonia cargoes to Envalior (Belgium) in Q3 2025, produced at Donaldsonville where CF already captures 2M mt CO2 annually. Premium pricing achieved.

---

## The Hormuz trade (March 2026)

### Why the stock is this strong

Four things rarely align this cleanly:

*Inelastic demand inside a biological deadline.* Corn requires 150-200 lbs/acre of nitrogen — there is no substitute input. The US Corn Belt planting window runs mid-April to late May. Farmers *must* buy nitrogen before then regardless of price. This is not discretionary demand that can be deferred — miss the window, lose the crop. CF gets to name its price for the next 4-6 weeks, and buyers have no choice but to pay.

*Every alternative supplier is offline simultaneously.* The Gulf (~49% of traded urea) is trapped behind the [[Strait of Hormuz]]. [[China]] — the world's largest urea producer — halted ALL fertilizer exports on Mar 16, not just phosphate. [[Russia]] is capped at 18.7M mt through May with Ukrainian drone strikes constraining capacity. European producers (~20% of ammonia, ~25% of urea) are curtailed from energy costs. CF's Donaldsonville complex — the world's largest ammonia plant — is one of the few large-scale nitrogen facilities still running at full capacity with cheap feedstock. There is nowhere else to go.

*The margin expansion is mechanical, not speculative.* CF's input cost is North American natural gas at ~$3.34/MMBtu — that hasn't moved. Its output prices have roughly doubled: urea from pre-war consensus $350-380/t to ~$700/t (NOLA barges at $673-675/st as of Mar 18). The spread widens automatically. No new customers to win, no product to develop, no execution required. Every dollar of urea price increase drops almost entirely to the bottom line.

*The market is still underpricing duration.* [[BofA]]'s DeYoe expects nitrogen prices to peak by mid-Q2 and subside — that's the consensus view, and it's why he maintains Underperform at $103. But the [[Nitrogen Trap]] analysis says 120+ days, not 45. Iran's FM (Mar 15): *"We never asked for a ceasefire."* [[Mojtaba Khamenei]] (Mar 12): *"The lever of blocking the Strait of Hormuz must definitely continue to be used."* Insurance architecture takes 30-60 days *after* a ceasefire to recover. If duration is longer than consensus, [[BofA]]'s +29% EBITDA raise is the first revision, not the last.

### Why 2026 is not 2022

Bears anchor to the 2022 mean-reversion: CF surged on Ukraine, then revenue fell -40.7% in 2023 as European gas normalized. But the analogy fails on the mechanism:

| Dimension | 2022 Russia-Ukraine | 2026 Hormuz |
|-----------|--------------------|--------------------|
| Physical flow | Molecules rerouted via shadow fleet over months | Molecules physically trapped behind a mined chokepoint |
| Onset | Gradual, allowing adaptation | Instantaneous, inside the planting window |
| Alternative supply | China exported freely; Russia found new buyers | China halted ALL exports; Russia capped |
| Insurance | Repriced but ships kept moving | Fractured — capacity contracted, ships stopped |
| Duration precedent | Gas prices normalized in ~12 months | Red Sea: 26 months, premiums never returned |

In 2022, the supply shock was financial — prices adjusted, trade routes adapted. In 2026, the supply shock is physical — the molecules cannot move.

### Price action

| Date | Event |
|------|-------|
| Year-start | ~$73 |
| Mar 12 | $136 all-time high (+86% in 10 weeks, biggest intraday jump since 2020 at +14%) |
| Mar 13 | $129.57 |
| March overall | #1 S&P 500 performer; +37% YTD |

Reddit led the call: retail sentiment on r/stocks flagged the fertilizer angle before mainstream coverage, noting *"not just oil — one third passes through the Hormuz."* Prediction markets assign only ~19.5% probability to Iran conflict resolution by March 31.

### Political risk

Senator Josh Hawley (R-MO) sent a nine-question letter to CEO Chris Bohn on March 12 accusing CF Industries and four other fertilizer producers ([[Mosaic]], [[Koch Fertilizer]], [[Nutrien]], [[Yara]]) of price gouging. Also sent letter to AG Pam Bondi requesting DOJ investigation. Urea price surge of ~32% as evidence.

This is reputational and regulatory risk — potential hearings, scrutiny. Not yet material, but worth tracking. An EVP sold 18,041 shares around the same time.

---

## Investment case

The core question is duration. The [[Wells Fargo]] $150 vs [[BofA]] $103 gap is a duration bet: if you think 45 days, sell; if you think 120+, the stock isn't done.

Bull:
- Inelastic demand inside a biological deadline — buyers can't wait
- Every alternative supplier offline simultaneously (Gulf, China, Russia constrained, Europe curtailed)
- Mechanical margin expansion: $3.34/MMBtu input vs ~$700/t urea output
- Duration thesis (120+ days) means earnings revision cycle hasn't started — BofA's +29% EBITDA raise may be the first of several
- Irreplaceable capacity: Donaldsonville is world's largest ammonia complex, can't be replicated in months
- Aggressive buybacks — retired ~30% of float, $2B new authorization
- ~$2.2B cash fortress, $1.79B free cash flow
- Blue ammonia platform creates valuation floor independent of geopolitics
- Section 45Q credits ($85/mt CO2) create high-margin green revenue stream

Bear:
- Geopolitical premium is inherently fragile — Hormuz resolution crashes the trade overnight
- 2022 showed how fast revenue can mean-revert (-40.7% in 2023) — but see above on why the mechanism differs
- Price-gouging accusations could escalate to hearings/regulation (Hawley deadline Mar 27, DOJ inquiry Mar 4)
- Natural gas feedstock costs can rise (already $3.34/MMBtu, was lower)
- Blue Point Complex ($4B) execution risk — 2029 is far away
- Blue ammonia economics unproven at scale
- Insider selling at all-time highs

---

## Analyst timeline

| Date | Analyst | Rating | PT | Context |
|------|---------|--------|----|---------|
| Mar 2026 | [[Wells Fargo]] | Overweight | $150 | Raised from $113; nitrogen market tailwinds |
| Mar 2026 | [[Barclays]] | — | $120 | Raised on nitrogen supply forecasts; stock already blown past |
| Mar 2026 | [[BofA]] (Matthew DeYoe) | Underperform | $103 | Raised 2026 EBITDA forecast +29%; views spike as cyclical, expects nitrogen prices to peak by mid-Q2 then subside |

---

## Related

- [[Ammonia]] — primary product
- [[urea]] — major derivative product
- [[Strait of Hormuz]] — geopolitical catalyst for 2026 rally
- [[JERA]] — Blue Point Complex JV partner (35%)
- [[Mitsui]] — Blue Point Complex JV partner (25%)
- [[Exxon]] — Yazoo City CCS partner
- [[Mosaic]] — peer; also received Hawley price-gouging letter; acquired CF's phosphate business
- [[Koch Fertilizer]] — peer
- [[Nutrien]] — peer
- [[Yara]] — peer
- [[Iran]] — conflict driving Hormuz disruption
- [[Nitrogen Trap]] — systemic framework for the fertilizer supply crisis; Perera's positioning: indicative target $175, hard stop $115

*Updated 2026-03-19*

---
aliases: [中际旭创, 300308]
---
#actor #optical #china #ai-infrastructure

**Zhongji Innolight** — Chinese optical module maker. [[Shenzhen]]-listed, seeking $3B+ Hong Kong secondary listing. Key supplier for AI data center networking.

---

## Why Zhongji Innolight matters

Optical modules are critical for AI data center interconnects:

| Metric | Value |
|--------|-------|
| Primary listing | [[Shenzhen]] (300308) |
| Secondary listing | Hong Kong ($3B+, 2026) |
| Business | Optical communication modules |
| End market | Data centers, AI networking |

---

## AI networking exposure

**Why optical modules matter for AI:**
- Connect GPUs within data centers
- High-speed data transfer for training clusters
- 800G/1.6T transceivers for AI workloads
- Demand scales with GPU deployment

Key beneficiary of AI infrastructure buildout.

---

## Hong Kong listing

| Metric | Value |
|--------|-------|
| [[Target]] raise | **$3B+** |
| Exchange | [[HKEX]] |
| [[Banks]] | Selected |
| Status | In preparation |

Part of wave of [[Shenzhen]]-listed Chinese firms adding HK listings (see [[Asia IPO pipeline 2026]]).

---

## Competitive position

| Competitor | Notes |
|------------|-------|
| Lumentum | US optical |
| Coherent | US optical |
| II-VI | US optical |
| [[Fabrinet]] | Contract manufacturer |

Chinese domestic leader with cost advantage.

---

## Investment case

**Bull:**
- AI data center demand driver
- $3B+ raise = significant expansion capacity
- Dual listing = international investor access
- Critical infrastructure component

**Bear:**
- Export control risk (if targeted)
- Customer concentration
- Cyclical end markets
- Competition from US/[[Japan]] optical

---

*Created 2026-01-05*

---

## Related

- [[Asia IPO pipeline 2026]] — context (HK listing wave)
- [[AI hyperscalers]] — customers
- [[Power constraints]] — related (DC buildout)
- [[Lumentum]] — competitor (US optical)
- [[Coherent]] — competitor (US optical)

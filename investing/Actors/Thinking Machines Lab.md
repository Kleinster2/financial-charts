---
aliases: [TML]
---
#actor #ai #frontier #usa #private

**Thinking Machines Lab** — Mira Murati's lab. $12B valuation. $2B seed (largest ever). ⅔ team ex-OpenAI.

---

## Why Thinking Machines Lab matters

Largest seed round ever, elite OpenAI alumni:

| Metric | Value |
|--------|-------|
| Valuation | $12B |
| Seed raised | $2B |
| Team | ~30 |
| Ex-OpenAI | ⅔ of team |

---

## Mission

**Human-AI collaboration:**
> Building AI models and products that support more "human-AI collaboration" across every field of work.

Enterprise-focused, not consumer.

---

## Leadership

| Person | Role | Background |
|--------|------|------------|
| [[Mira Murati]] | CEO | OpenAI CTO |
| [[John Schulman]] | Chief Scientist | OpenAI co-founder |
| Barret Zoph | CTO | OpenAI post-training |
| Bob McGrew | Advisor | OpenAI Chief Research Officer |
| Alec Radford | Advisor | OpenAI key researcher |

Most concentrated ex-OpenAI talent outside Anthropic.

---

## Cap table / Investors

| Round | Date | Amount | Valuation |
|-------|------|--------|-----------|
| Seed | July 2025 | $2B | $12B |
| **Total** | | **$2B** | |

**Key investors:**

| Investor | Notes |
|----------|-------|
| [[a16z]] | Lead |
| [[NVIDIA]] | Strategic |
| [[AMD]] | Strategic (rare NVIDIA + AMD combo) |
| Accel | Growth |
| ServiceNow | Enterprise strategic |
| Cisco | Enterprise strategic |
| Jane Street | Financial |

**Total raised:** $2B

**Record:** Largest seed round in Silicon Valley history.

---

## Product: Tinker

**Launched October 2025:**
- Fine-tuning API
- Open-weight model support
- Enterprise customers submit jobs
- TML runs on internal clusters

Enterprise fine-tuning as a service.

---

## Governance

**Founder-controlled:**
- Murati has deciding board vote
- Founding shareholders: 100x votes
- Learned from OpenAI governance chaos

---

## Team composition

**~30 people:**
- ⅔ from OpenAI
- Elite research talent
- Post-training expertise
- Small but concentrated

---

## Comparison to peers

| Lab | Valuation | Team | Seed |
|-----|-----------|------|------|
| **TML** | $12B | ~30 | $2B |
| [[SSI]] | $32B | ~20 | $3B (total) |
| [[Anthropic]] | $61.5B | 1,000+ | — |
| [[Cohere]] | $7B | 450 | — |

TML = enterprise focus, SSI = pure research.

---

## Investment case

**Bull:**
- Elite OpenAI talent
- Largest seed ever = investor conviction
- Enterprise fine-tuning market
- Murati brand
- AMD + NVIDIA both invested

**Bear:**
- Early stage
- No moat yet
- Competition from Anthropic, OpenAI
- Execution on enterprise sales
- Talent retention

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | Private |
| Valuation | $12B |
| Seed raised | $2B |
| Team | ~30 |
| CEO | Mira Murati |

*Updated 2026-01-01*

---

## Related

- [[Mira Murati]] — CEO (OpenAI CTO alumna)
- [[John Schulman]] — Chief Scientist (OpenAI co-founder)
- [[OpenAI]] — origin (⅔ team from OpenAI)
- [[SSI]] — peer (Ilya's lab, $32B valuation)
- [[Anthropic]] — competitor (frontier lab)
- [[a16z]] — investor (led $2B seed)

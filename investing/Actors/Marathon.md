---
aliases: [Marathon Digital, MARA]
---
#actor #crypto #datacenter #usa

**Marathon Digital (MARA)** — Largest public Bitcoin miner by market cap. Potential AI/HPC pivot candidate.

---

## Why Marathon matters

Largest crypto miner with DC infrastructure:

| Metric | Value |
|--------|-------|
| Ticker | MARA |
| Market cap | ~$5B+ |
| Hash rate | ~30+ EH/s |
| Power capacity | 1+ GW |
| AI pivot | Exploring |

---

## Crypto-to-AI pivot potential

Marathon has infrastructure that could serve AI:

| Asset | AI relevance |
|-------|--------------|
| Power contracts | Cheap energy for GPUs |
| Data centers | Cooling, racking |
| Land | Expansion capacity |
| Operations | 24/7 uptime expertise |

See [[Crypto-to-AI pivot]] for industry trend.

---

## Competitive position

| Miner | Hash rate | AI pivot status |
|-------|-----------|-----------------|
| **Marathon** | ~30 EH/s | Exploring |
| [[Core Scientific]] | ~20 EH/s | CoreWeave deal |
| [[Riot Platforms]] | ~20 EH/s | Minimal |
| [[TeraWulf]] | ~10 EH/s | Anthropic consortium |
| [[Hut 8]] | ~5 EH/s | Anthropic consortium |
| [[Cipher Mining]] | ~8 EH/s | Anthropic consortium |

Marathon is largest but hasn't announced major AI deal yet.

---

## Infrastructure

| Metric | Value |
|--------|-------|
| Power capacity | 1+ GW |
| Facilities | Multiple US locations |
| Cooling | Air + immersion |
| Grid contracts | Long-term PPAs |

---

## AI opportunity

**Bull case:**
- Largest miner = most infrastructure to convert
- Power contracts valuable for AI
- Bitcoin mining margins compressing
- AI/HPC offers diversification

**Bear case:**
- No announced AI deals yet
- Competitors (Core Scientific, TeraWulf) moving faster
- Retrofit costs uncertain
- Management focused on Bitcoin

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | MARA |
| Market cap | ~$5B+ |
| Power | 1+ GW |
| Hash rate | ~30+ EH/s |
| AI status | Exploring |

*Updated 2026-01-04*

---

## Related

- [[Crypto-to-AI pivot]] — thesis context
- [[Core Scientific]] — competitor (CoreWeave deal)
- [[TeraWulf]] — competitor (Anthropic deal)
- [[Hut 8]] — competitor (Anthropic deal)
- [[Cipher Mining]] — competitor (Anthropic deal)
- [[Riot Platforms]] — competitor
- [[Power constraints]] — infrastructure value
- [[BYOP]] — power as moat

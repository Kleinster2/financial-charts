---
aliases: [HUT]
---
#actor #crypto #ai #canada #public

**Hut 8** — [[Crypto]] miner → AI. $7B Google-backed Fluidstack deal. Highrise AI subsidiary. Spun out American Bitcoin.

---

## Why Hut 8 matters

Major AI infrastructure pivot with Google + Anthropic backing:

| Metric | Value |
|--------|-------|
| Ticker | HUT |
| 2025 YTD | +139% |
| AI deal | $7B (15yr, 245MW) |
| **Anthropic deal** | DC infrastructure for ~1M TPUv7 |
| Subsidiary | Highrise AI |
| GPUs | 1,000+ H100s |

---

## Fluidstack deal (2025)

**Google-backed AI infrastructure:**

| Aspect | Details |
|--------|---------|
| Value | $7B |
| Term | 15 years |
| Capacity | 245MW |
| Location | River Bend, Louisiana |
| Backer | Google (via Fluidstack) |

Long-term visibility with hyperscaler backing.

---

## Highrise AI subsidiary

**GPU-as-a-Service (Sept 2024):**
- 1,000+ NVIDIA H100 GPUs
- AI cloud services
- Separate business unit
- Growing revenue stream

---

## American Bitcoin spinout

**March 2025:**
- Spun out Bitcoin mining operations
- Hut 8 remains majority shareholder
- Allows focus on AI/HPC
- Clean separation of businesses

---

## AI pivot strategy

**Full transformation:**
- From Bitcoin miner to AI infrastructure
- Highrise AI = GPU cloud
- Long-term hyperscaler contracts
- Power infrastructure leverage

**Anthropic deal (Jan 2026, per SemiAnalysis):**
- DC infrastructure for [[Anthropic]]'s ~1M TPUv7 deployment
- Part of consortium with [[TeraWulf]], [[Cipher Mining]]
- [[FluidStack]] handles deployment operations
- First major AI lab (not hyperscaler) customer

---

## Infrastructure

| Asset | Details |
|-------|---------|
| River Bend (LA) | 245MW, Fluidstack deal |
| H100 GPUs | 1,000+ deployed |
| Power capacity | Significant MW |

---

## Comparison to peers

| Company | AI deal | Approach |
|---------|---------|----------|
| [[IREN]] | $9.7B [[Microsoft]] | Hybrid |
| **Hut 8** | $7B Fluidstack | Full pivot |
| [[Core Scientific]] | $3.5B [[CoreWeave]] | Hosting |
| [[TeraWulf]] | 200MW Fluidstack | HPC focus |

Hut 8 = most complete transformation.

---

## Investment case

**Bull:**
- Google backing (via Fluidstack)
- 15-year contract visibility
- Clean Bitcoin spinout
- 1,000+ H100s deployed
- Full AI infrastructure focus

**Bear:**
- Execution on 245MW buildout
- Fluidstack intermediary risk
- Competition intensifying
- Capital requirements high
- Stock already +139%

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | HUT |
| Market cap | — |
| HQ | [[Canada]] |
| AI deal | $7B Fluidstack |
| GPUs | 1,000+ H100 |

*Updated 2026-01-01*

---

## Related

- [[Google]] — backer (via Fluidstack $7B deal)
- [[Anthropic]] — customer (~1M TPUv7 infrastructure)
- [[IREN]] — competitor, peer (crypto-to-AI)
- [[Core Scientific]] — competitor, peer
- [[TeraWulf]] — peer (Anthropic consortium)
- [[Cipher Mining]] — peer (Anthropic consortium)
- [[FluidStack]] — deployment partner (operations)
- [[NVIDIA]] — GPU supplier (1,000+ H100s)
- [[Crypto-to-AI pivot]] — thesis context

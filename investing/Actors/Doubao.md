---
aliases: [豆包, ByteDance AI]
---
#actor #ai #llm #china #subsidiary

**Doubao** — ByteDance's AI chatbot. **100M+ DAU**, 159M MAU. China's most popular AI app (passed [[DeepSeek]]). Lowest user acquisition cost of any ByteDance product to reach 100M. Now powering agentic commerce via [[Douyin]].

---

## Why Doubao matters

| Metric | Value |
|--------|-------|
| DAU | **100M+** (Dec 2025) |
| MAU | **159M** (Oct 2025) |
| China AI rank | **#1** (passed DeepSeek Aug 2025) |
| Global AI rank | #4 (behind ChatGPT, [[Gemini]], Copilot) |
| Parent | [[ByteDance]] |
| Marketing spend | Lowest of any ByteDance 100M DAU product |

China's AI usage leader — and did it with minimal marketing.

---

## Growth trajectory

| Date | MAU | Notes |
|------|-----|-------|
| Feb 2025 | 82M | #1 China AI app |
| Aug 2025 | 157M | Passed DeepSeek |
| Oct 2025 | 159M | 2x nearest competitor |
| Dec 2025 | **100M DAU** | Milestone with minimal spend |

**vs competitors (Oct 2025):**

| App | MAU |
|-----|-----|
| **Doubao** | **159M** |
| [[DeepSeek]] | 143M |
| Tencent Yuanbao | 73M |
| [[Alibaba]] [[Qwen]] | ~100M |
| Baidu Ernie | — |

Doubao leads by 2x over #3.

---

## Product capabilities

### Core chatbot

| Feature | Description |
|---------|-------------|
| Text generation | GPT-style conversation |
| Image understanding | Multimodal input |
| Code assistance | Programming help |
| Translation | Multiple languages |
| Voice | Real-time voice assistant |

### Voice assistant (Dec 2025)

**ByteDance launched real-time voice capabilities:**
- Natural conversation (low latency)
- ZTE smartphone prototype integration
- Hands-free operation
- Agentic task completion

---

## Agentic commerce (Dec 2025)

**Doubao now completes transactions:**

| Capability | Integration | Status |
|------------|-------------|--------|
| Ticket booking | [[Douyin]] | Live (Dec 2025) |
| Shopping | Douyin e-commerce | Live |
| Food ordering | Douyin local services | In progress |
| [[Travel]] booking | Douyin | In progress |

**ZTE partnership:**
- Prototype smartphone with Doubao pre-installed
- Voice-first AI assistant
- Can complete Douyin purchases via voice commands
- "Agentic AI smartphone" concept

See [[Douyin]] for e-commerce context.

---

## Why Doubao won

### Distribution advantage

| Factor | Impact |
|--------|--------|
| [[Douyin]] integration | 790M MAU funnel |
| [[ByteDance]] ecosystem | Lark, CapCut, [[TikTok]] |
| Recommendation algorithm | Best-in-class discovery |
| Mobile-first | Optimized for phone use |

**Key insight:** ByteDance didn't need to market Doubao — it leveraged existing distribution.

### Technical foundation

| Asset | Contribution |
|-------|--------------|
| $14B GPU investment | Training infrastructure |
| Recommendation AI expertise | Personalization |
| Content understanding | Video/image analysis from Douyin |
| Scale | Billions of daily interactions to learn from |

---

## AI infrastructure

**ByteDance 2026 AI investment:** ~¥160B ($23B) planned for infrastructure.

| Component | Details |
|-----------|---------|
| GPUs | $14B NVIDIA order (H200s) |
| Custom chips | Developing in-house |
| Data centers | Expanding capacity |
| Training data | Douyin content (massive scale) |

See [[ByteDance]] for GPU spending details.

---

## Competitive position

| Competitor              | Strength                | vs Doubao                 |
| ----------------------- | ----------------------- | ------------------------- |
| [[DeepSeek]]            | Reasoning, open weights | Doubao has distribution   |
| [[Alibaba]] [[Qwen]]        | Enterprise, open source | Doubao has consumer scale |
| [[Tencent]] [[Yuanbao]] | WeChat integration      | Doubao has 2x users       |
| [[Baidu]] [[Ernie]]     | Search integration      | Doubao more engaging      |
| [[ChatGPT]]             | Global leader           | Not available in China    |

**Why Doubao leads:**
1. Distribution (Douyin/ByteDance ecosystem)
2. Mobile-first design
3. Agentic commerce integration
4. Minimal marketing needed

---

## Investment implications

**Not directly investable** — Doubao is a ByteDance product (private).

**Indirect exposure:**
- [[NVIDIA]] — $14B GPU order
- [[SK Hynix]] — [[HBM]] for ByteDance infrastructure

**Thesis implications:**
- China AI usage consolidating around ByteDance
- Agentic commerce = real monetization path
- ByteDance AI spend benefits memory/GPU suppliers

---

## Open questions

- Can Doubao maintain lead as DeepSeek improves?
- Will agentic commerce drive meaningful revenue?
- How will export controls affect model quality vs US?
- Will Tencent's WeChat distribution catch up?

---

## Quick stats

| Metric | Value |
|--------|-------|
| Parent | [[ByteDance]] |
| DAU | 100M+ |
| MAU | 159M |
| China rank | #1 AI app |
| Global rank | #4 AI app |
| Launched | 2023 |
| Status | Product (not standalone) |

*Created 2026-01-21*

---

## Related

- [[ByteDance]] — parent company
- [[Douyin]] — distribution channel, commerce integration
- [[DeepSeek]] — competitor (#2 in China)
- [[Alibaba]] — competitor ([[Qwen]])
- [[Tencent]] — competitor (Yuanbao)
- [[NVIDIA]] — GPU supplier
- [[Agentic AI]] — category (commerce integration)
- [[China]] — market context

---

## Sources

- [TechNode: Doubao reaches 100M DAU](https://technode.com/2025/12/25/bytedances-doubao-reaches-100m-dau-with-minimal-marketing-spend/)
- [HowAIWorks: Doubao becomes China's leading chatbot](https://howaiworks.ai/blog/bytedance-doubao-china-leading-chatbot)
- [Statista: Top AI applications by MAU](https://www.statista.com/statistics/1609163/top-ai-applications-mau-worldwide/)

---
aliases: [Z, ZG, Zillow Group]
---
#actor #technology #realestate #usa #public

**Zillow** — \#1 real estate website by traffic. 227 million monthly users. Zestimate home valuations. Exited iBuying (2021). NASDAQ: Z, ZG.

---

## Why it matters

| Metric | Value |
|--------|-------|
| Monthly unique visitors | **227 million** |
| Market position | **\#1** real estate web traffic |
| 2024 revenue | $2.2B (+15% YoY) |
| Q1 2025 revenue | $598M (+13% YoY) |
| Ticker | Z (Class C), ZG (Class A) |

---

## Business model

| Segment | Description |
|---------|-------------|
| **Premier Agent** | Lead gen to real estate agents |
| **Rentals** | Rental listings marketplace |
| **Mortgages** | Zillow [[Home]] Loans |
| **New Construction** | Builder listings |

**Exited iBuying (Zillow Offers) in 2021** — massive write-down, refocused on marketplace.

---

## Zestimate

**Automated home valuations:**

| Metric | Accuracy |
|--------|----------|
| On-market homes | 1.83% median error |
| Off-market homes | 7.01% median error |

**Example:** For $500K home, off-market Zestimate could be off by $35K.

---

## 2025 housing market view

**Zillow economists forecast:**

| Metric | 2025 | 2026E |
|--------|------|-------|
| [[Home]] value growth | Flat | +1.2% |
| Existing home sales | ~4.09M | 4.26M (+4.3%) |
| Homes losing value | 53% | Improving |

**Mischa Fisher (Zillow Chief Economist):** "The housing market is finally settling into a healthier state."

---

## Competitive landscape

| Company | Focus |
|---------|-------|
| **Zillow** | \#1 traffic, consumer brand |
| [[CoStar Group]] (Homes.com) | \#2 traffic, aggressive growth |
| Realtor.com (News Corp) | NAR partnership |
| Redfin | Now part of [[Rocket Companies]] |

**Threat:** CoStar's Homes.com grew to \#2 with 111M monthly visitors.

---

## iBuying exit (2021)

| What happened | Impact |
|---------------|--------|
| Zillow Offers shut down | $500M+ write-down |
| Algorithmic buying failed | Couldn't price homes accurately |
| 25% workforce reduction | Refocused on core |

**Lesson:** Technology couldn't solve home pricing variability.

---

## Investment implications

**Bull case:**
- Dominant consumer brand (\#1 traffic)
- Housing recovery = more transactions
- Mortgage business growing
- Rental marketplace expansion

**Bear case:**
- CoStar/Homes.com competitive threat
- Agent commission pressure (NAR settlement)
- Housing volume depressed
- iBuying failure overhang

---

## Related

- [[CoStar Group]] — competitor (Homes.com)
- [[Rocket Companies]] — competitor (owns Redfin)
- [[Housing]] — sector context
- [[Housing affordability crisis]] — demand driver
- [[Mortgage rates]] — transaction volume driver

*Created 2026-01-16*

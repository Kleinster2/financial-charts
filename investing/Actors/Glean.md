---
aliases: []
---
#actor #ai #enterprise #usa #private

**Glean** — Enterprise AI search. $7.2B valuation. 100M+ agent actions/year. Ex-Google founders.

---

## Why Glean matters

Enterprise search + AI agents at scale:

| Metric | Value |
|--------|-------|
| Valuation | $7.2B (June 2025) |
| ARR | $100M+ |
| Total raised | $610M |
| Agent actions | 100M+/year |

---

## Funding trajectory

**Rapid valuation growth:**

| Date | Round | Valuation |
|------|-------|-----------|
| Early 2024 | — | $2.2B |
| Late 2024 | Series E | $4.6B |
| June 2025 | Series F | $7.2B |

**Series F investors:**
- Wellington Management (lead)
- [[Khosla Ventures]]
- [[Sequoia Capital]]
- [[Lightspeed Venture Partners]]
- Kleiner Perkins
- General Catalyst

---

## Product

**Enterprise AI search:**
- Unified search across all apps
- Google Workspace, Microsoft 365, Slack, Salesforce
- Personalized knowledge graph per user
- Natural language queries

**Glean Agents (2025):**
- AI agents for enterprise tasks
- 100M+ actions annually
- Workflow automation

---

## Founding team

**Ex-Google engineers:**
- Arvind Jain (CEO) — ex-Google
- Piyush Prahladka — ex-Google
- Tony Gentilcore — ex-Google
- T.R. Vishwanath — ex-Facebook

Founded 2019, Palo Alto.

---

## Growth metrics

| Metric | Value |
|--------|-------|
| ARR | $100M+ |
| Time to $100M | <3 years from launch |
| Agent actions | 100M+/year |

Crossed $100M ARR in last fiscal year.

---

## Competitive landscape

| Competitor | Approach | Glean vs |
|------------|----------|----------|
| Microsoft Copilot | M365-native | Multi-platform |
| Google Workspace AI | Google-native | Multi-platform |
| Elastic | Search infrastructure | Enterprise focus |

Glean = Switzerland (works across ecosystems).

---

## Investment case

**Bull:**
- Enterprise search essential
- AI agents growing
- Multi-platform advantage
- Strong investor syndicate
- Rapid ARR growth

**Bear:**
- Microsoft/Google could bundle
- $7.2B = high expectations
- Enterprise sales cycles
- Competition intensifying

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | Private |
| Valuation | $7.2B |
| ARR | $100M+ |
| Total raised | $610M |
| CEO | Arvind Jain |

*Updated 2026-01-01*

---

## Related

- [[Perplexity]] — peer (AI search)
- [[Microsoft]] — potential competitor (Copilot)
- [[Google]] — potential competitor (Workspace AI)
- [[Cohere]] — peer (enterprise AI)
- [[Harvey]] — peer (vertical AI)
- [[Sequoia Capital]] — investor
- [[Lightspeed Venture Partners]] — investor
- [[Agentic AI]] — thesis context (100M+ agent actions)

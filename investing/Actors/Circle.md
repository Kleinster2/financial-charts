---
tags:
  - actor
  - fintech
  - crypto
  - usa
  - public
aliases:
  - CRCL
  - Circle Internet Financial
---

**Circle** — Issuer of USDC stablecoin. Partner with [[Coinbase]]. Critical infrastructure for crypto and tokenization.

---

## Why Circle matters

Circle issues the second-largest stablecoin:

| Metric | Value |
|--------|-------|
| Ticker | CRCL ([[NYSE]]) |
| Product | USDC stablecoin |
| Market cap | ~$30B+ |
| Backing | USD, [[Treasuries]] |
| Partner | [[Coinbase]] (co-creator) |

---

## USDC stablecoin

Dollar on the blockchain:
- 1 USDC = 1 USD (pegged)
- Backed by cash and short-term [[Treasuries]]
- Redeemable for dollars
- Used for trading, DeFi, payments

vs USDT ([[Tether]]):

| Aspect | USDC | USDT |
|--------|------|------|
| Issuer | Circle | [[Tether]] |
| Transparency | High (audited) | Lower |
| Size | \#2 (~$75B) | \#1 (~$120B) |
| Trust | Higher | Questions |

---

## Business model

| Revenue | Description |
|---------|-------------|
| Interest income | Earn yield on reserves |
| Enterprise services | APIs, treasury management |
| Cross-border payments | Remittances |

Circle earns interest on USDC reserves ([[Treasuries]]) — higher rates = more revenue.

---

## Coinbase partnership

Centre Consortium (dissolved 2023):
- Circle and [[Coinbase]] co-created USDC
- [[Coinbase]] earns share of USDC revenue
- Deep integration with [[Coinbase]] exchange
- Circle now sole issuer

---

## Tokenization relevance

Infrastructure for tokenized assets:
- Settlement in USDC
- Stablecoin payments for tokens
- On/off ramp to traditional finance

See [[Tokenized private shares]].

---

## GENIUS Act Impact (July 2025)

The passage of the [[GENIUS Act]] was the "IPO Unlock" Circle needed:
- Legitimacy: USDC effectively became a US-sanctioned digital dollar format
- Compliance: mandated 100% reserve backing (Circle was already compliant)
- Advantage: puts pressure on non-compliant offshore rivals like [[Tether]] (USDT)
- IPO unlock: Circle IPO'd June 2025 at $31/share, trading as CRCL on [[NYSE]]

---

## IPO (June 2025)

Circle IPO'd in June 2025 at $31/share after the [[GENIUS Act]] passage provided regulatory clarity. The 2022 SPAC deal ($9B target) had fallen through. Trading as CRCL on [[NYSE]].

---

## Investment case

Bull:
- Now public (CRCL), liquid
- [[GENIUS Act]] regulatory moat
- Stablecoin market ~$300B and growing
- Interest income scales with USDC growth
- Payments use case expanding
- [[Meta]]/[[Stripe]] stablecoin integration could be largest USDC demand catalyst since IPO

Bear:
- [[Tether]] still dominates
- Interest rate sensitivity (down = less revenue)
- Competition ([[PayPal]] PYUSD, bank stablecoins)
- Regulatory implementation uncertainty ([[GENIUS Act]] requirements due Jul 2026)

---

## Cap table / Investors

| Round | Date | Amount | Valuation |
|-------|------|--------|-----------|
| Seed-Series D | 2013-2018 | $250M+ | — |
| Series E | 2021 | $440M | $4.5B |
| SPAC (cancelled) | 2022 | — | $9B target |
| Total | | $700M+ | |

Key investors:

| Investor | Notes |
|----------|-------|
| [[Goldman Sachs]] | Series D lead |
| IDG Capital | Early |
| Breyer Capital | Early |
| [[General Catalyst]] | Growth |
| [[Fidelity]] | Growth |
| [[Coinbase]] | Partner (USDC co-creator) |

Total raised: $700M+

SPAC deal ($9B) fell through 2022. IPO'd June 2025 at $31/share (CRCL, [[NYSE]]).

## Financials

| Metric | Value |
|--------|-------|
| Status | Public (CRCL, [[NYSE]]) |
| IPO | June 2025 at $31/share |
| USDC in circulation | ~$75.3B |
| FY2025 revenue | $2.7B |
| Q4 2025 revenue | $770M |
| Founded | 2013 |
| CEO | Jeremy Allaire |
| HQ | Boston |

---

## Meta / Stripe stablecoin integration (Feb 2026)

[[Meta]] is testing stablecoin-based payments across Facebook, Instagram, and WhatsApp for H2 2026 launch. [[Stripe]] (via its Bridge subsidiary) is the leading infrastructure partner. Bridge received conditional OCC national trust bank charter in Feb 2026.

**Why this matters for Circle:** Bridge's stablecoin orchestration likely routes through USDC as the primary regulated, US-compliant stablecoin. If [[Meta]]'s 3.5B daily active users get stablecoin payments — even for creator payouts and cross-border transfers — USDC circulation could surge, directly increasing Circle's reserve-based interest income.

**Scale of opportunity:** Meta's creator economy runs ~$50B annually across platforms. Even a fraction denominated in USDC would be material relative to current ~$30B USDC market cap.

**Competitive risk:** Bridge could also support other stablecoins, and [[Meta]] hasn't disclosed which tokens will be integrated. [[PayPal]] PYUSD and potential bank-issued stablecoins under the [[GENIUS Act]] framework are alternatives.

Source: [Bloomberg](https://www.bloomberg.com/news/articles/2026-02-24/meta-testing-stablecoin-payments-as-digital-currencies-take-off)

---

## Q4 2025 earnings (Feb 25, 2026)

First earnings report as a public company. Blowout results drove a **+35.5% two-day rally** (Feb 25-26) to $83.14.

### Q4 highlights

| Metric | Q4 2025 | Y/Y |
|--------|---------|-----|
| Revenue + reserve income | **$770M** | +77% |
| Net income | $133M | +$129M |
| Adj EBITDA | $167M | +412% |
| USDC in circulation | **$75.3B** | +72% |
| USDC onchain txn volume | $11.9T | +247% |
| On-platform USDC | $12.5B | +5.6x |

### FY2025

| Metric | FY2025 | Y/Y |
|--------|--------|-----|
| Revenue + reserve income | **$2.7B** | +64% |
| Net loss (continuing ops) | -$70M | (IPO SBC: $424M) |
| Adj EBITDA | $582M | +104% |

### Business momentum

- **Circle Payments Network (CPN):** 55 financial institutions enrolled, 74 in review. Annualized txn volume $5.7B (trailing 30-day as of Feb 20)
- **Arc blockchain:** Public testnet live, 100+ participants, 2.3M daily txns, half-second finality. Mainnet on track for 2026
- **EURC:** €310M in circulation (+284% Y/Y)
- **OCC national trust charter:** Conditional approval received Dec 2025
- **Visa:** US issuers/acquirers can now settle with Visa using USDC
- **[[Intuit]]:** Multi-year strategic partnership for USDC integration across platform
- **Polymarket:** USDC as core collateral/settlement asset
- **Bermuda:** Plans to become world's first fully onchain national economy using Circle infrastructure

### Regulatory context

- [[GENIUS Act]] implementation: regulators finalizing licensing/custody/compliance requirements by July 18, 2026
- [[CFTC]] pilot: USDC approved as derivatives margin collateral (Dec 2025)

---

## Related

- [[Coinbase]] — partner, USDC co-creator
- [[Meta]] — testing stablecoin payments (H2 2026), potential USDC demand catalyst
- [[Stripe]] — Bridge subsidiary is Meta's likely stablecoin infrastructure partner; OCC charter (Feb 2026)
- [[Tether]] — USDT competitor (#1 vs Circle's #2)
- [[GENIUS Act]] — regulatory framework, implementation July 2026
- [[Crypto]] — industry context
- [[PayPal]] — competitor (PYUSD)
- [[Tokenized private shares]] — infrastructure


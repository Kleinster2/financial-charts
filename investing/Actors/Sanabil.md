---
aliases: [Sanabil Investments, Saudi Sanabil]
---
#actor #cvc #swf #saudi

**Sanabil** — Saudi PIF's venture/growth arm. **$8B capital base**, ~$3B annual deployment. 87 investments including [[World Labs]], Lucid, Stripe. 50% VC, 30% PE allocation.

---

## Why Sanabil matters

| Metric | Value |
|--------|-------|
| Parent | PIF (Saudi Public Investment Fund) |
| Capital base | **$8B** |
| Annual deployment | ~**$3B** |
| Portfolio companies | **87** |
| HQ | Riyadh |
| Allocation | 50% VC, 30% PE, 20% liquid |

PIF's tech-focused arm — deploying Saudi oil wealth into global startups.

---

## Asset allocation (Dec 2024)

| Category | Allocation |
|----------|------------|
| **Venture Capital** | 50% |
| Private Equity | 30% |
| Liquid portfolio | 20% |
| Cash | 0% |

Heavy VC tilt = aggressive growth orientation.

---

## Notable investments

| Company | Sector | Notes |
|---------|--------|-------|
| [[World Labs]] | Spatial AI | Nov 2025 (latest) |
| Lucid Motors | EVs | Major stake |
| Stripe | Fintech | — |
| HALA | Saudi fintech | $157M Series B lead (Sep 2025) |
| Cruise | Autonomous vehicles | — |
| ByteDance | Social/AI | — |

---

## Investment strategy

| Focus | Approach |
|-------|----------|
| Stage | Venture, growth, small buyouts |
| Risk | Moderate risk, high returns |
| Geography | Global (US, Europe, MENA) |
| Sectors | Tech, fintech, mobility, AI |

**Independence:** Operates separately from PIF's direct investments with specialized strategy.

---

## PIF context

| Metric | Value |
|--------|-------|
| PIF AUM target (2025) | $1.07T |
| PIF AUM target (2030) | $2T |
| Saudi VC activity Q1 2025 | $391M (+53% YoY) |

Sanabil is PIF's vehicle for venture-scale bets while PIF does mega-deals (NEOM, Lucid, etc.).

---

## Regional role

**Saudi startup ecosystem:**
- Sanabil = anchor LP/investor for regional funds
- HALA investment = largest Saudi fintech Series B
- Catalyzes local VC market

---

## Investment implications

**Not directly investable** — subsidiary of PIF.

**Signals:**
- Sanabil investment = Saudi sovereign validation
- Co-investment opportunities with PIF
- MENA startups gaining global capital access

**Related Saudi vehicles:**
- PIF (direct, mega-deals)
- [[Mubadala]] (Abu Dhabi, peer)
- [[ADQ]] (Abu Dhabi, peer)

---

## Quick stats

| Metric | Value |
|--------|-------|
| Parent | PIF (Saudi Arabia) |
| Capital | $8B |
| Annual deploy | ~$3B |
| Portfolio | 87 companies |
| Allocation | 50% VC |

*Created 2026-01-27*

---

## Related

- [[World Labs]] — portfolio company
- [[Mubadala]] — peer (Abu Dhabi)
- [[ADQ]] — peer (Abu Dhabi)
- [[ADIA]] — peer (Abu Dhabi)
- [[Abu Dhabi Sovereign Wealth]] — regional context
- [[NVentures]] — peer CVC
- [[AMD Ventures]] — peer CVC
- [[Databricks Ventures]] — peer CVC

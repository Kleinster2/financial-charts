---
aliases: [EQIX]
---
#actor #reit #datacenter #usa

**Equinix** — Largest data center REIT. Interconnection-focused. Global footprint.

---

## Why Equinix matters

Equinix is the largest publicly traded data center company:

| Metric | Value |
|--------|-------|
| Market cap | ~$80B+ |
| Data centers | 260+ facilities |
| Markets | 70+ metros, 30+ countries |
| Model | Colocation + interconnection |

---

## Business model

**Colocation:**
- Customers rent space, power, cooling
- Bring their own servers
- Multi-tenant facilities
- Enterprise and cloud customers

**Interconnection (key differentiator):**
- Customers connect to each other inside Equinix
- Cloud on-ramps (AWS, Azure, GCP)
- Network density creates moat
- Higher margin than pure colo

---

## Interconnection moat

**Why interconnection matters:**
- Once inside Equinix, customers connect to partners
- [[Network effects]] — more customers = more valuable
- Switching costs increase over time
- Premium pricing for connectivity

**Revenue mix:**
- ~20% from interconnection
- Higher margin than colocation
- Recurring, sticky revenue

---

## AI exposure

**Mixed picture:**
- Traditional colo less suited for AI (power density)
- Building xScale (hyperscale JV) for AI workloads
- Interconnection valuable for AI inference
- Not primary AI infrastructure play

**xScale JV:**
- Partnership with [[GIC]] (Singapore sovereign wealth)
- Hyperscale-focused facilities
- Better suited for AI workloads

---

## Competitive position

| Competitor | Focus | Equinix advantage |
|------------|-------|-------------------|
| [[Digital Realty]] | Hyperscale wholesale | Interconnection density |
| [[QTS]] ([[Blackstone]]) | Hyperscale | Global footprint |
| [[Coresite]] | US regional | Scale |

Equinix wins on interconnection and global reach.

---

## Geographic footprint

| Region | Presence |
|--------|----------|
| Americas | Major metros |
| EMEA | Strong ([[EU]] data sovereignty) |
| [[Asia]]-Pacific | Expanding |

Global footprint = enterprise appeal.

---

## Investment case

**Bull:**
- Interconnection moat
- Global scale
- Enterprise cloud migration tailwind
- REIT = dividend
- Stable, recurring revenue

**Bear:**
- Traditional colo less AI-relevant
- Lower power density than hyperscale
- Premium valuation
- Interest rate sensitivity (REIT)

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | EQIX |
| Market cap | ~$80B+ |
| Data centers | 260+ |
| Countries | 30+ |
| Structure | REIT |

*Updated 2026-01-01*

---

## Related

- [[Digital Realty]] — competitor (DC REIT, wholesale vs retail)
- [[Coresite]] — competitor (US regional interconnection)
- [[DigitalBridge]] — adjacent (digital infrastructure investor)
- [[Blackstone]] — competitor ([[QTS]] platform)
- [[AI hyperscalers]] — customers (interconnection value)
- [[AI infrastructure financing]] — thesis context
- [[Ashburn]] — major presence
- [[Bogotá Data Hub]] — operator (since 2017)

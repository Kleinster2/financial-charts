---
aliases: [TOK, 4186.T]
---
#actor #semiconductor #materials #japan #public #photoresist

**Tokyo Ohka Kogyo (TOK)** — #2 global photoresist maker. The largest publicly traded pure-play in the space. Aggressive capacity expansion targeting 2nm nodes.

---

## Why TOK matters

With [[JSR Corporation]] now private, TOK is the best public way to play the photoresist chokepoint:

| Metric | Value |
|--------|-------|
| Ticker | 4186.T (Tokyo) |
| Market cap | ~¥500B (~$3.3B) |
| Global photoresist share | ~20-25% |
| EUV share | Top 3 (with JSR, Shin-Etsu = 90%) |
| Primary business | >50% revenue from electronic materials |

---

## Business mix

| Segment | % Revenue | Products |
|---------|-----------|----------|
| **Electronic functional materials** | >50% | [[Photoresists]], packaging materials |
| High-density packaging | ~15% | [[Advanced packaging]] materials |
| LCD/Display | ~15% | Display photoresists |
| Other | ~20% | Equipment, process materials |

---

## Product portfolio

| Type | Position | Trend |
|------|----------|-------|
| **EUV photoresists** | Top 3 | +35% sales growth (2024) |
| **ArF immersion** | Strong | +15% target (2024) |
| **KrF** | Market leader | Stable demand |
| **High-density packaging** | Growing | Chiplet/[[HBM]] tailwind |

---

## Capacity expansion

**[[Japan]] facility (largest domestic):**
- Investment: >¥200B
- Construction: Started H2 2024
- Production: H2 2026
- Products: KrF and EUV photoresists

**South Korea facility:**
- Investment: ¥20B ($130M)
- [[Target]] customers: [[Samsung]], [[SK Hynix]]
- Operations: 2030
- Capacity: 3-4x current Korea production

**Intel partnership:**
- July 2023: Collaborative development agreement
- Focus: Sub-2nm photoresists
- Significance: Validates TOK's technology leadership

---

## 2024-2025 targets

| Metric | [[Target]] |
|--------|--------|
| Advanced materials sales | +15% |
| EUV photoresist sales | +35% |
| Driver | AI chip demand |

---

## Competitive position

**[[Japan]] dominance:** JSR + TOK + Shin-Etsu = 90%+ of EUV photoresists

| Company | Share | Status |
|---------|-------|--------|
| [[JSR Corporation]] | ~27% | Private |
| **TOK** | ~20-25% | **Public** |
| [[Shin-Etsu]] | ~15% | Public (diversified) |

**TOK advantage:** Largest pure-play photoresist company still publicly traded.

---

## Customers

- [[Samsung]] — memory and foundry
- [[SK Hynix]] — memory (Korea expansion)
- [[Intel]] — development partner
- [[TSMC]] — leading-edge nodes
- All major fabs globally

---

## Investment case

**Bull:**
- Only large-cap public pure-play photoresist
- EUV demand structural (every leading node needs it)
- [[Japan]] monopoly position defensible
- Capacity expansion positions for 2nm wave
- Intel partnership validates technology

**Bear:**
- Cyclical (follows chip cycle)
- Yen exposure
- JSR (private) and Shin-Etsu (diversified) competition
- Technology risk (metal-oxide resists)
- Customer concentration

---

## Valuation context

Pure-play scarcity premium — with JSR private, TOK is the cleanest public exposure to photoresist growth.

---

*Updated 2026-01-20*

---

## Related

- [[Long Japan photoresists]] — investment thesis (TOK as primary vehicle)
- [[JSR Corporation]] — #1 competitor (private)
- [[Shin-Etsu]] — #3 competitor (wafers + photoresists)
- [[Fujifilm]] — diversified competitor
- [[Sumitomo Chemical]] — growing competitor
- [[Samsung]] — major customer
- [[SK Hynix]] — major customer (Korea expansion)
- [[Intel]] — development partner
- [[Export controls]] — photoresists as chokepoint
- [[Japan Semiconductor Revival]] — national context

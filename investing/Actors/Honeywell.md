---
aliases: [HON]
---
#actor #industrial #automation #usa #public

**Honeywell** — Diversified industrial conglomerate. Building automation, aerospace, process controls. [[Quantum computing]] investment via Quantinuum.

---

## Why Honeywell matters

| Metric | Value |
|--------|-------|
| Ticker | HON (NASDAQ) |
| Market cap | ~$140B |
| Position | Top 3 industrial conglomerate |
| Segments | 4 major divisions |

---

## Business segments

| Segment | Revenue | Focus |
|---------|---------|-------|
| [[Aerospace]] | ~35% | Avionics, engines, defense |
| Building Technologies | ~25% | HVAC, fire, security, controls |
| Performance Materials | ~20% | Chemicals, materials |
| Safety & [[Productivity]] | ~20% | Warehouse automation, PPE |

---

## AI/Data center relevance

**Building automation for data centers:**

| Product | DC Application |
|---------|----------------|
| Building management systems | HVAC optimization |
| Fire/life safety | DC protection |
| Energy management | Power efficiency |
| Industrial software | Operations optimization |

Honeywell Forge platform uses AI for building optimization.

---

## Quantinuum ([[Quantum computing]])

**Joint venture with Cambridge Quantum:**

| Metric | Value |
|--------|-------|
| Ownership | Honeywell majority |
| Technology | Trapped-ion quantum |
| Customers | Honeywell, [[Airbus]], [[BMW]], [[HSBC]], JPMorgan |
| Last valuation | $10B (Sep 2025) |
| **IPO target** | **$20B+** (Jan 2026 filing) |

Leading commercial quantum computing company.

### IPO filing (Jan 14, 2026)

| Detail | Value |
|--------|-------|
| Filing | Confidential S-1 to SEC |
| [[Target]] valuation | **$20B+** |
| Expected raise | ~$1B |
| Timing | Q1-Q2 2026 |

**Significance:** First major quantum computing IPO. Validates commercial quantum thesis.

**Stock reaction:** HON +1.6% on news.

---

## [[Aerospace]] strength

| Product | Position |
|---------|----------|
| Avionics | Major supplier |
| APUs | Market leader |
| Engines | Turboprops, helicopters |
| [[Defense]] | Guidance, navigation |

Benefits from commercial aviation recovery.

---

## Investment case

**Bull:**
- [[Quantum computing]] optionality
- Building automation growth
- [[Aerospace]] recovery
- Margin expansion potential
- Software/recurring revenue growth

**Bear:**
- Conglomerate discount
- Cyclical end markets
- Competition from pure-plays
- Pension liabilities

---


![[hon-employees-chart.png]]
*Headcount: 101,000 (2025) — down 1.0% YoY*

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | HON |
| Market cap | ~$140B |
| Dividend yield | ~2% |
| HQ | Charlotte |

*Updated 2026-01-17*

---

## Related

- [[Quantinuum]] — quantum computing unit (IPO filing Jan 2026)
- [[Rockwell Automation]] — peer (industrial automation)
- [[Siemens]] — peer (building tech, industrial)
- [[Microsoft]] — partner (quantum via Azure)
- [[Power constraints]] — context (DC building management)

---
aliases: [Infineon Technologies]
---
#actor #semiconductor #germany #automotive #power

**Infineon** — German semiconductor company. #1 globally in automotive and power semiconductors. European champion.

---

## Why Infineon matters

Infineon is the global leader in two critical categories:

| Role | Details |
|------|---------|
| Auto semis | #1 globally |
| Power semis | #1 globally |
| EV exposure | Major beneficiary |
| European | German champion |

---

## Market position

| Segment | Position | Share |
|---------|----------|-------|
| Automotive semiconductors | #1 globally | ~13% |
| Power semiconductors | #1 globally | ~20% |
| Security ICs | Top 3 | — |
| Industrial | Top 5 | — |

Double leadership in auto + power = unique position for EV transition.

---

## Product portfolio

**Power semiconductors:**

| Technology | Applications |
|------------|--------------|
| IGBTs | EV inverters, industrial drives |
| MOSFETs | Power management |
| SiC | High-efficiency EVs |
| GaN | Fast chargers, data centers |

**Automotive:**

| Product | Applications |
|---------|--------------|
| MCUs | Engine control, body |
| Power | EV drivetrain |
| Sensors | ADAS, radar |
| Security | V2X, authentication |

---

## EV transition beneficiary

**More content per EV:**

| Vehicle type | Infineon content |
|--------------|------------------|
| ICE vehicle | ~$50 |
| Hybrid | ~$150 |
| BEV | ~$200+ |

EVs = 4x the semiconductor content of ICE. Infineon's power focus = maximum leverage.

---

## Key customers

| Customer | Products |
|----------|----------|
| [[Tesla]] | Power semiconductors |
| VW Group | Full portfolio |
| BMW | Power, MCUs |
| Toyota | Power, sensors |
| Tier 1s | Bosch, Continental, Denso |

Broad OEM and Tier 1 relationships.

---

## Manufacturing

**Owned fabs (power focus):**

| Location | Focus |
|----------|-------|
| Dresden, Germany | Power, automotive |
| Villach, Austria | Power (new 300mm) |
| Kulim, Malaysia | Power (expanding) |
| Regensburg, Germany | Automotive |

Power semis less node-dependent = own fabs viable.

**Foundry partners:** [[TSMC]], [[GlobalFoundries]] for advanced logic.

---

## Competitive position

| vs | Infineon advantage | Infineon disadvantage |
|----|--------------------|-----------------------|
| [[STMicro]] | Auto breadth, power scale | SiC specific, MEMS |
| [[NXP]] | Power semis, MCUs | Radar, connectivity |
| [[Renesas]] | Power, European presence | Japan auto relationships |
| ON Semi | Scale, breadth | SiC focus |
| Texas Instruments | Auto focus | Analog breadth |

---

## Cypress acquisition (2020)

**$10B deal added:**

| Capability | Value |
|------------|-------|
| Automotive MCUs | Expanded portfolio |
| Memory (NOR flash) | Automotive storage |
| Connectivity | Wi-Fi, Bluetooth |
| USB-C | Consumer/industrial |

Made Infineon more complete in automotive.

---

## Financial position

| Metric | Value |
|--------|-------|
| Ticker | IFX (Frankfurt), IFNNY (OTC) |
| Market cap | ~€45B (~$50B) |
| Revenue (FY24) | ~€15B |
| Automotive revenue | ~50% |
| Power & sensor revenue | ~30% |
| Gross margin | ~42% |
| P/E | ~15x |

---

## Investment implications

**Bull case:**
- #1 in two growing markets
- EV transition = secular tailwind
- European strategic asset
- Cypress integration complete

**Bear case:**
- Auto cycle exposure
- China EV competition
- SiC competition heating up
- European cost structure

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | IFX |
| Market cap | ~$50B |
| Revenue | ~€15B |
| Auto semi rank | #1 globally |
| Power semi rank | #1 globally |
| EV content | ~$200/vehicle |

*Updated 2026-01-04*

---

## Related

- [[Sensors]] — automotive radar, sensors
- [[STMicro]] — competitor (European auto/power)
- [[NXP]] — competitor (#2 automotive)
- [[Renesas]] — competitor (auto MCUs)
- [[Tesla]] — customer (power semis)
- [[Toyota]] — customer
- [[TSMC]] — foundry partner
- [[GlobalFoundries]] — foundry partner
- [[Automotive semiconductors]] — market context
- [[Germany]] — home country
- [[EU Chips Act]] — European semiconductor policy
- [[Europe]] — regional hub

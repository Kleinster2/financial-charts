---
aliases: [Unity, U]
---
#actor #company #gaming #software #usa #public

**Unity Technologies** — Game engine platform. NYSE: U. Powers 70% of mobile games. Founded 2004.

---

## Why Unity matters

| Metric | Value |
|--------|-------|
| Ticker | U (NYSE) |
| Market cap | ~$10B |
| Revenue | ~$2B |
| MAUs (games) | 3.6B+ |
| HQ | San Francisco |

Dominant game development platform.

---

## Business model

| Segment | Description |
|---------|-------------|
| **Create** | Game engine, subscriptions |
| **Grow** | Ads, monetization |
| **Industry** | Non-gaming (auto, film) |

Engine + monetization ecosystem.

---

## Market position

| Metric | Value |
|--------|-------|
| Mobile games | 70%+ use Unity |
| Total games | 50%+ of new games |
| Developers | Millions |
| Platforms | 20+ |

---

## Unity vs. Unreal

| Aspect | Unity | Unreal |
|--------|-------|--------|
| Owner | Unity | Epic Games |
| Strength | Mobile, indie | AAA, visuals |
| Pricing | Subscription | Revenue share |
| Learning | Easier | Steeper curve |

Different market segments.

---

## 2023 pricing controversy

| Event | Impact |
|-------|--------|
| Runtime fee | Proposed per-install fee |
| Backlash | Developer revolt |
| Result | Walked back, CEO resigned |
| New CEO | Matt Bromberg (2024) |

Major self-inflicted wound.

---

## Growth drivers

| Driver | Description |
|--------|-------------|
| Mobile gaming | Continued growth |
| Non-gaming | Auto, architecture, film |
| AI tools | Content generation |
| VR/AR | Spatial computing |

---

## Investment case

**Bull:**
- Platform dominance
- Non-gaming expansion
- New management
- AI opportunity

**Bear:**
- Trust damaged
- Unreal competition
- Profitability challenges
- Gaming cyclicality

---

## Q4 2025 / Feb 2026 crash

**Q4 2025 results (Feb 11, 2026):**

| Metric | Q4 2025 | YoY |
|--------|---------|-----|
| Revenue | $503.1M | +10% |
| Adj EPS | $0.24 | Beat $0.21 est |
| Create Solutions | $165M | +8% |
| Grow Solutions | $338M | +11% |

**Q1 2026 guidance (the miss):** Revenue $480-490M (below $492M est), EBITDA $105-110M (below $116.9M est).

**Stock -26% premarket (Feb 12).** Down 52% YTD. SaaSpocalypse + [[Genie 3]] fear.

[[Google]]'s [[Genie 3]] — an AI tool generating interactive 3D environments from text prompts — threatens Unity's core game engine business. This is [[AI disintermediation]] hitting gaming infrastructure.

Vector platform targeting $1B quarterly run rate by end-2026. Refinanced $690M of 2026 convertibles to 2030. Cash >$2B.

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | U (NYSE) |
| Market cap | ~$5B (post-crash) |
| Revenue | ~$2B |
| Focus | Game engine |

---

## Related

- [[Roblox]] — customer/peer
- [[Electronic Arts]] — customer
- [[Gaming]] — sector
- [[Genie 3]] — AI threat to game engine model
- [[Claude Cowork disruption February 2026]] — SaaSpocalypse context

---

*Updated 2026-02-12*

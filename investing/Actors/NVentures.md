---
aliases: [NVIDIA Ventures, Nvidia Ventures]
---
#actor #cvc #ai

**NVentures** — [[NVIDIA]]'s corporate venture arm. **59 portfolio companies**, 31 investments in 2025 alone. Backs AI infrastructure, quantum, robotics. Key investments: [[Anthropic]], Cohere, Cursor, PsiQuantum.

---

## Why NVentures matters

| Metric | Value |
|--------|-------|
| Parent | [[NVIDIA]] |
| Portfolio companies | **59** |
| 2025 investments | **31** (vs 21 in 2024) |
| Unicorns created | 12 |
| HQ | Santa Clara |
| Focus | AI, quantum, robotics |

NVIDIA's strategic arm — aligns startup ecosystem with GPU dominance.

---

## Notable investments (2025-2026)

| Company | Round | Valuation | Date |
|---------|-------|-----------|------|
| [[Anthropic]] | Strategic | — | Nov 2025 (up to $10B) |
| Cursor | Series D | $29.3B | Nov 2025 |
| Cohere | Series D | $6.8B | Aug 2025 |
| PsiQuantum | Series E | $7B | Mar 2025 |
| Black Forest Labs | — | $3.25B | Dec 2025 |
| [[World Labs]] | Seed/A | $1B → $5B | 2024-2026 |
| Synthesia | Series E | — | Jan 2026 |

---

## Investment focus

| Sector | Examples |
|--------|----------|
| **AI/ML** | Anthropic, Cohere, Cursor |
| **Quantum** | PsiQuantum, QuEra, Quantinuum |
| **Robotics** | Flexion Robotics |
| **Enterprise** | N8n, PolyAI |
| **Life sciences** | Relation Therapeutics, Abridge |

**54 Enterprise (B2B)** companies, **27+ AI-native** companies.

---

## 2025 activity

| Metric | Value |
|--------|-------|
| NVentures deals | 31 |
| NVIDIA total deals | 67 (including non-NVentures) |
| vs 2024 | +48% (NVentures), +24% (total) |

**European push:** Black Forest Labs, N8n, PolyAI, Synthesia — NVIDIA ramping European AI investments.

---

## Quantum pivot (2025)

**Signaled strategic shift:**

| Company | Modality | Round |
|---------|----------|-------|
| PsiQuantum | Photonic | $1B Series E |
| QuEra | Neutral atom | $230M |
| Quantinuum | Trapped ion | — |

After years of quantum skepticism, NVIDIA now backs all three major hardware approaches.

---

## Strategic value

**For startups:**
- Access to NVIDIA technical talent
- GPU allocation priority
- Marketing/PR support
- Ecosystem connections

**For NVIDIA:**
- Early access to AI innovations
- Ensures ecosystem builds on CUDA/GPUs
- Strategic intelligence on market trends
- Prevents competitors from capturing key startups

---

## Investment implications

**Not directly investable** — subsidiary of [[NVIDIA]].

**Signals:**
- NVentures investments = NVIDIA validation
- Portfolio companies likely NVIDIA GPU customers
- Quantum investments signal long-term roadmap

---

## Quick stats

| Metric | Value |
|--------|-------|
| Parent | [[NVIDIA]] |
| Portfolio | 59 companies |
| 2025 deals | 31 |
| Unicorns | 12 |
| Focus | AI, quantum, robotics |

*Created 2026-01-27*

---

## Related

- [[NVIDIA]] — parent company
- [[Anthropic]] — portfolio ($10B commitment)
- [[World Labs]] — portfolio (spatial intelligence)
- [[AMD Ventures]] — peer CVC
- [[Adobe Ventures]] — peer CVC
- [[Databricks Ventures]] — peer CVC

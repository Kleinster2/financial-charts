# David Luan

#actor #founder #usa #ai #talent

**David Luan** — VP of Autonomy at [[Amazon]], leads AGI SF Lab. Former CEO of [[Adept]] ($1B valuation, acqui-hired by Amazon June 2024). Ex-OpenAI VP of Engineering. Building AI agents for digital and physical worlds.

---

## Key facts

| Metric | Value |
|--------|-------|
| Current role | VP Autonomy, Head of AGI SF Lab |
| Company | [[Amazon]] |
| Previous | CEO, [[Adept]] |
| Before that | VP Engineering, [[OpenAI]] |
| Reports to | Rohit Prasad (Amazon AGI head) |

---

## Career path

Elite AI trajectory:

| Company | Role | Period |
|---------|------|--------|
| Dextro | CEO/Founder | 2011-2017 |
| [[OpenAI]] | VP of Engineering | 2018-2020 |
| Google | Director | 2020-2021 |
| [[Adept]] | CEO/Founder | 2022-2024 |
| [[Amazon]] | VP Autonomy, AGI SF Lab | 2024- |

---

## Adept story

**Founded 2022:** Built AI agents that use software like humans — controlling mouse, keyboard, web browsers.

| Metric | Value |
|--------|-------|
| Total raised | $415M |
| Peak valuation | $1B |
| Series B | $350M (March 2023) |
| Key investors | Greylock, General Catalyst, [[NVIDIA]] |

**Why it ended:** Needed more capital, couldn't raise at higher valuation. Competed against OpenAI, Anthropic, Google all building agents.

**Amazon deal (June 2024):** Acqui-hire structure — Amazon licensed tech, hired team, avoided M&A regulatory scrutiny. Investors roughly broke even.

---

## Amazon AGI SF Lab

**Mission:** Build agents for digital and physical worlds.

| Capability | Detail |
|------------|--------|
| Computer use | Control mouse/keyboard |
| Web browsing | Navigate browsers |
| Code execution | Run and debug code |
| Complex workflows | Multi-step automation |

**Team:** Seeded by Adept employees, expanding to dozens of researchers from physics, math, quantitative finance.

**Collaborators:** Working with Pieter Abbeel (Covariant co-founder) on the project.

---

## Investment relevance

**Why track Luan:**
1. **Agent expertise** — Pioneered the category at Adept
2. **Amazon AI signal** — His lab direction = Amazon's agent strategy
3. **Talent magnet** — Elite AI researchers follow him
4. **OpenAI alumni** — Part of diaspora building elsewhere

---

## Related

### Current
- [[Amazon]] — employer, AGI SF Lab
- Rohit Prasad — reports to (Amazon AGI head)

### Previous
- [[Adept]] — founded, acqui-hired by Amazon
- [[OpenAI]] — VP Engineering

### Investors in Adept
- [[NVIDIA]] — strategic investor
- [[Andrej Karpathy]] — angel investor

### Category
- [[Agentic AI]] — category he helped pioneer

---

*Created 2026-01-14*

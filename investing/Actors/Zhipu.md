---
aliases: [Zhipu AI, 智谱AI, Knowledge Atlas Technology, Z.AI, 2513.HK]
---
#actor #ai #china #models #public

Zhipu — Chinese AI model company. First major Chinese AI startup to IPO (Jan 8, 2026). Hong Kong listed.

![[zhipu-price-chart.png]]
*IPO'd at HK$116.20 (Jan 8). Hit record HK$496 on Feb 13 (+327% from IPO). Chart data lags — Feb spike not yet captured by yfinance.*

---

## Why Zhipu matters

First Chinese AI lab to go public:

| Metric | Value |
|--------|-------|
| Full name | Knowledge Atlas Technology JSC Ltd. |
| Ticker | 2513.HK ([[HKEX]]) |
| Market cap | ~$22B (Feb 13) |
| IPO date | January 8, 2026 |
| IPO valuation | $6.6B |
| Founders | Tang Jie, Li Juanzi (Tsinghua professors) |

Tang Jie on US gap (Jan 2026): "The real answer is that the gap may actually be widening."

---

## Product: GLM models

ChatGLM series — open-source foundation models, bilingual (Chinese/English).

### GLM-5 (Feb 11, 2026)

| Metric | GLM-5 | GLM-4.7 |
|--------|-------|---------|
| Parameters | 744B | 355B |
| Training data | 28.5T tokens | — |
| Architecture | DeepSeek Sparse Attention | — |
| License | Open-source | — |

Claims industry-leading scores for open models in coding and agentic tasks, surpassing [[Google]] DeepMind's Gemini 3 Pro on internal benchmarks. Claims to approach [[Claude]] Opus 4.5 on coding. Topped open-source leaderboard on Artificial Analysis.

Milestone: trained GLM-Image entirely on [[Huawei]] Ascend chips — first Chinese AI company to train a major model fully on domestic silicon. [[DeepSeek]] has struggled with Huawei hardware.

Framing shift: Zhipu describes GLM-5 as moving from "vibe coding" to "agentic engineering" — AI-automated coding at larger scale.

Stock surged ~120% in the week of launch (Feb 10-13), hitting record HK$496 (+327% from IPO). Catalysts: GLM-5 release + JPMorgan Overweight initiation (see below).

Competing with [[ChatGPT]], [[Ernie]], [[Qwen]], [[DeepSeek]]. Enterprise API available. Part of [[China]]'s "rising stars" alongside Moonshot, MiniMax.

---

## Hong Kong IPO (Jan 8, 2026)

First major Chinese AI startup to IPO — beat [[OpenAI]] and [[Anthropic]] to public markets.

| Metric | Value |
|--------|-------|
| IPO size | $558M |
| IPO price | HK$116.20 |
| First day close | HK$131.50 (+13.2%) |
| Post-debut (Jan 10) | +36% from IPO price |
| Valuation | $7.4B |
| Retail subscription | 1,159x oversubscribed |

Use of proceeds: 70% to R&D for general-purpose AI models.

Investors (pre-IPO): Alibaba, [[Tencent]], [[Meituan]], [[Xiaomi]], [[Saudi Aramco]], Prosperity7

Part of wave of Chinese AI/chip firms listing in HK:

| Company | Date | Performance |
|---------|------|-------------|
| [[Biren Tech]] | Jan 3 | +76% |
| Zhipu | Jan 8 | +13.2% |
| [[MiniMax]] | Jan 9 | — |

---

## Shanghai dual listing (Feb 2026)

Planning a second listing on [[Shanghai]]'s Star Market following the HK IPO. Hired [[Guotai Haitong Securities]] and [[CICC]] as advisers. Stock jumped 23.4% to record HK$496 on the news (Feb 13), extending gain since IPO to ~320%.

If completed, Zhipu would trade on both [[HKEX]] and SSE — accessing mainland investor pools and government-linked capital.

---

## JPMorgan initiation (Feb 10, 2026)

[[JPMorgan]] analyst Olivia Xu initiated coverage with Overweight rating and HK$400 price target. Key calls:

| Metric | JPMorgan view |
|--------|---------------|
| Rating | Overweight |
| Target | HK$400 |
| Sales CAGR | 127% through 2030 |
| Profitability | 2029 |
| Thesis | Potential leader in next phase of global AI value creation |

JPMorgan argues China's AI industry is consolidating from 200+ model developers to <10, and Zhipu is positioned to survive as an independent foundation model lab with expanding international footprint.

Also initiated [[MiniMax]] at Overweight (HK$700 target). [[Goldman Sachs]] initiated around the same time.

Stock climbed 24% to record high on the initiation day alone, before GLM-5 added another leg up.

---

## Pricing power

February 2026: Zhipu hiked prices on GLM Coding Plan subscriptions by at least 30%, citing rapid user growth. Base coding plan remains ~20 yuan/month (~$3) — roughly 1/7 the price of [[Claude]] Pro.

Tension: [[DeepSeek]]'s aggressive API pricing (Q4 2025) pressured public cloud margins, pushing Zhipu to double down on higher-margin enterprise/on-prem contracts (84.5% of 2024 revenue was on-prem/privatized cloud).

---

## Competitive position

[[China]] AI labs:

| Company | Backing | Model |
|---------|---------|-------|
| [[DeepSeek]] | High-Flyer quant fund | R1/V3 |
| [[Alibaba]] | Alibaba Group | [[Qwen]] |
| [[ByteDance]] | ByteDance | [[Doubao]] |
| [[Baidu]] | Baidu | [[Ernie]] |
| Zhipu | Independent | GLM |
| [[MiniMax]] | Independent | Various |
| Moonshot | VC-backed | Kimi |

---

## Investment case

Bull:
- First publicly traded pure-play foundation model developer globally
- JPMorgan Overweight, 127% sales CAGR projection through 2030
- GLM-5 competitive with frontier Western models on coding/agentic benchmarks
- On-prem/enterprise model (84.5% of revenue) = stickier, higher-margin than API
- International expansion via "sovereign AI" alliances (ASEAN, Belt and Road)
- Successfully training on domestic chips ([[Huawei]] Ascend) — reduces Entity List impact

Bear:
- Cumulative losses of RMB 6.2B, breakeven not until 2029
- Cloud gross margin collapsed to -0.4% (H1 2025) under [[DeepSeek]] pricing pressure
- P/S ~150x on 2024 revenue — priced for perfection
- US Entity List (Jan 2025) cuts off [[NVIDIA]] H100/H200 access
- Crowded market consolidating — may not survive the shakeout
- R&D burn at 7-8x revenue is unsustainable without continued capital raises

M&A angle (Jan 2026): Bloomberg Opinion (Parmy Olson) named Zhipu and Moonshot AI as potential US acquisition targets for Alphabet or [[Microsoft]]. However, HK IPO makes outright acquisition less likely than if still private. See [[AI consolidation]].

---

## Financials

### Revenue (RMB)

| Period | Revenue | YoY |
|--------|---------|-----|
| 2022 | 57.4M | — |
| 2023 | 124.5M | +117% |
| 2024 | 312.4M | +151% |
| H1 2025 | 190.9M | — |

Revenue CAGR 2022-2024: ~130%. On-prem/privatized cloud: 84.5% of 2024 revenue. API & SDK licenses saw 30x YoY growth in 2024.

### Losses (RMB)

| Period | Net loss | R&D spend |
|--------|----------|-----------|
| 2022 | (143.7M) | — |
| 2023 | (788.0M) | — |
| 2024 | (2,958.0M) | ~2,200M (7x revenue) |
| H1 2025 | (2,357.9M) | 1,590M (8x revenue) |

Cumulative net loss 2022-H1 2025: RMB 6.2B (~$888M). Cumulative R&D: >RMB 4.4B.

Cloud deployment gross margin collapsed: 76.1% (2022) → -0.4% (H1 2025) — reflects intense price competition from [[DeepSeek]] and others.

### Valuation

| Metric | Value |
|--------|-------|
| Market cap | ~$22B (Feb 13, 2026) |
| P/S (2024 revenue) | ~175x |
| P/S (2025E) | ~39x |
| JPMorgan breakeven | 2029 |
| IPO proceeds | $558M (70% to R&D) |

China AI market rank: #1 among independent foundation model labs, #3 overall (after [[Baidu]] and [[Alibaba]]) per IDC 2024 (~18% share).

---

## Quick stats

| Metric | Value |
|--------|-------|
| Ticker | 2513.HK |
| IPO date | Jan 8, 2026 |
| IPO price | HK$116.20 |
| Current price | HK$496 (Feb 13) |
| Market cap | ~$22B |
| 2024 revenue | RMB 312M |
| Net loss (H1 2025) | RMB (2.36B) |
| HQ | Beijing |
| Founders | Tang Jie, [[Liu Debing]], Li Juanzi ([[Tsinghua University|Tsinghua]]) |
| Liu Debing net worth | $1.58B (Bloomberg Feb 2026) |
| Analyst | JPMorgan OW, PT HK$400 |

*Updated 2026-02-13*

---

## Related

### Products
- [[GLM]] — AI model family (GLM-5: 744B params, Feb 2026)

### Competitors/Peers
- [[MiniMax]] — peer (HK IPO same week, also JPMorgan OW)
- [[Moonshot AI]] — peer (Tiger, private)
- [[Biren Tech]] — peer (HK IPO, chips)
- [[DeepSeek]] — competitor ([[China]] AI leader)

### Context
- [[JPMorgan]] — coverage (Overweight, HK$400 PT, Feb 2026)
- [[Goldman Sachs]] — coverage (initiated Feb 2026)
- [[Asia IPO pipeline 2026]] — context
- [[US-China AI race]] — context
- [[AI consolidation]] — potential US acquisition target

---
aliases: [Sequoia]
---
#actor #vc #usa

**Sequoia Capital** — Legendary VC. Early NVIDIA investor. Backed OpenAI, Google, Apple. 2000+ portfolio companies.

---

## Why Sequoia matters

Most storied VC firm, early on transformative companies:

| Metric | Value |
|--------|-------|
| Founded | 1972 |
| Portfolio companies | 2,000+ |
| Exits | 400+ |
| Famous bets | Apple, Google, NVIDIA, Cisco |

---

## AI portfolio

**Current AI investments:**

| Company | Category |
|---------|----------|
| [[OpenAI]] | Frontier lab (major) |
| [[NVIDIA]] | Early investor |
| Stability AI | Image generation |
| Character.AI | Consumer AI |
| Harvey | Legal AI |
| Typeface | Enterprise AI |

---

## NVIDIA relationship

**Early investor:**
- Backed NVIDIA pre-IPO
- Massive returns as AI drove GPU demand
- Demonstrates long-term thesis conviction

One of best VC investments ever.

---

## Investment approach

**"We help the daring build legendary companies"**

| Stage | Check size |
|-------|------------|
| Seed | $500K-$2M |
| Series A-D | $10M-$100M |
| Growth | $100M+ |

Multi-stage investor, follows winners.

---

## Sequoia restructuring (2022-2024)

**Major changes:**
- Split from China/India operations
- Now separate: Sequoia Capital, HongShan (China), Peak XV (India)
- US/Europe entity focused
- Evergreen fund structure

---

## Semiconductor exposure

**Direct + indirect:**
- NVIDIA = direct chip exposure
- OpenAI = indirect (buys NVIDIA)
- Portfolio companies drive chip demand

More chip-exposed than most VCs via NVIDIA history.

---

## Thesis on AI

**AI as platform shift:**
- Comparable to internet, mobile
- Both infrastructure and applications
- Aggressive deployment of capital
- Willing to write large checks

---

## Notable partners

| Partner | Focus |
|---------|-------|
| Roelof Botha | Managing partner |
| Alfred Lin | Growth |
| Pat Grady | Enterprise |

Deep bench across stages.

---

## Investment case (as signal)

**What Sequoia bets tell us:**
- Platform shifts = big opportunities
- Patience on frontier tech pays off
- Infrastructure investments compound

Their NVIDIA bet = template for long-term thesis investing.

---

## Quick stats

| Metric | Value |
|--------|-------|
| Type | Venture Capital |
| AUM | $30B+ |
| HQ | Menlo Park, CA |
| Founded | 1972 |
| Famous bets | Apple, Google, NVIDIA |

*Updated 2026-01-01*

---

## Related

- [[NVIDIA]] — portfolio (early investor, massive return)
- [[OpenAI]] — portfolio (major position)
- [[a16z]] — peer (top-tier VC)
- [[Lightspeed Venture Partners]] — peer (top-tier VC)
- [[Benchmark]] — peer (top-tier VC)

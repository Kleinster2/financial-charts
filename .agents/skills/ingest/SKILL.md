...
- the `/event-tape` skill — adjusted market-reaction windows, peer read-through, and sigma stack
...
**Public-company M&A reaction trigger.** If the source is a merger, acquisition, take-private, spin-merger, or strategic combination involving any listed company, enumerate market-reaction data as first-class data points — the full merger-arb field set lives in `docs/note-checklist.md#public-company-ma-market-reaction`. Treat this as separate from the strategic rationale: strategic logic answers why the deal could make sense; market reaction records whether investors agreed.

**Listed-company event / peer-reaction trigger.** If the source is a joint venture, strategic partnership, product launch, financing, customer win/loss, regulatory event, or competitive announcement involving listed companies or a listed peer set, run `/event-tape` before writing the analysis — the required measurements and checklist live in `docs/note-checklist.md#listed-company-event--peer-reaction`. If local price data is stale or missing, verify with an external historical-price source rather than leaving `TODO verify`; a `## Market Reaction` section with unresolved TODO/TBD/verify placeholders is not complete.

**Physical-bottleneck trigger.** If the source mentions infrastructure capital, MW/GW capacity, data centers, power, grid access, interconnections, PPAs, sites, permits, construction, transformers, cooling, "long-term capacity," or "supply assurance," enumerate the physical constraint alongside the capital stack: who provides money, who provides technology/offtake, and who converts that into powered/operable capacity? Checklist and constraint hubs: `docs/note-checklist.md#physical-bottleneck--constraint-cluster-sweep`.
...
**Public-company M&A event gate:** when the event is M&A involving at least one listed company, the event note must include `## Market Reaction` covering the full checklist in `docs/note-checklist.md#public-company-ma-market-reaction`, closing with one sentence answering "what did the tape believe?" Mirror the short version into the public acquirer/target securities notes as `## Market reaction` sections that link back to the event note.

**Listed-company event / peer-reaction gate:** when the event is not M&A but changes the competitive or financing map for listed actors, the event note must include a completed `## Market Reaction` section built with `/event-tape` per the checklist in `docs/note-checklist.md#listed-company-event--peer-reaction`. Do not mark the note done with `TODO verify`, `TBD`, or "verify later" inside that section. If the market has not closed yet, write the status explicitly ("same-day close not yet available as of HH:MM ET") and log a daily-note follow-up to fill it after the close.
